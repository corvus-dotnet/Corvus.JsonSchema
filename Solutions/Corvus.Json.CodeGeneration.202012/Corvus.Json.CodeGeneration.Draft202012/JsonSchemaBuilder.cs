...
public class JsonSchemaBuilder : IJsonSchemaBuilder
{
    ...
    /// </summary>
    private TypeDeclaration BuildTypeDeclarationFor(string location, Schema draft202012Schema)
    {
        ...
        if (!draft202012Schema.Validate(ValidationContext.ValidContext).IsValid)
        {
            throw new InvalidOperationException("Unable to build types for an invalid schema.");
        }

        if (this.TryReduceSchema(location, draft202012Schema, out TypeDeclaration? reducedTypeDeclaration))
        {
        ...
        var result = new TypeDeclaration(location, draft202012Schema);
    ...
    private bool TryReduceSchema(string absoluteLocation, Schema draft202012Schema, [NotNullWhen(true)] out TypeDeclaration? reducedTypeDeclaration)
    {
        if (draft202012Schema.IsNakedReference())
        {
            return this.ReduceSchema(absoluteLocation, out reducedTypeDeclaration, "$ref");
        }

        if (draft202012Schema.IsNakedRecursiveReference())
        {
            return this.ReduceSchema(absoluteLocation, out reducedTypeDeclaration, "$recursiveRef");
        }

        if (draft202012Schema.IsNakedDynamicReference())
        {
    ...
    private void AddPropertiesFromType(TypeDeclaration source, TypeDeclaration target, HashSet<string> typesVisited, bool treatRequiredAsOptional)
    {
        ...
        if (source.Schema.Ref.IsNotUndefined() && !source.Schema.IsNakedReference())
        {
            ...
            this.AddPropertiesFromType(refTypeDeclaration, target, typesVisited, false);
        }
