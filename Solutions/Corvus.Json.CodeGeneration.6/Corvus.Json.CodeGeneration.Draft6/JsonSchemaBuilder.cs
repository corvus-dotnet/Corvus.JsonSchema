...
public class JsonSchemaBuilder : IJsonSchemaBuilder
{
    ...
    /// </summary>
    private TypeDeclaration BuildTypeDeclarationFor(string location, Schema draft6Schema)
    {
        ...
        if (!draft6Schema.Validate(ValidationContext.ValidContext).IsValid)
        {
            throw new InvalidOperationException("Unable to build types for an invalid schema.");
        }

        if (this.TryReduceSchema(location, draft6Schema, out TypeDeclaration? reducedTypeDeclaration))
        {
        ...
        var result = new TypeDeclaration(location, draft6Schema);
    ...
    private bool TryReduceSchema(string absoluteLocation, Schema draft6Schema, [NotNullWhen(true)] out TypeDeclaration? reducedTypeDeclaration)
    {
        if (draft6Schema.IsNakedReference())
        {
