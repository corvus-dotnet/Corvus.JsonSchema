...
/// </summary>
/// <typeparam name="TSchema">The type of the schema for the type declaration.</typeparam>
/// <typeparam name="TPropertyDeclaration">The the type of property for the property declaration.</typeparam>
/// <typeparam name="TTypeDeclaration">The implementing type.</typeparam>
public abstract class TypeDeclaration<TSchema, TPropertyDeclaration, TTypeDeclaration>
...
{
    /// <summary>
    /// The location for built-in types.
    /// </summary>
    protected const string BuiltInsLocation = "https://github.com/Corvus-dotnet/Corvus/tree/master/schema/builtins";

    private ImmutableArray<TTypeDeclaration> children = ImmutableArray<TTypeDeclaration>.Empty;
    private ImmutableArray<TPropertyDeclaration> properties = ImmutableArray<TPropertyDeclaration>.Empty;

    /// <summary>
    /// Initializes a new instance of the <see cref="TypeDeclaration{TSchema, TPropertyDeclaration, TTypeDeclaration}"/> class.
    /// </summary>
    ...
    protected TypeDeclaration(string absoluteLocation, string lexicalLocation, TSchema schema)
    {
    ...
    /// <param name="schema">The schema with which this type declaration is associated.</param>
    protected TypeDeclaration(string location, TSchema schema)
    {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="TypeDeclaration{TSchema, TPropertyDeclaration, TTypeDeclaration}"/> class for a built-in type.
    /// </summary>
    /// <param name="schema">The schema for the built-in type.</param>
    protected TypeDeclaration(TSchema schema)
    ...
    protected TypeDeclaration(string? ns, string typename)
        : this(BuiltInsLocation, new TSchema())
    {
        this.SetNamespaceAndTypeName(ns, typename);
    }

    /// <summary>
    /// Gets a value indicating whether this schema is a built-in type.
    /// </summary>
    public bool IsBuiltInType => this.Location == BuiltInsLocation || this.SchemaIsBuiltInType;
    ...
    /// </summary>
    public TTypeDeclaration? Parent { get; private set; }

    /// <summary>
    /// Gets the set of <see cref="TypeDeclaration{TSchema, TPropertyDeclaration, TTypeDeclaration}"/> instances referenced by this <see cref="TypeDeclaration{TSchema, TPropertyDeclaration, TTypeDeclaration}"/>.
    /// </summary>
    public ImmutableHashSet<TTypeDeclaration>? ReferencedTypes { get; private set; }

    /// <summary>
    /// Gets the embedded children of this type.
    /// </summary>
    public ImmutableArray<TTypeDeclaration> Children => this.children;

    /// <summary>
    /// Gets the list of property declarations in this type.
    /// </summary>
    public ImmutableArray<TPropertyDeclaration> Properties => this.properties;
    ...
    /// Gets a value indicating whether this schema is a built-in type.
    /// </summary>
    protected abstract bool SchemaIsBuiltInType { get; }

    /// <summary>
    /// Gets the simple type string for the schema.
    /// </summary>
    protected abstract string? SchemaSimpleType { get; }

    /// <summary>
    /// Gets the format for the schema.
    /// </summary>
    protected abstract string? SchemaFormat { get; }

    /// <summary>
    /// Gets a value indicating whether the schema is empty.
    /// </summary>
    protected abstract bool SchemaIsEmpty { get; }

    /// <summary>
    /// Gets the content encoding for the schema.
    /// </summary>
    protected abstract string? SchemaContentEncoding { get; }

    /// <summary>
    /// Gets the content encoding for the schema.
    /// </summary>
    protected abstract string? SchemaContentMediaType { get; }

    /// <summary>
    /// Gets a value indicating whether this is the 'any' schema type.
    /// </summary>
    protected abstract bool SchemaIsTrue { get; }

    /// <summary>
    /// Gets a value indicating whether this is the 'any' schema type.
    /// </summary>
    protected abstract bool SchemaIsFalse { get; }

    /// <summary>
    /// Gets a value indicating whether the schema has an enum constraint.
    /// </summary>
    protected abstract bool SchemaHasEnum { get; }

    /// <summary>
    /// Gets a value indicating whether the schema is explicitly an array type.
    /// </summary>
    protected abstract bool SchemaIsExplicitArrayType { get; }

    /// <summary>
    /// Gets a value indicating whether the schema is a simple type without other constraints.
    /// </summary>
    protected abstract bool SchemaIsSimpleType { get; }

    /// <summary>
    /// Sets the parent type declaration.
    /// </summary>
    /// <param name="parent">The parent type declaration.</param>
    public void SetParent(TTypeDeclaration parent)
    {
        if (this.Parent is not null)
        {
            ...
            this.Parent.RemoveChild((TTypeDeclaration)this);
        }

        this.Parent = parent;
        parent.AddChild((TTypeDeclaration)this);
    }
    ...
    /// <remarks>
    /// A builder should call <see cref="SetParent(TTypeDeclaration)"/> before calling set name.
    /// </remarks>
    public void SetDotnetTypeNameAndNamespace(string rootNamespace, string fallbackBaseName)
    {
        ...
        else
        {
            ...
            if (this.SchemaIsExplicitArrayType)
            {
            ...
            else if (this.SchemaIsSimpleType)
            {
                Span<char> dnt = stackalloc char[typename.Length + 5];
                typename.CopyTo(dnt);
    ...
    public void SetReferencedTypes(HashSet<TTypeDeclaration> referencedTypes)
    {
    ...
    public void AddOrReplaceProperty(TPropertyDeclaration propertyDeclaration)
    {
        int index = this.properties.IndexOf(propertyDeclaration, TPropertyDeclarationEqualityComparer.Instance);
        if (index >= 0)
        {
    ...
    public void SetDotnetPropertyNames()
    {
        ...
        if (this.SchemaHasEnum)
        {
            existingNames.Add("EnumValues");
        }

        foreach (TPropertyDeclaration property in this.properties)
        {
            SetDotnetPropertyName(existingNames, property);
        }

        static void SetDotnetPropertyName(HashSet<string> existingNames, TPropertyDeclaration property)
        {
    ...
    private string BuildFullyQualifiedDotnetTypeName(string dotnetTypeName)
    {
        ...
        var rootParent = (TTypeDeclaration)this;
        TTypeDeclaration? parent = this.Parent;
        while (parent is TTypeDeclaration p)
        {
    ...
    private void AddChild(TTypeDeclaration typeDeclaration)
    {
        this.children = this.children.Add(typeDeclaration);
    }

    private void RemoveChild(TTypeDeclaration typeDeclaration)
    {
    ...
    private void MergeProperties(int index, TPropertyDeclaration propertyDeclaration)
    {
        TPropertyDeclaration? original = this.properties[index];

        // Merge whether this is a required property with the parent
        TPropertyDeclaration propertyToAdd =
    ...
    private class TPropertyDeclarationEqualityComparer : IEqualityComparer<TPropertyDeclaration>
    {
        public static readonly TPropertyDeclarationEqualityComparer Instance = new();

        public bool Equals(TPropertyDeclaration? x, TPropertyDeclaration? y)
        {
            return x is TPropertyDeclaration x1 && y is TPropertyDeclaration y1 && x1.JsonPropertyName == y1.JsonPropertyName;
        }

        public int GetHashCode(TPropertyDeclaration obj)
        {
