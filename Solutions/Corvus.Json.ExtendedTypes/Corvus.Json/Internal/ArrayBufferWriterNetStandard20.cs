...
namespace Corvus.Json
{
    ...
    sealed class ArrayBufferWriter<T> : IBufferWriter<T>
    {
        ...
        {
            throw new InvalidOperationException($"Buffer writer advanced too far at position {capacity}");
        }

        private static void ThrowOutOfMemoryException(uint capacity)
        {
            throw new OutOfMemoryException($"Buffer maximum size exceeded at {capacity}");
        }
    }
}
