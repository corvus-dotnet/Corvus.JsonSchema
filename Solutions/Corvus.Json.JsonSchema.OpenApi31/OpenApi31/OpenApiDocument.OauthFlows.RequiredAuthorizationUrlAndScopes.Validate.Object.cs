            ...
            private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                foreach (JsonObjectProperty property in this.EnumerateObject())
                {
                    if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
                    {
                        ...
                        if ((this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.AuthorizationUrlUtf8)) || (!this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.AuthorizationUrl)))
                        {
                            foundAuthorizationUrl = true;
                        }
                        else if ((this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.ScopesUtf8)) || (!this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.Scopes)))
                        {
                            foundScopes = true;
                        }
                    }
