    ...
    public readonly partial struct SecurityScheme
    {
        ...
        public readonly partial struct TypeHttpBearerEntity
        {
            private ValidationContext ValidateIfThenElse(in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                ValidationContext ifResult = this.As<Corvus.Json.JsonSchema.OpenApi31.OpenApiDocument.SecurityScheme.TypeHttpBearerEntity.RequiredTypeAndScheme>().Validate(validationContext.CreateChildContext(), level);
                if (!ifResult.IsValid)
                {
                ...
                if (ifResult.IsValid)
                {
                    ...
                    ValidationContext thenResult = this.As<Corvus.Json.JsonSchema.OpenApi31.OpenApiDocument.SecurityScheme.TypeHttpBearerEntity.ThenEntity>().Validate(validationContext.CreateChildContext(), level);
