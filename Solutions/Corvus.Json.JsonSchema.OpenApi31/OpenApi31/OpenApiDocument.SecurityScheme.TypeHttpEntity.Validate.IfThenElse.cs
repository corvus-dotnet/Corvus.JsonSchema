    ...
    public readonly partial struct SecurityScheme
    {
        ...
        public readonly partial struct TypeHttpEntity
        {
            private ValidationContext ValidateIfThenElse(in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                ValidationContext ifResult = this.As<Corvus.Json.JsonSchema.OpenApi31.OpenApiDocument.SecurityScheme.TypeHttpEntity.RequiredType>().Validate(validationContext.CreateChildContext(), level);
                if (!ifResult.IsValid)
                {
                ...
                if (ifResult.IsValid)
                {
                    ...
                    ValidationContext thenResult = this.As<Corvus.Json.JsonSchema.OpenApi31.OpenApiDocument.SecurityScheme.TypeHttpEntity.RequiredScheme>().Validate(validationContext.CreateChildContext(), level);
