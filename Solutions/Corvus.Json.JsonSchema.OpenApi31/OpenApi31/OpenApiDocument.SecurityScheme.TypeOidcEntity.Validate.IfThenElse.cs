    ...
    public readonly partial struct SecurityScheme
    {
        ...
        public readonly partial struct TypeOidcEntity
        {
            private ValidationContext ValidateIfThenElse(in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                ValidationContext ifResult = this.As<Corvus.Json.JsonSchema.OpenApi31.OpenApiDocument.SecurityScheme.TypeOidcEntity.RequiredType>().Validate(validationContext.CreateChildContext(), level);
                if (!ifResult.IsValid)
                {
                ...
                if (ifResult.IsValid)
                {
                    ...
                    ValidationContext thenResult = this.As<Corvus.Json.JsonSchema.OpenApi31.OpenApiDocument.SecurityScheme.TypeOidcEntity.RequiredOpenIdConnectUrl>().Validate(validationContext.CreateChildContext(), level);
