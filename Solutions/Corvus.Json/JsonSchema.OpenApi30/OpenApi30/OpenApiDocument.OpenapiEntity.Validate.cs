                ...
                ValidationLevel level = ValidationLevel.Flag)
            {
                ...
                static bool StringValidator(ReadOnlySpan<char> input, in Corvus.Json.Validate.ValidationContextWrapper context, out ValidationContext result)
                {
                    ...
                    if (Pattern.IsMatch(input))
                    {
                        ...
                        {
                            result = result.WithResult(isValid: true, $"Validation pattern - {input.ToString()} matched  '^3\\.0\\.\\d(-.+)?$'");
                        }
                    }
                    else
                    {
                        ...
                        {
                            result = result.WithResult(isValid: false, $"Validation pattern - {input.ToString()} did not match  '^3\\.0\\.\\d(-.+)?$'");
                        }
                        else if (context.Level >= ValidationLevel.Basic)
                        {
                            result = result.WithResult(isValid: false, "Validation pattern - The value did not match  '^3\\.0\\.\\d(-.+)?$'");
                        }
                        else
                        {
