...
- **`Corvus.Text.Json.Validator` no longer compiles or loads generated code.** `DynamicJsonElement`, `DynamicJsonType`, `DynamicAssemblyLoadContext`, `ParseInstance`, `FromElement`, `HostAssembly` and the `IDocumentResolver` and `FallbackVocabulary` options are removed, and `JsonSchema` is a struct whose `Validate` overloads evaluate directly; the `AdditionalDocumentResolver` option is now the `JsonSchemaDocumentResolver` delegate of the evaluator. Callers that only created a `JsonSchema` and called `Validate` are unaffected.
