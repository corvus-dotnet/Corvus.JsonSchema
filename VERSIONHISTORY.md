...
- **A string bound from the wire is validated as its value, not as its escaped text.** A generated OpenAPI server binds a string query, header or path parameter into a `FixedJsonValueDocument`, which holds the value as JSON text, so a character the encoder escapes is stored as its escape (a `+` as `\u002B`). The document's own string accessors unescape, but its `ValueIsEscaped` answered a hard-coded `false`, which dates from when the type held only expression results and predates the escaping factory. The runtime evaluator introduced in V5.6.0 asks the document that question and, when the answer is `false`, reads the raw text as the value. That is correct for a document that answers truthfully, which is why the JSON Schema Test Suite was unaffected: every case there is a parsed document. For a bound parameter every string keyword was evaluated against the escaped text. A valid `date-time` with a numeric offset such as `2026-01-01T01:00:00+00:00` was refused with a `400` while the same instant written with `Z` was accepted, `minLength` and `maxLength` counted an escaped character as six, and a `pattern` was matched against the escaped text, so a value containing `<` satisfied a pattern written to exclude it. Deep equality trusts the same answer, so an escaped fixed string did not equal the same string parsed from JSON. `ValueIsEscaped` now reports what the text holds, decided the way the accessors decide it, as a stateless scan that cannot go stale when a pooled instance is reused. Request bodies are parsed documents and were never affected. Servers generated by V5.6.0 or V5.7.0 are corrected by taking the updated `Corvus.Text.Json` package; no regeneration is needed. See [#971](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/971).

## V5.7.0

V5.7.0 makes every generated `oneOf`/`anyOf` type a C# union, orders and documents generated code deterministically and more compactly, gives the CLI a lock file per output folder and YAML input on every command, cuts what generation allocates and holds, and moves the build to the .NET 11 SDK. The ordering change is the breaking one. For some schemas it reorders positional `Create` parameters and `[Flags]` bits and renames a heuristic type name, so regenerate and recompile.
...
- **Everything generated is ordered ordinally.** Property members, subschema types, positional `Create` parameters, `[Flags]` enum bits and documentation keywords follow the ordinal (UTF-16 code unit) order of the JSON names, whatever the culture of the machine that runs the generator; the V4 engine keeps its order. Under an English culture most output is unchanged; property sets with mixed case or non-ASCII names, or names that culture-aware comparison ordered differently, reorder their positional `Create` parameters and flag bits, and the required-property and constant-property naming heuristics can choose a different name. Numbers written into generated code use the invariant culture, so a negative constant or enum value compiles under every culture. The shared simple types (`JsonString` and the rest) are emitted in the order they are first seen, so two generations of one schema produce files in the same order. See [#966](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/966).

- **Smaller files, same code.** Every generated file starts with the single-line `// <auto-generated/>` header, so analyzers that skip generated code skip it. A type's documentation and `[DebuggerDisplay]` are emitted once, in its core partial; non-public members carry no XML documentation; and members that mirror a `JsonElement` member (`TryGetProperty`, the indexers, the enumerators, `ParseValue`, `Clone`, `Freeze`, the equality operators, the mutable `SetProperty` and `RemoveProperty`, and more) carry `/// <inheritdoc cref="global::Corvus.Text.Json.JsonElement..."/>`, which IntelliSense and documentation tools resolve from the `Corvus.Text.Json.xml` file in the package. The AsyncAPI 3.0 corpus shrinks from 103 MB to 84 MB of source; a consumer build of it takes about 10 % less CPU and 7 % less peak memory, and its XML documentation file is 27 % smaller.

- **Fixes.** A colliding generated file name takes its index before the extension (`Foo1.cs`, not `Foo1..cs`) ([#965](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/965)); a type no heuristic names takes the fallback name rather than the length of that name ([#968](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/968)); a `patternProperties` regex whose `{m,n}` quantifier holds non-ASCII digits no longer crashes generation ([#964](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/964)); the checked-in `Corvus.Text.Json.AsyncApi26`, `Corvus.Text.Json.AsyncApi30` and migration model trees are regenerated with this generator.

### CLI

- **A lock file per output folder.** Every `jsonschema` run leaves a `corvusjson-jsonschema.lock` beside its output. It records the folder's whole generation specification, the hash of every local schema, the generator version and the files written. A later run into the same folder adds its schemas to that specification and regenerates the whole set as one `CorvusJsonSchemaProgram`, so the shared simple types are wired to the program that survives; a run that would change nothing is skipped, a run with other options is refused unless `--force` is passed, and files an earlier run wrote that this run no longer generates are deleted. See [#969](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/969) and the [lock file documentation](docs/CodeGenerator.md#lock-file-and-regeneration).

- **YAML on every command.** `openapi-server` and `openapi-callback-server` read YAML specifications as `openapi-client` already did; every OpenAPI and AsyncAPI command shares one specification reader. See [#967](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/967).

- **Files are written as they are generated,** into a staging directory under the output folder that replaces the output when generation succeeds, so a failed run leaves the folder as it was and the process never holds the whole output in memory (peak memory for the AsyncAPI 3.0 schema falls from 285 MB to 128 MB).

### Source generator

- **Analyzer diagnostics are hidden in generated code.** The package's build targets pass an analyzer configuration that sets the bulk analyzer severity to `none` over this generator's trees; opt out with `<CorvusTextJsonSuppressAnalyzerDiagnosticsInGeneratedCode>false</CorvusTextJsonSuppressAnalyzerDiagnosticsInGeneratedCode>`. Analyzers that opt into generated code still execute on those trees in the IDE and in `csc`, which is a compiler limitation; the [documentation](docs/SourceGenerator.md#analyzers-and-generated-code) explains what a project can configure. A second generation in a long-lived host no longer fails with CS8785 ([#963](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/963)), and an unchanged generation is reused across incremental runs.

### Runtime

- `JsonWorkspace` validates a document's cached index before using it, so a document shared between workspaces on different threads resolves to its own index ([#928](https://github.com/corvus-dotnet/Corvus.JsonSchema/issues/928)).

### Performance

- Generation allocates about a third less than 5.6 (schema documents reach the evaluator program compiler as UTF-8; keyword sets, location lookups and reference walks no longer copy strings; type declaration metadata is a plain dictionary with shared value boxes; the source generator reuses an unchanged generation, so a re-run after an unrelated file edit takes 11 ms instead of 310 ms for OpenAPI 3.1). Generated files are held in chunks below the large object heap and written without a string: in-process generation of the AsyncAPI 3.0 schema holds 35 % less peak memory, and the two runtime slow paths the profile singled out (uncached nullable-tuple casts in the metadata lookups and a lock on every read of the shared declarations) are gone.
