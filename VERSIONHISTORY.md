...
- **A runtime JSON Schema evaluator, `Corvus.Text.Json.RuntimeEvaluator.JsonSchemaEvaluator`.** `Compile` takes a schema as UTF-8 or text (or `CompileFromUri` resolves one), for drafts 4, 6, 7, 2019-09 and 2020-12 with `$ref`, `$dynamicRef` and `$recursiveRef` across documents; `Evaluate` takes an `IJsonElement<T>`, a document and index, or JSON text, and returns whether the instance is valid, with an optional `IJsonSchemaResultsCollector` for basic, detailed or verbose results and annotations. Flag-mode evaluation of a parsed document allocates nothing. `JsonSchemaEvaluatorOptions` sets the default dialect, format assertion (`AssertFormat`, `AssertFormatInLegacyDrafts`, per-format `FormatModes`), content assertion, regular-expression compilation and timeouts or a `RegexProvider` of pre-built expressions, document resolvers, the base URI, the entry point and the depth limit. `ForEntryPoint` and `RegisterEntryPoints` give evaluators for other schemas of the same document set, sharing every already-compiled subschema.

- **Program images.** `ToProgramImage` serialises a compiled evaluator, and `FromProgramImage` loads it without the schema documents or the compiler's work; `GetImagePatterns` lists the regular expressions an image needs and `ToDotNetPattern` converts an ECMA pattern to .NET syntax, so a host can precompile them. The source generator and the `corvusjson` CLI embed an image in every generated set as the partial class `CorvusJsonSchemaProgram`, with one `[GeneratedRegex]` method per pattern the image cannot match without a regular expression (on .NET 8 and later), so generated types validate through the evaluator with no compile step at run time. `Corvus.Text.Json.CodeGeneration` exposes the hook (`SchemaProgramCompiler`, `SchemaProgramSource`, `SchemaProgramImage`, `CSharpLanguageProvider.Options.ProgramCompiler`) for other hosts of the generator.

- **Generated types gain `EvaluateSchema(IJsonSchemaResultsCollector? resultsCollector = null)`**, the evaluation of the instance against the type's schema through the embedded program, alongside the existing validation surface.

- **`Corvus.Text.Json.Validator` runs the runtime evaluator.** `JsonSchema.FromText`, `FromStream`, `FromFile`, `FromUri` and `From` compile the schema in process, and `Validate` accepts text, UTF-8 memory or sequence, a stream or a `JsonElement`. There is no Roslyn compilation, no dynamic assembly and no first-validation delay: a schema is ready in milliseconds. `JsonSchema.Options` gains `DefaultDialect`, `AlwaysAssertFormat` and an `AdditionalDocumentResolver` delegate, with `PrepopulatedDocuments` for schemas supplied in memory.

- **Native AOT and ReadyToRun guidance and support.** The `Corvus.Text.Json` package carries `profiles/Corvus.Text.Json.mibc`, a static optimisation profile of the evaluator recorded by the build from the code being packaged, and `buildTransitive/Corvus.Text.Json.targets` adds it as a `MibcFile` when an application publishes with `PublishAot` (`CorvusTextJsonUseProfile=false` leaves it out; `CorvusTextJsonProfile` substitutes another). With it, native AOT evaluates within about 6% of the JIT's steady state and starts in about 5 ms; the consumer guidance in [RuntimeEvaluator.md](docs/RuntimeEvaluator.md) ("Publishing an application") covers native AOT for one-shot processes, the JIT for long-running ones, and the ReadyToRun trade-off. The `corvusjson` CLI can be published as native AOT (`-p:CliAot=true -p:CliAotRooted=true`), which takes its `validateDocument` command from 182 ms to 11 ms.

- **Direct row access for evaluators of parsed documents.** `JsonDocument.TryGetRawSpans` and `TryGetRawAccess` (virtual, overridden by `ParsedJsonDocument<T>`) and `RawDocumentAccess` expose a parsed document's metadata rows and UTF-8 text to code that walks a document without the element API; the evaluator's flag mode is built on them.

- **A benchmark harness and tools for the evaluator** under `benchmarks/Corvus.Text.Json.RuntimeEvaluator.Benchmarks`: per-corpus warm, cold, parse-and-evaluate, compile and allocation measurements for the runtime evaluator, program images, generated models, ReadyToRun and native AOT, against Blaze's CLI, with `tools/measure.sh` and `tools/README.md`; the end-of-branch tables are checked in under `docs/measurements`.

### Bug fixes

- **A parsed document's pooled buffers grow to cover a request.** The growth of the property-map buffers doubled a buffer and returned, which cannot cover a request larger than twice the buffer: a property map for an object with more entries than the entries buffer held, after small objects had rented a small one, threw on the slice that followed. The new capacity is the larger of double and the request.

### Breaking changes

- **The per-keyword validation handlers are gone from `Corvus.Text.Json.CodeGeneration`.** `StandaloneEvaluatorGenerator` and the public handler types under `ValidationHandlers` (`AllOfSubschemaValidationHandler`, `AnyOfConstValidationHandler`, `AnyOfSubschemaValidationHandler`, `OneOfSubschemaValidationHandler`, `ContainsValidationHandler`, `ItemsValidationHandler`, `ItemCountValidationHandler`, `UniqueItemsValidationHandler`, `PropertiesValidationHandler`, `PatternPropertiesValidationHandler`, `PropertyNamesValidationHandler`, `PropertyCountValidationHandler`, `UnevaluatedPropertyValidationHandler`, `NumberRangeValidationHandler`, `StringLengthValidationHandler`, `StringRegularExpressionValidationHandler`, `KeywordValidationHandlerBase`, `IJsonSchemaClassSetup`, `ITypeSensitiveKeywordValidationHandler`, `INamedPropertyChildHandler`, `ObjectPropertyValidatorParameter`) are removed: generated validation is the compiled program, not emitted code. The `SchemaEvaluationOnly` and `Both` generation modes still emit a standalone evaluator class; it delegates to the program. Code that extended the generator through those handlers has no replacement; keywords are compiled by the runtime evaluator (see [AddingKeywords.md](docs/AddingKeywords.md) and [RuntimeEvaluator.md](docs/RuntimeEvaluator.md)).

- **`Corvus.Text.Json.Validator` no longer compiles or loads generated code.** `JsonSchema` keeps its contract (`FromText`, `FromStream`, `FromFile`, `FromUri`, `From`, the six `Validate` overloads, the cache by canonical URI, `AdditionalSchemaFiles`, `AllowFileSystemAndHttpResolution`, `AlwaysAssertFormat`); callers that create a `JsonSchema` and call `Validate` are unaffected, and `FromUri` now accepts a fragment as the entry point. Three members of `JsonSchema.Options` were tied to the Roslyn pipeline and change: `FallbackVocabulary` (an `IVocabulary` of the code generator) becomes `DefaultDialect` (a `JsonSchemaDialect`), `AdditionalDocumentResolver` is a `JsonSchemaDocumentResolver` delegate rather than the generator's `IDocumentResolver`, and `HostAssembly` is removed. The dynamic-compilation types the package exposed (`DynamicJsonType`, `DynamicJsonElement`, `DynamicAssemblyLoadContext`, `DynamicCompiler`) move to the test utilities; hosts no longer need `PreserveCompilationContext`, and the package no longer depends on Roslyn or the V4 code generation packages.

- **The shape of generated validation code changes.** Every generated set now contains `CorvusJsonSchemaProgram.cs` (the embedded program image and its regular-expression table) in place of per-keyword validation methods; regenerate with the 5.6 generator to pick up the runtime evaluator, and expect the diff to be large the first time. Types generated by 5.5 keep working against the 5.6 library.
