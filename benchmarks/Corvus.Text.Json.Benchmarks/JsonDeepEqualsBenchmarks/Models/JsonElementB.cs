...
public readonly partial struct JsonElementB : IJsonElement<JsonElementB>
{
    private readonly IJsonDocument _parent;
    private readonly int _idx;

    internal JsonElementB(IJsonDocument parent, int idx)
    {
    ...
    ///   The parent <see cref="JsonDocument"/> has been disposed.
    /// </exception>
    public JsonElementB this[int index]
    {
        get
        {
            CheckValidInstance();

            return _parent.GetArrayIndexElement<JsonElementB>(_idx, index);
        }
    }
    ...
    /// <summary>
    ///   Gets a <see cref="JsonElementB"/> representing the value of a required property identified
    ...
    /// </param>
    /// <returns>
    ///   A <see cref="JsonElementB"/> representing the value of the requested property.
    /// </returns>
    /// <exception cref="InvalidOperationException">
    ///   This value's <see cref="ValueKind"/> is not <see cref="JsonValueKind.Object"/>.
    /// </exception>
    ...
    public JsonElementB GetProperty(ReadOnlySpan<byte> utf8PropertyName)
    {
        if (TryGetProperty(utf8PropertyName, out JsonElementB property))
        {
            return property;
        }
    ...
    ///   The parent <see cref="JsonDocument"/> has been disposed.
    /// </exception>
    public bool TryGetProperty(ReadOnlySpan<byte> utf8PropertyName, out JsonElementB value)
    {
        CheckValidInstance();
    ...
    public static void EnsurePropertyMap(in JsonElementB element)
    {
...
#pragma warning restore CA2265
    ...
    /// <summary>
    ///   Get an enumerator to enumerate the values in the JSON array represented by this JsonElementB.
    /// </summary>
    /// <returns>
    ///   An enumerator to enumerate the values in the JSON array represented by this JsonElementB.
    /// </returns>
    /// <exception cref="InvalidOperationException">
    ///   This value's <see cref="ValueKind"/> is not <see cref="JsonValueKind.Array"/>.
    /// </exception>
    ...
    public ArrayEnumerator<JsonElementB> EnumerateArray()
    {
        ...
        return EnumeratorCreator.CreateArrayEnumerator<JsonElementB>(_parent, _idx);
    }

    /// <summary>
    ///   Get an enumerator to enumerate the properties in the JSON object represented by this JsonElementB.
    /// </summary>
    /// <returns>
    ///   An enumerator to enumerate the properties in the JSON object represented by this JsonElementB.
    /// </returns>
    /// <exception cref="InvalidOperationException">
    ///   This value's <see cref="ValueKind"/> is not <see cref="JsonValueKind.Object"/>.
    /// </exception>
    ...
    public ObjectEnumerator<JsonElementB> EnumerateObject()
    {
        ...
        return EnumeratorCreator.CreateObjectEnumerator<JsonElementB>(_parent, _idx);
    }
    ...
    /// <remarks>
    ///   <para>
    ///     For JsonElementB built from <see cref="JsonDocument"/>:
    ///   </para>
    ///
    ///   <para>
    ...
    /// </summary>
    /// <returns>
    ///   A JsonElementB which can be safely stored beyond the lifetime of the
    ///   original <see cref="JsonDocument"/>.
    /// </returns>
    /// <remarks>
    ///   <para>
    ///     If this JsonElementB is itself the output of a previous call to Clone, or
    ///     a value contained within another JsonElementB which was the output of a previous
    ///     call to Clone, this method results in no additional memory allocation.
    ///   </para>
    /// </remarks>
    public JsonElementB Clone()
    {
        CheckValidInstance();
        ...
        return _parent.CloneElement<JsonElementB>(_idx);
    }
...
#if NET
    public static JsonElementB CreateInstance(IJsonDocument parentDocument, int parentDocumentIndex) => new(parentDocument, parentDocumentIndex);
#endif
