// <copyright file="BenchmarkInArray.cs" company="Endjin Limited">
...
/// <summary>
/// Benchmark for "in" operator: check if a value is in an array.
/// </summary>
[MemoryDiagnoser]
public class BenchmarkInArray : JsonLogicBenchmarkBase
{
    private const string RuleJson = """{"in":[{"var":"role"},["admin","editor","moderator","viewer","guest"]]}""";
    private const string DataJson = """{"role":"moderator"}""";
    ...
    {
        using JsonWorkspace workspace = JsonWorkspace.Create();
        InArrayCodeGen.Evaluate(this.CorvusData, workspace);
    }
}