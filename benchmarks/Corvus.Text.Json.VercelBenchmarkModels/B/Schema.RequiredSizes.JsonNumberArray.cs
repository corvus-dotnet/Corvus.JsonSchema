    ...
    public readonly partial struct RequiredSizes
    {
...
#endif
        {
            ...
            public bool TryGetNumericValues(Span<double> items, out int written)
            {
                CheckValidInstance();
                written = 0;
                foreach (Corvus.VercelBenchmark.Baseline.JsonNumber item in EnumerateArray())
                {
            ...
            public Corvus.VercelBenchmark.Baseline.JsonNumber this[int index]
            {
                get
                {
                    CheckValidInstance();
                    return _parent.GetArrayIndexElement<Corvus.VercelBenchmark.Baseline.JsonNumber>(_idx, index);
                }
            }
            ...
            /// </summary>
            /// <exception cref="InvalidOperationException">The value is not an array.</exception>
            public ArrayEnumerator<Corvus.VercelBenchmark.Baseline.JsonNumber> EnumerateArray()
            {
                CheckValidInstance();
                return EnumeratorCreator.CreateArrayEnumerator<Corvus.VercelBenchmark.Baseline.JsonNumber>(_parent, _idx);
            }
