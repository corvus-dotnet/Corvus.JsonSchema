...
| `--assertFormat` | `true` | Enforce the `format` keyword as a validation assertion (global default). Takes a value: pass `--assertFormat false` for annotation-only output (effective where the schema's vocabulary already treats `format` as an annotation, e.g. 2020-12; for drafts whose vocabulary asserts `format`, such as draft-04/06/07, use `--formatMode disable`). |
...
| `--unions` | `true` | Make each `oneOf`/`anyOf` type a C# union as well as giving it `Match`: with the C# 15 compiler (the .NET 11 SDK or later, any target framework) `switch` and `is` patterns over the branch types work. Pass `--unions false` to leave the union members out. |
...
| `unions` | No | `true` | When `true`, a `oneOf`/`anyOf` type is also a C# union (`switch` and `is` patterns over its branch types with the C# 15 compiler, the .NET 11 SDK or later, any target framework). Set to `false` to leave the union members out. |
