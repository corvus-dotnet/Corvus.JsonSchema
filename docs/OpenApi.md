...
By default the generated server buffers the whole multipart body before the handler runs, subject to the size limit described under [Request Body Size Limits](#request-body-size-limits). For OpenAPI 3.2 specs you can opt in to streaming instead:

```bash
corvusjson openapi-server petstore-3.2.json --rootNamespace PetStore.Server --outputPath Generated --serverBinaryParts stream
```

In streaming mode a `multipart/form-data` endpoint never buffers its binary parts. The non-binary parts are still read into the typed `Body` first (they are form fields, capped by `ApiServerOptions.MaxNonBinaryPartsLength`, 1 MiB by default), and each `format: binary` property becomes a `BinaryPartHandle` on the Params struct. Open the handle to read that part's bytes directly from the request:

```csharp
public async ValueTask<UploadPetPhotoResult> HandleUploadPetPhotoAsync(
    UploadPetPhotoParams parameters, JsonWorkspace workspace, CancellationToken cancellationToken = default)
{
    Stream? photo = await parameters.File.OpenStreamAsync(cancellationToken);
    if (photo is not null)
    {
        // Forward the upload without ever holding it in memory.
        await blobClient.UploadAsync(photo, cancellationToken);
    }

    return UploadPetPhotoResult.Created(receipt, workspace);
}
```

The contract is small. Each handle opens at most once. Consume the stream before the handler returns; it is not valid afterwards. Anything the handler does not open is drained and cleaned up by the generated endpoint. Opening a `required` part that is absent from the body produces a 400 for the request; an absent optional part opens as `null`.

#### Part Ordering Policies

Because the typed `Body` must be complete before the handler runs, the placement of binary parts on the wire matters. The policy is chosen where you map the endpoints, not at generation time, so one generated server can serve different clients with different policies:

- `MultipartBinaryOrdering.RequireBinaryLast` (the default) is true streaming. Binary parts must arrive after all non-binary parts; a request that interleaves them is rejected with 400. Nothing touches disk or memory beyond the form fields. Clients generated by Corvus already send binary parts last.
- `MultipartBinaryOrdering.SpoolOutOfOrder` accepts standard browser part order. A binary part arriving before the last form field is spooled, in pooled memory at or below `SpoolMemoryThresholdBytes` (64 KiB by default) and in a temporary file above it, and parsing continues to the end of the body. Handles then read from the spool in any order. The framework deletes spool files when the request completes, including on failures; handler code never cleans anything up. The trade is that out-of-order files hit disk rather than streaming straight through to their destination.
...
app.MapApiEndpoints(
    ...
    serverOptions: new ApiServerOptions
    {
        ...
        SpoolMemoryThresholdBytes = 256 * 1024,
        SpoolDirectory = "/var/tmp/uploads",
    });
```

Only the binary parts declared by the operation's schema are ever bound or spooled; undeclared and duplicate parts are drained without allocation. `MaxSpooledBodyLength` caps the total bytes spooled per request (unbounded by default, on the basis that the web server's request size limit governs; a request over the cap is rejected with 413).

#### Streaming multipart/mixed

`multipart/mixed` bodies are positional, so the wire order is the schema order and the ordering policy does not apply; they always stream live. Streaming applies when the schema's binary parts all come after its non-binary parts (a JSON envelope followed by a payload, for example); a schema that interleaves them keeps the buffered path. Binary prefix parts arrive as `BinaryPartHandle Part{n}` properties, and repeating binary items arrive as a `BinaryPartSequence`:

```csharp
while (await parameters.Items.MoveNextAsync(cancellationToken) is { } item)
{
    await ProcessAsync(item, cancellationToken);
}
```

#### Browser Forms with RequireBinaryLast

Browsers submit form fields in DOM order, so a form with its file input above a text field sends the file first. Under `RequireBinaryLast` either put file inputs last in the form, or build the `FormData` yourself with files appended last:
...
form.addEventListener("submit", async (event) => {
  ...
  // Append text fields first, then files, so binary parts go last on the wire.
...
Servers generated with `--serverBinaryParts stream` do not buffer multipart binary parts at all, so this limit does not apply to their multipart endpoints. Their non-binary parts are capped by `MaxNonBinaryPartsLength`, and spooled parts by `MaxSpooledBodyLength`; see [Streaming File Uploads (Server)](#streaming-file-uploads-server).
...
Same options as client generation, plus `--serverBinaryParts buffer|stream`. The default, `buffer`, binds each multipart binary part's bytes onto the Params struct; `stream` (OpenAPI 3.2 specs) hands the handler streaming part handles instead, as described under [Streaming File Uploads (Server)](#streaming-file-uploads-server). The output includes handler interfaces, endpoint registration, and shared model types.
