...
When the code generator templates change (e.g. in `src/Corvus.Text.Json.CodeGeneration/`), the migration model types must be regenerated. The runs below share one output folder: each run adds its schema to the folder's `corvusjson-jsonschema.lock` and regenerates the whole set as one `CorvusJsonSchemaProgram`, so the shared simple types (`JsonString` and the rest) are wired to the program that survives (see [Lock file and regeneration](CodeGenerator.md#lock-file-and-regeneration)).
