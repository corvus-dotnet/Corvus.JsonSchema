...
4. **Dynamic references**: `$dynamicRef` and `$recursiveRef` are resolved statically first. Only when the
   ...
   demoted to a static `$ref`, and if no dynamic references remain the engine never maintains a dynamic
   scope at all (`UsesDynamicScope == false`).
...
|---|---|---|
| Discriminator selection for `oneOf`/`anyOf` | `SchemaCompiler.BuildDiscriminator`, `Evaluator.TrySelectBranches` | Branches are classified by the constraint their `properties[X]` places on strings: positive (`const`/string `enum`), negative (`type: string` + `not: {enum}`), or wildcard. The instance's `X` value selects the candidate branch list; absent/non-object instances fall back to full evaluation. Flag mode only. |
...
| Small-array uniqueness | `Evaluator.IsDuplicate` | Up to eight items: pairwise comparison with token-type and raw-byte prefilters (canonical integer literals compare textually) instead of the shared hash set, whose object hashing transcodes every string. Took unreal-engine-uproject from 1.3× to 0.86. |
