...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete**. Verified against the code 2026-09-21, and the one divergence found then was fixed the same day. Rules conjoin (`VerbClauseFor`), bindings union (`ResolveReach`), an unknown rule name yields no clause, and the shell's mandated rules are ANDed in. An `Unrestricted` verb is every row the shell admits (`SecurityShell.BuildUnrestrictedFilter`), and a null reach only where the shell mandates no rule, so the shell cannot be widened by any grant. Before 2026-09-21 it returned full reach ahead of the shell. A binding document also carries scopes beside its verbs, so the planes are resolved separately and not stored separately. Scope: the grant-binding and rule model.
