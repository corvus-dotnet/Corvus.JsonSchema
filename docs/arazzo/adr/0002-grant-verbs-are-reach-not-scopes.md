...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete, with one divergence**. Verified against the code 2026-09-21. Rules conjoin (`VerbClauseFor`), bindings union (`ResolveReach`), an unknown rule name yields no clause, and the shell's mandated rules are ANDed in. Divergence: an `Unrestricted` verb returns full reach before the shell is applied (`PersistentRowSecurityPolicy.ResolveReach`, "the operator path", pinned by `An_unrestricted_binding_grants_full_reach_including_untagged_rows`), so "cannot be widened by any user grant" holds for rule-bounded grants only. A binding document also carries scopes beside its verbs, so the planes are resolved separately and not stored separately. Scope: the grant-binding and rule model.
