...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete, with divergences**. Verified against the code 2026-09-21. All four facets match by subset: reach (`PersistentRowSecurityPolicy.Matches`), the self-elevation guard (`CallerMatches`), administration (`WorkflowAdministrators.IsAdministeredBy`) and the reverse index (`SecurityIdentityDigest.SubsetDigests`), plus credential usage. Divergences: `WorkflowIdentity.SameAdministrator` is exact set-equality, reserved for identity operations, and is not the forward membership check this record says it is. An empty set is a subset of anything, so an empty identity is never matched: `IsAdministeredBy` skips one on both administrator records, none is persisted, and an identity-less version 1 is administered only by a caller the deployment has not identified either (fixed 2026-09-21; it was administered by anyone). A doc comment in `ControlPlaneRowSecurity.cs` and the grantee picker's warning text still say exact identity. Scope: how every authorization facet matches a principal against a
