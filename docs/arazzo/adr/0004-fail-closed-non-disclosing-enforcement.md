...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete**. Verified against the code 2026-09-21. `SecurityFilter.IsSatisfiedBy` denies on an empty rule set or an untagged row, the policy starts denying everything before its first refresh and every replica refreshes it within the policy refresh bound (5 s by default, required by the mapping since 2026-09-23), a wildcard `Unrestricted` grant is demoted unless the deployment opts in, expired bindings are excluded, and an out-of-reach row resolves as absent and is answered 404. Administrator-gated operations answer 403 by design. Not checked exhaustively: that every handler answers 404 and not 403; a sample did. Scope: the default behaviour of reach enforcement at every gate.
...
- **Reach denial is non-disclosing.** The design (`docs/arazzo/guides/auth-and-authorization.md`) fixes an out-of-reach
