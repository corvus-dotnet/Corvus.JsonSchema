...
Date: 2026-07-21. Status: **Accepted**. Implementation: **partly, with divergences**. Verified against the code 2026-09-21. Built: `SecurityShell.BuildFilter` ANDs the mandated rules, an `Unrestricted` grant is bounded by them too, a governed dimension is removed from the claim map whether or not the context resolves it (both fixed 2026-09-21, having been found as divergences), `ValidateUserTags` refuses the reserved keyspace in every mode, an ambient dimension replaces a same-named token claim, and `AmbientIdentityStamp` restamps governed keys. Divergence: the internal namespace is stripped from catalog and workspace responses only, while the sources and credentials handlers project management tags verbatim. `SecurityShell.StripInternal` has no production caller. Scope: the deployment-wide constraint every principal's reach is
