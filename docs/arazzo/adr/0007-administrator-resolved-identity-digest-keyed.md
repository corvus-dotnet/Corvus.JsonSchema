...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete where an administrator store is configured**. Verified against the code 2026-09-23. `WorkflowAdministrators` and `EnvironmentAdministrators` are governed by the secured catalog and environment administration, the publish gate throws on a non-administrator, version 1 establishes administration, and the digest-keyed reverse index is bounded at 4,096 subset digests and under-reports when truncated. Without a store, publishing and every administration read and mutation refuse with `NotSupportedException`, and a control plane maps no surface over a catalog without one, so the explicit record is the only source of administrators; until 2026-09-22 administration fell back to an identity derived from version 1's tags (and see ADR 0003 for the empty identity, which is never matched or persisted). A version's author-chosen tags are reach and never identity: since 2026-09-23 the catalog's `AddAsync` takes the publisher's identity and the author's tags apart, establishes and checks administration from the identity alone, refuses an author tag in the reserved keyspace, and stores the union on the version for reach. The reverse index was confirmed in six backends; MySql, Azure Storage and NATS were not individually checked. Scope: how a workflow's (and an environment's) administrators are
