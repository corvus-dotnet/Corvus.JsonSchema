...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete** for the surfaces that name a grantee. Verified against the code 2026-09-23. Built: grantee search over observed, directory and merged sources, reach-filtered, refusing the merged and directory searches with 502 when the directory cannot answer for any kind (since 2026-09-23, P1-15) rather than answering from the observed identities alone, the directory adapters, a picker that yields only server-resolved grantees, the add-administrator collision guard, and the server-side resolution every write goes through: `GranteeResolver` (`ControlPlane.Server/GranteeResolver.cs`) resolves the `{kind, value}` a write names (workflow and environment administrators, transfers, a credential's usage grantee) through the directory, then the recorded observed identities, then the policy's mapping of the kind, and the handlers store that identity; a client cannot supply one (the write shape `GranteeReference` has no identity field) and a grantee nothing resolves is refused. Remaining: a security binding (`ArazzoControlPlaneSecurityHandler.ReadBinding`) still takes a `claimType`, `claimValue` and additional clauses from the body, which is the binding's own selector rather than a grantee, and the policy's default resolution of a person carries no issuer, so a deployment without a directory records a person as a partial identity, flagged `complete: false`.
