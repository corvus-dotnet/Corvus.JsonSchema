...
purge (P1-5 of the 2026-08-07 security audit). Status: **Accepted**. Implementation: **partly, with one divergence**. Verified against the code 2026-09-21. Built: the self-elevation guard is a server check on binding create and update, covering read, write and purge reach, any scope, eligible-only bindings and the wildcard, audited as `refused-self-elevation`. Not built: the split by binding type. The server refuses no directly authored per-person binding for someone other than the caller; the rule exists only in the web kit's grants panel. "It is a server check, not a UI affordance" is true of the self-elevation guard and false of the per-person rule, so granting one person elevated access does not always involve a second party. Scope: which grants may be authored
