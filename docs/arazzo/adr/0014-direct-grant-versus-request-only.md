...
purge (P1-5 of the 2026-08-07 security audit). Status: **Accepted**. Implementation: **complete**. Verified against the code 2026-09-21, and the split by binding type was built the same day. Built: the self-elevation guard is a server check on binding create and update, covering read, write and purge reach, any scope, eligible-only bindings and the wildcard, audited as `refused-self-elevation`. The split by binding type is a server check too: a directly authored binding that names one person, a `sub` clause in the primary or an additional position, and confers anything is refused on create and update with `per-person-request-only` and audited as `refused-per-person`. The approval flow writes its grants through the store and is unaffected. Scope: which grants may be authored
