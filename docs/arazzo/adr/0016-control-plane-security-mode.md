...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete at the mapping entry point, with one divergence**. Verified against the code 2026-09-21. `MapArazzoControlPlane` takes the mode as a required parameter with no default, throws on a mode and policy pairing that would be insecure by omission, derives scope gating and the effective policy from the mode, and warns for `Open`. Divergence: "there is no default value" is true of the method and not of the type. The enum's zero member is `Open`, so a host that carries the mode in an options object or binds it from configuration and never sets it gets `Open` in silence, and an out-of-range value is not refused. The `Open` warning is skipped where no logger factory is registered. Scope: how a deployment selects which security planes are enforced.
