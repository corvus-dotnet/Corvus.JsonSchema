...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete**. Verified against the code 2026-09-21. `MapArazzoControlPlane` takes the mode as a required parameter with no default, throws on a mode and policy pairing that would be insecure by omission, derives scope gating and the effective policy from the mode, and warns for `Open`. The enum's zero member is `None` since 2026-09-22, and the mapping refuses it and any value outside the enum, so a host that carries the mode in an options object or binds it from configuration and never sets it fails at start-up. Until then the zero member was `Open` and a never-set mode ran open in silence. The `Open` warning is skipped where no logger factory is registered. Scope: how a deployment selects which security planes are enforced.
