...
Date: 2026-07-21. Status: **Accepted**. Implementation: **partly, with divergences**. Verified against the code 2026-09-21. Built: the authorization lifecycle and its withdrawal in every backend, the claim gate (`RunnerAuthorizationBindings.ResolveAsync`, where only `Authorized` binds), and the in-flight fence, which expires a revoked runner's leases through `IWorkflowLeaseAdministration`. Divergences: the claim refusal is in the runner API layer and not in the store, whose lease-acquire path never consults the authorization store; a resolution is cached, for up to `MaximumCacheWindow`. Since 2026-09-21 each durable authorization decision is announced to that cache (`IRunnerAuthorizationChangeObserver`), so a revocation takes effect before the revoke returns on the replica that handled it, a resolution that was reading the store when it landed is not cached, and the window, now 5 seconds and no longer 30, bounds only a replica that did not handle it. Where the state store does not implement `IWorkflowLeaseAdministration` the in-flight fence cannot run, and the audit record says so (`revoked-leases-not-fenced`) where it used to be skipped in silence, and a deployment wired with `DeclaredRunnerEnvironmentBindings` has no lifecycle or revocation at all. Scope: which runner may execute which runs. Builds on
