...
Date: 2026-07-21. Status: **Accepted**. Implementation: **complete**. Verified against the code 2026-09-21, and the one divergence found then was fixed the same day. Built: the store assigns the version number, the id is rewritten to `{base}-v{n}`, a versioned base id is refused, and the content-hash guard blocks an in-place replacement of the workflow or its sources in every backend sampled. The content hash covers the workflow and its sources only, so `IWorkflowCatalogStore.UpdatePackageAsync` also holds every stored entry byte for byte and takes new entries under `metadata/native/` and `metadata/native-attestation/` alone (`CatalogPackage.EnsureAddsOnlyNativeArtifacts`, in all ten backends and held by the shared conformance suite). The executor, its manifest and signature, the schema metadata and a native binary already attached cannot change under a fixed version number. A target a version already carries is not rebuilt over: the build worker completes such a job as Ready, and a binary believed bad means a new version. Before 2026-09-21 the seam replaced the whole package wherever the hash matched. The stores still assume version writes are serialized by the control plane, so two builds of different targets for one version, finishing together on two workers, can lose one of the two additions, and the loser's retry attaches it. This record cites `CatalogPackage.Process`; the method is `Project`. Scope: how a workflow is stored in the catalog. Builds on
