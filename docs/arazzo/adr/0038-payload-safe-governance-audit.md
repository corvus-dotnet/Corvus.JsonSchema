...
appends to the audit chain of ADR 0069. Status: **Accepted**. Implementation: **complete, with one convention**. Verified against the code 2026-09-21. The primitive is built as this record describes, and every operation of the control plane that changes state calls it. That was false when this was verified: 22 of 82 mutating operations recorded nothing, publishing a working copy into the catalog among them, and all of the designer's surface but its debug runs. They record now, the two OAuth callbacks included, which are `GET`s that take custody of a user's token. `EveryMutationIsAuditedTests` reads the contract and the handlers and fails for an operation that changes state and makes no audit call, so the claim is held by a test and not by review. Six `POST`s that compute and store nothing are exempt there, each with its reason. Payload safety is a convention and not a construction: every parameter is a string, and call sites already pass composed text (a rebudget outcome carrying both limits, a re-run target carrying both run ids). Scope: how a governed action is audited. This records why every
