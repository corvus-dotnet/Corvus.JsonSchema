...
- Executing a custom `provided.al2023` native-AOT bootstrap under LocalStack's Lambda executor is **verified when the deployer is first built, not assumed**. A gap there is a work item against LocalStack fidelity, not a change to this design or the deployer.
