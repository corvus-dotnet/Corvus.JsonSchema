...
- **A run's genesis row is clear until input sealing lands**, so a sealed environment's run inputs are readable by the control plane from the start of the run until the first runner save encrypts them, and a run that never claims keeps them in the clear.
