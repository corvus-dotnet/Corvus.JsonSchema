...
| Label | Info |
|---|---|
| `wrap` | `len‖"wrap" ‖ len‖environmentId ‖ len‖keyId` |
...
Without framing, `(keyId "k1", runId "0abc")` and `(keyId "k10", runId "abc")` derive the *same* key — and with a counter nonce that is a repeated `(key, nonce)` pair over different plaintexts, which yields the GCM authentication subkey and hence payload forgery. The four subkeys that omit `runId` are derived once per key generation and cached; only `data-key` is per encryption. The wait index is stored keyed by `(environmentId, keyId, index)`.
...
- **Anchor**: the high-water mark must live where the control plane cannot rewrite it, so the tenant keeps one. It is a **tenant-owned, environment-scoped, replicated key-value record** — not runner-host-local, because leases fail over between hosts and a host-local anchor would brick every run its host was carrying — holding `runId → committed(epoch, sequence)` plus at most one `pending(epoch, sequence + 1)`.

  The anchor is a write-ahead record, and its states are what make honest failure recoverable rather than fatal:

  | Anchor | Store | Meaning | Action |
  |---|---|---|---|
  | `committed(e, n)` | at `n` | normal | proceed |
  | `pending(e, n+1)` | at `n+1` | the save landed, the acknowledgement was lost | promote `pending` to `committed`, proceed |
  | `pending(e, n+1)` | at `n` | the save did not land | discard `pending`, proceed from `n` |
  | any | below `committed` | **rollback** | hard fault |
  | missing | beyond genesis | anchor lost | hard fault, recoverable only by an audited re-anchor |

  A missing entry is a fault rather than a licence to trust the row — trusting the row on a cold open is precisely what removes the detection. Recovery from a lost anchor, and from a restore (where the store legitimately moves *below* every anchor), is the same audited operation: a **re-anchor**, permitted once per run per incarnation, gated on the tenant-side operator key of decision 8, and recorded. A restore is therefore a tenant-visible rollback window, not a transparent event.
...
Purge is described by enumeration, because a compliance reader needs precision: it removes the run row (envelope and payload together) and its wait and timer index rows, while **retaining a run-id tombstone** — the id and its creation time, no envelope, no payload — for the environment's lifetime, along with its anchor entry. Without the tombstone the primary-key collision that decision 9 relies on for anti-replay expires with retention, and a captured sealed-input start becomes replayable. It does **not** reach governance-audit and telemetry records keyed by run and workflow id, nor store backups — which hold envelopes in the clear. Because payloads are envelope-encrypted, **crypto-shredding** (destroying a key generation, or wrapping per-run data keys under a per-run key) is the mechanism that makes payload erasure verifiable where row deletion cannot reach.
