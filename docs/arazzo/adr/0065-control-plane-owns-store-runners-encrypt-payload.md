...
**Nothing is shipped.** There is no deployed installation, no tenant data at rest, and no external consumer of the current wire or storage formats. This decision therefore carries no migration obligation and no compatibility hedge: the target design is built directly, the phases below are our own delivery sequencing rather than an upgrade path, and where an existing component contradicts this ADR (the checkpoint listener's deployment shape, the runner's store connection) it is corrected rather than deprecated.
...
## Tenant-side data minimization (outside this platform's scope, and the strongest posture available)

Everything above concerns data the tenant chooses to put *into* a workflow. A tenant with strict governance obligations — GDPR and comparable regimes for personal data, or sector rules that forbid regulated values leaving a jurisdiction or a system of record — has a stronger option than any platform control: **design the workflow so the sensitive values never enter it.**

The techniques are ordinary workflow design, not platform features:

- **Pass handles, not values.** A step carries a customer reference, a case id, or an opaque token, and the tenant's own service resolves it to the personal data at the point of use. The run's payload holds the handle; the platform never holds the data even in ciphertext.
- **Proxy through a tenant-controlled endpoint.** Where a workflow must call a third party with regulated values, it calls a tenant-owned facade instead; the facade injects the sensitive fields server-side, inside the tenant's boundary, and returns only what the workflow needs to make its next decision.
- **Return decisions, not records.** A step's output is the verdict (`eligible`, `tier: 2`) rather than the data the verdict was computed from, so nothing regulated is available to leak.
- **Keep the sensitive branch outside the workflow.** A sub-process that must handle regulated data runs entirely within the tenant's systems and reports only its outcome to the workflow.

This is worth stating in a security decision record for three reasons. It composes with the design above rather than replacing it: handles and decisions still ride the encrypted payload. It is the only technique that also removes the accepted residues — a value that never entered the workflow cannot leak through envelope metadata, blind-index equality, ciphertext length, or a compromised control plane. And it means the platform can support strict-governance workloads without the platform itself becoming a processor of the regulated data, which is frequently the deciding constraint.

The platform's obligations are to make this practical and to be accurate about what it holds: expression handling and step outputs stay opaque to the platform, the documentation states exactly which fields are platform-visible (envelope, tags, security tags, timing) so a tenant can design around them, and no guidance ever suggests putting regulated values into tags, correlation ids, or workflow identifiers. Advising tenants on their own data-protection obligations is out of scope; the workflow guides carry a short pattern note pointing at these techniques.
...
Accepted residues, stated so nobody has to rediscover them. Each is a property of data the tenant chose to put into the workflow, so each is removed outright — not merely mitigated — by the data-minimization patterns above:
...
- The checkpoint listener moves inside the tenant boundary, correcting the deployment shape its live-proof recipe used.
...
- The checkpoint serialization is the envelope/payload split with the chain, built directly rather than migrated to: nothing is deployed, so the format carries no legacy shape. An unsealed environment writes the same structure with its payload clear, and a multi-tenant-mode deployment cannot create one.
- A tenant with strict governance obligations can go further than the platform's controls by designing sensitive values out of the workflow entirely (see the data-minimization section); the guides carry that as a pattern note.
