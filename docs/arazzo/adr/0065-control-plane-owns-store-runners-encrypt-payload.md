...
- **The check is serialized.** Two concurrent creates introducing two different owner groups would otherwise both pass and both commit, and a gate that two simultaneous requests bypass is not a gate. The distinct owner groups are held in one row, compare-and-swapped by every write that would introduce one, which is the acceptance shape the tenant anchor already uses. **This part is not yet built.** The rule itself is enforced, reading the population and deciding, so the sequential case is refused; the simultaneous one is not. The row is the remaining work, and until it exists the gate is stated here as what it is rather than as what it will be.
