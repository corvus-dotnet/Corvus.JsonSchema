...
owner group independently of the caller's reach. Status: **Accepted**. Implementation: **partly, as this record says**. Verified against the code 2026-09-21. Rates are metered before the work in every runner handler, with the guard on by default when none is configured, the runner's bounded hold is built, and the capacity guard covers run start, pre-authorization and enrolment. Not built, as stated: a shared-state guard, the parked-wait cap and the payload-bytes quota. One statement is stale: the runner API has an audit seam since ADR 0071, and a 429 is recorded as `refused-quota`. Lease renewal is metered at the machine scope alone by default, and the capacity guard was a nullable parameter on two handlers until 2026-09-22, and is required now. Scope:
