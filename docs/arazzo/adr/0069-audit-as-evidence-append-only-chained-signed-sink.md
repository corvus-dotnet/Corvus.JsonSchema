...
Date: 2026-09-07. Status: **Accepted**. Implementation: **in progress**. Built: the record, the chain writer, the sink seam with its file and in-memory sinks, and the chain verifier, in the durability library; the governance audit routed through one `GovernanceAuditor` per deployment, which the host gives to its bootstrap and its control plane alike; the startup assertion in the secured postures; and the fail-closed append, a 500 problem of type `audit-record-failed` with a failure counter, an error log and a health state. Not yet built: signed heads, the Azure Storage sink, and the verify command. Scope: how a governance audit record survives and how its integrity is shown. Resolves GAP-6 of the 2026-08-07 security audit and is the durability half remediation row 11 names. Builds on [ADR 0038](0038-payload-safe-governance-audit.md), which decides what a record contains and is not changed here, and on the signing stack of [ADR 0025](0025-integrity-binding-optional-signature.md), which this record reuses. Revised 2026-09-20, on starting the implementation: a chain belongs to one writer, a writer abandons a chain it failed to append to and says in the next chain where it continues from, verification reads the sink directly, and the heads are signed with a key of the audit's own.
