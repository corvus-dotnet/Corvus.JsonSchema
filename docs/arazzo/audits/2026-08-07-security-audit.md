...
> **Partly resolved 2026-09-21.** The invoke is authenticated: the invoke authenticator is a required seam on `ServerlessRunExecutionBackend` with no anonymous implementation, the baked trigger is at the `Function` level behind a key the deployer sets from the runner's secret store, and Entra can be layered on top, with the deployer refusing an app that does not require it ([ADR 0059](../adr/0059-serverless-deploy-runs-on-the-runner-as-the-secure-boundary.md) decision 4). The statement above that Lambda did this correctly was wrong, since nothing signed the invoke (V-37), and that is fixed with it. The management-plane half has no emulator and is not yet proven against a real Function App. The `checkpointUrl` criterion is still open.
...
| # | Finding | ADR | State |
|---|---------|-----|-------|
| V-36 | The Azure Functions trigger is emitted as `AuthorizationLevel.Anonymous` and the invoke carries no key or identity. An anonymous caller can post an invocation naming its own `checkpointUrl`, and the function loads and advances that checkpoint with its own source credentials. The ADR 0062 token protects the runner's checkpoint surface and not the function's invoke. This is P1-11 | 0059, 0061 | **Fixed 2026-09-21.** The trigger is at the `Function` level behind a key the deployer sets from the runner's secret store, with Entra as an optional second layer. The `checkpointUrl` half of P1-11 is separate |
