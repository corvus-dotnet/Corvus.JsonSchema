...
> **Partly resolved 2026-09-21.** An empty identity is refused everywhere it could match: `IsAdministeredBy` skips one on the workflow and environment records, none is persisted when version 1 is published or administration is first mutated, and an identity-less version 1 is administered only by a caller the deployment has not identified either, so the Open posture stays consistent with itself and no authenticated caller inherits an unowned workflow. Still open: author-supplied tags are part of the identity the catalog derives. The catalog is handed one tag set holding both, and its own tests use unprefixed identities, so telling them apart is a change to the `AddAsync` seam (pass the identity and the author's tags separately) and not a filter.
