...
> **Resolved 2026-09-23.** Both surfaces authenticate: the admin surface takes a shared bearer token the sidecar refuses to start without (`--admin-token`), presented by the deployer on every call and by `MicroGuestSidecarInvokeAuthenticator` on each invoke from the runner's own secret store; the guest surface takes a per-sandbox token the sidecar mints at evolve and freezes into the sandbox's argv (`ARAZZO_GUEST_TOKEN`), so `GET`/`POST /guest/{id}` answer only the sandbox whose token they carry and an unknown sandbox is the same refusal. The sidecar verifies the artifact for itself: it starts only with a trust store of attestation-signing public keys (`--trusted-key`), the deploy request carries the attestation's exact signed bytes and its signature on from the runner's own verify, and an evolve is refused with a 403 unless the signature verifies under a trusted key and the guest binary in the staged CPIO digests to what the attestation names. Neutering either check fails its tests. The `packageHash` binding cannot be checked without the package and stays on the runner's deploy path, recorded in [ADR 0063](../adr/0063-microguest-backend-hyperlight-unikraft-warm-snapshot.md) decision 5.
...
> **Resolved 2026-09-21.** The invoke is authenticated: the invoke authenticator is a required seam on `ServerlessRunExecutionBackend` with no anonymous implementation, the baked trigger is at the `Function` level behind a key the deployer sets from the runner's secret store, and Entra can be layered on top, with the deployer refusing an app that does not require it ([ADR 0059](../adr/0059-serverless-deploy-runs-on-the-runner-as-the-secure-boundary.md) decision 4). The statement above that Lambda did this correctly was wrong, since nothing signed the invoke (V-37), and that is fixed with it. The `checkpointUrl` criterion is met by an allowlist and not by one expected host, because a run checkpoints back to the runner that dispatched it and an environment can have several: the function takes a checkpoint URL only from an origin in its required `ARAZZO_CHECKPOINT_ORIGINS` setting, fails at start-up without one, and refuses any other origin before anything is loaded from it. The management-plane half of the key has no emulator and is not yet proven against a real Function App.
...
> **Resolved 2026-09-23.** An empty identity is refused everywhere it could match: `IsAdministeredBy` skips one on the workflow and environment records, none is persisted when version 1 is published or administration is first mutated, and an identity-less version 1 is administered only by a caller the deployment has not identified either, so the Open posture stays consistent with itself and no authenticated caller inherits an unowned workflow. Author-supplied tags were part of the identity the catalog derived, because it was handed one tag set holding both; since 2026-09-23 `AddAsync` takes the identity and the author's tags apart, administration is established and checked from the identity alone, an author tag in the reserved keyspace is refused, and the version keeps the union for reach.
...
|---|---|---|---|
| SEQ-1 | Envelope and payload split, unified MAC over runner region and payload ciphertext hash. **Piece 1 built 2026-09-23:** the framed row, the split into closed-schema runner region and payload, the control-plane region, the lease epoch in the region checked at the runner API. **Piece 2 built 2026-09-24:** the runner submits only its own bytes, the server joins and merges on conflict (decision 7 as reworded). **Piece 3 built 2026-09-24:** the unified MAC over the framed header, key id, runner region and payload digest, sealed and verified by the runner through its key ring, and refused clear by the runner API for a sealed environment. **Closed** for the phase-B MAC; the encryption is SEQ-3 | ADR 0065 §4 | P1-3, P1-7 |
| SEQ-2 | Tenant anchor store, acceptance predicate, open decision table. **Built 2026-09-24:** a tenant-owned anchor store (in memory, and PostgreSQL for the demo's own tenant database) that enforces exactly the acceptance predicate under a whole-record compare-and-swap and holds the environment's attested incarnation; the runner's sealing store evaluating the open decision table on every load and staging every save with the tenant before dispatching it; the incarnation in the runner region beside the epoch; the genesis row at sequence 0; the runner client refusing a sealed environment without an anchor and answering every anchor refusal by giving the lease back and carrying on. **Closed** for the store, the predicate and the table. Not built: the runner's verification and application of an operator-signed re-anchor or abandon (waits on decision 8's pinned operator key), a control-plane-advertised incarnation, anchoring of listener-terminated environments | ADR 0065 §6 | **P1-1**, P1-7 |
| SEQ-3 | Envelope encryption, per-operation derived data key, four-label derivation. **Built 2026-09-24:** AES-256-GCM under a data key derived per operation with a salt drawn inside the encryption, the run, environment, generation and sequence as associated data, the runner's sealing store encrypting on save and opening on load, the runner API and the control plane's own checkpoint surface refusing anything but an encrypted, MAC'd submission for a sealed environment, the listener hosts sealing what they terminate, and every keyless reader degrading to the envelope. **Closed.** A plain start's genesis row stays clear; a sealed start's is the initiator's seal since SEQ-5 piece 1 | ADR 0065 §5 | pure functions already conformance-tested |
| SEQ-4 | Blind wait and correlation indexes. **Built 2026-09-24:** the blind wait index under the `wait-index` subkey (framed channel and correlation id, a sentinel for a channel-only wait, keyed by generation), carried in the runner region and the index column in place of the channel for every environment on the runner's key ring, matched by every backend by plain equality, claimed through the runner API by index (a sealed environment is never swept by channel), queried by the delivering runner under its own index and the channel-only index, and re-checked against the run's own region before the message is handed over. **Closed** for the wait match key. Not built: the run-level correlation id (the control plane's trace id until an initiator supplies a business one at a sealed start, SEQ-5) | ADR 0065 §4 | every backend, since the index is a column the row already had |
| SEQ-5 | Initiator sealing and signature, runner-side input-schema validation. **Piece 1 built 2026-09-25:** the HPKE seal to the registered P-256 seal key (RFC 9180 base mode, DHKEM(P-256, HKDF-SHA256), HKDF-SHA256, AES-256-GCM, held against the RFC's vector) under the framed binding of environment, base workflow id, version, generation and the initiator-chosen run id; the ES256 initiator signature over the binding and the seal, verified against initiator keys pinned on the runner's key ring beside the private seal half (together or not at all); the sealed genesis row and its digest under the genesis label; the sealing store's open at first claim under the binding re-derived from the address and the envelope; the runner client's validation of the opened inputs against the version's inputs schema (one builder shared with the control plane) and the start faults `sealed-start-inputs-invalid` and `sealed-start-unopenable`, sealed saves that stop the run being claimed again; `StartSealedAsync` on the management seam; the sealed-start mark in every later save. **Piece 2 built 2026-09-25:** the control-plane `startSealedCatalogWorkflowRun` endpoint through the one admission chain (active generation required, `run-id-occupied` and `seal-key-not-active` refusals, `sealedStart` on the run detail); the CLI `start` command, plain and `--sealed`, the latter the initiator with a required seal-key fingerprint pin; the demo's provisioning of the seal and initiator pairs, the private seal half in the production runner's Vault, the initiator pinned on that runner, the operator's handoff, and a live sealed production start. **Closed** for decision 9's sealed start. Not built and stated: the runner-side trigger host for sealed schedules and message triggers, the start quota (GAP-3), the run-level correlation id's blinding, the idempotent blinded run-id derivation | ADR 0065 §9 | P1-7 |
| SEQ-6 | Runner allowlist of environment, seal-key fingerprint and sealed flag, plus minimum generation. **Built 2026-09-26:** the runner key ring is a default-deny allowlist of clear and sealed entries (a runner with none serves nothing; the demo's development and system runners name their environment clear); the runner client hands back a claim for an environment it does not admit and the sealing store, shared by the runner client, the checkpoint listener and the serverless runner, refuses a checkpoint for one; a keyed entry pins the registered seal key's fingerprint, the runner API's new `getEnvironmentSealKey` advertises a bound environment's generations, and the client suspends an environment whose advertised key, generation state or answer does not match, re-checking once a minute; the minimum generation is recorded as the generation held, and a minimum naming another does not build until SEQ-7's multi-generation ring. **Closed.** Not built and stated: the listener and serverless runner check the pin over no wire; the reduced journal and padding of decision 10 (SEQ-8's residue mitigations) | ADR 0065 §10 | |
| SEQ-7 | Re-key sweep and generation retirement. **Piece 1 built 2026-09-26:** the rotation chain (a registration into an environment that holds a generation carries the predecessor's signature over the framed rotation tuple or is refused with `environment-key-rotation`, a first generation carries none, a retired predecessor still hands over, the link is stored on the generation and advertised by `listEnvironmentKeys` and `getEnvironmentSealKey`), the shared chain verifier, the multi-generation runner key ring (generations oldest first, each with its own payload key and optional seal half, the minimum generation enforced on open, rows and sealed starts under any held generation opened under that generation's keys), the write generation the runner client selects at its check as the newest held generation advertised active that reaches the pin, deliveries querying the blind index under every held generation, and the CLI initiator following a rotation along verified links. **Piece 2 built 2026-09-26:** the clear wait inside the payload, the runner API's `claimForRekey` (sealed environments of the principal, rows under the named generation, free leases only, terminal runs excluded, paged, quota-limited) over a new environment filter on the visibility query of every backend, the runner client's `RunnerRekeySweep` re-sealing resting runs under the write generation and re-parking blinded waits (one pass per dispatch poll), the run detail's `keyGeneration`, the demo's provisioned successor generation and operator handoff, and a live rotation test. **Closed.** Not built and stated: the maximum generation lifetime, the first-registration operator confirmation, a server challenge; the listener and serverless runner write under the newest generation configured and run no sweep; a finished run is never re-sealed | ADR 0065 §12 | SEQ-3 |
...
> ADR 0067. Implemented: the read record is a kind of the audit chain, and the control plane's payload disclosures are on it (step journal, debug run trace, credential detail, checkpoint), with every refused read, capped for each subject, every other read metered, and a chain of the runner's own for the secrets it resolves. The finding below is left as it was measured.
...
> with GAP-3. Implemented: the authentication telemetry, as one scheme-agnostic middleware with its failure
> records capped for each remote address, and the runner API's refusals, recorded by one filter and capped for
> each runner. The finding below is left as it was measured.
...
| PROC-6 | **ADRs do not record implementation status**, so a reader credits designed-but-unbuilt barriers. The root cause of the DIV class | **Done 2026-09-21.** Every ADR carries a status line verified against the code, and [§11](#11-proc-6-verification-findings) holds what the verification found |
| PROC-7 | The threat model is committed at `docs/arazzo/reference/threat-model.md`. Wire its update triggers into the review checklist. The checklist also carries the read-side question ADR 0070 asks of a new operation, which is written down in the platform conventions guide and had no checklist to sit on until this landed | **Done 2026-09-21.** [`reference/review-checklist.md`](../reference/review-checklist.md) |
...
| V-6 | Grantee resolution was a UI convention and not a server invariant: the add-administrator, transfer and credential handlers accepted a client-supplied identity, and an interim single-grant path took a free-form `{dimension, value}`. ADR 0008 says a grantee is never authored as a free-form tuple | 0008 | **Fixed 2026-09-23.** Every write names a grantee as `{kind, value}` (`GranteeReference`) and `GranteeResolver` resolves it on the server through the directory, the observed identities and the policy; the client-identity and single-grant paths are deleted. The binding selector in the security handler is the binding's own rule input, not a grantee, and stays |
...
| # | Finding | ADR | State |
|---|---------|-----|-------|
| V-9 | The server refuses no directly authored per-person binding for someone other than the caller. The request-only rule for a named `sub` exists in the web kit's grants panel alone, so a holder of `security:write` can grant a colleague any reach and any scope with no second party, by calling the API. ADR 0014 says the split is a server check and not a UI affordance | 0014 | **Fixed 2026-09-21.** `per-person-request-only` on create and update, audited as `refused-per-person` |
| V-10 | The access overview reads bindings through `ISecurityPolicyStore.ListBindingsAsync` without an access context, where the binding search beside it passes the caller's. The security plane is reach-partitioned (P1-5), and the overview aggregates across the partition | 0015 | **Fixed 2026-09-21.** Both overview listings pass the caller's access context |
...
| V-14 | `default(ControlPlaneSecurityMode)` is `Open`. A host that binds the mode from configuration and never sets it runs open in silence, which is the omission ADR 0016 says cannot happen. An out-of-range value is not refused | 0016 | **Fixed 2026-09-22.** The zero value is `None`, refused at the mapping with any value outside the enum |
...
### Catalog, packaging, paging and seams, ADRs 0027 and 0029 to 0037 (2026-09-21)

| # | Finding | ADR | State |
|---|---------|-----|-------|
| V-21 | A version's package is edited in place. `IWorkflowCatalogStore.UpdatePackageAsync` replaces the stored bytes where the content hash is unchanged, and the hash covers the workflow and its sources only, so the executor, native binaries and attestations that run can change under a fixed version number and hash. ADR 0030 says a version is never edited in place and that the hash names exactly what executed. The seam does not limit which entries change | 0030 | **Fixed 2026-09-21.** The update seam holds every stored entry byte for byte and takes only new native artifacts, in all ten backends, and the build worker never rebuilds over a target a version carries |
| V-22 | A revoked runner can go on claiming new runs for up to 30 seconds, since `RunnerAuthorizationBindings` caches a resolution for `MaximumCacheWindow` and the claim refusal lives there and not in the store. ADR 0027 says revocation is enforced in the store, immediately and unconditionally. Only the expiry of leases already held is immediate, and it is skipped in silence where the state store lacks `IWorkflowLeaseAdministration` | 0027 | **Fixed 2026-09-21.** Each authorization decision is announced to the cache, so a revocation is immediate on the replica that handled it. The window is 5 seconds for any other. An unfenced revoke is audited as `revoked-leases-not-fenced`. Multi-replica notification stays with P1-14 |
| V-23 | "Every list is keyset-paged" and "a count never scans unbounded" do not hold everywhere: `AzureStorageSecurityPolicyStore` pages and counts in memory over the whole set, `ListAdministeredWorkflowsAsync` is unpaged on the access-request path, `IRunnerRegistry.CountAsync` lists a page to count it, and a NATS count parses candidates without bound under a selective filter | 0035, 0036 | Reported |
...
| V-35 | The step journal has no `suspended` status and nothing records `Skipped`. The journal count now carries the step budget of ADR 0068. The tolerance for runs that predate the journal is a compatibility path to delete | 0050 | **Partly fixed 2026-09-22.** The old-run tolerance is deleted. The `suspended` status, `Skipped` recording and the journal's budget role are ADR 0050 matters, still open |

### Serverless, micro-guest and runner trust, ADRs 0055, 0057 and 0059 to 0066 (2026-09-21)

| # | Finding | ADR | State |
|---|---------|-----|-------|
| V-36 | The Azure Functions trigger is emitted as `AuthorizationLevel.Anonymous` and the invoke carries no key or identity. An anonymous caller can post an invocation naming its own `checkpointUrl`, and the function loads and advances that checkpoint with its own source credentials. The ADR 0062 token protects the runner's checkpoint surface and not the function's invoke. This is P1-11 | 0059, 0061 | **Fixed 2026-09-21.** The trigger is at the `Function` level behind a key the deployer sets from the runner's secret store, with Entra as an optional second layer. The function also takes a `checkpointUrl` only from an origin it was deployed with |
| V-37 | The Lambda Function URL is `AWS_IAM` and nothing signs the invoke. No SigV4 signer exists in `src/`, the samples or the tests, so the path works on LocalStack alone | 0059 | **Fixed 2026-09-21.** `SigV4ServerlessInvokeAuthenticator`, pinned to the AWS signing suite's known answers. Not yet proven against real AWS |
| V-38 | The tenancy gate admits a second owner group once every tenant-owned environment has an active registered key. No code encrypts a payload under that key, so the gate credits a barrier that does not exist. Until SEQ-1 and SEQ-3 land, the rule the code supports is to refuse a second owner group in every reach-isolating mode | 0065 | **Fixed 2026-09-22.** A second owner group is refused in every authenticating mode. The sealing scan is gone from the gate, and the refusal says why a registered key does not change the answer. The retirement gate keeps its symmetric refusal, held by tests that seed the ledger directly |
| V-39 | ADR 0065's phase B is unbuilt and the tenant anchor is specification only. Rollback, substitution and replay of a checkpoint by the control plane are undetected. This is SEQ-1 to SEQ-8, already in the remediation order, and the ADR's status line now says so | 0065 | **In progress.** SEQ-1 landed 2026-09-24: substitution of the runner region or the payload of a sealed environment's row is now caught by the runner on load. SEQ-3 landed the same day: a sealed environment's payload is ciphertext to the control plane, the store, backups and operators from the first runner save, and its inputs from run start once SEQ-5 seals them. SEQ-2 landed the same day: for an anchored environment, a rollback, a substitution at a committed sequence and a replay of a finished run are refused by the runner at its next open. Recovery of a refused run waits on decision 8's operator key. SEQ-4 landed the same day: a sealed environment's message waits carry no channel or business key. SEQ-5 landed 2026-09-25: a sealed start's inputs are the initiator's seal from the genesis row on, opened and validated only by a runner that holds the seal key and pins the initiator, a start that does not open faults the run rather than running it, and the CLI is the initiator behind a pinned seal-key fingerprint. SEQ-6 landed 2026-09-26: a runner serves only the environments its own allowlist names, and a keyed one only while the control plane advertises the seal key the tenant pinned. SEQ-7 landed 2026-09-26: a rotation carries the outgoing key's signature, runners and initiators follow it along verified links and nothing else, a runner holds several generations, and its sweep carries resting runs to the current one so a compromised generation can be retired |
...
| V-42 | Optional seams that fail closed or are always supplied, to delete since nothing is shipped: the nullable checkpoint-token issuer and the function's no-token branch, and the nullable capacity guard on two handlers | 0062, 0066 | **Fixed 2026-09-22.** The token issuer and the capacity guard are required, and the function refuses an invocation carrying no token |

### Found while fixing, outside the fixes (2026-09-21)

| # | Finding | ADR | State |
|---|---------|-----|-------|
| V-43 | The Mongo catalog store's keyset paging is broken: `WorkflowCatalogStoreConformance.Query_pages_by_keyset` fails in `MongoWorkflowCatalogStore.QueryAsync` with a malformed continuation token. It fails against the store as it was before the V-21 work too. The cause is one call site the composite-address re-cut (H18 piece 3) missed: the catalog query decodes its cursor with `WorkflowContinuationToken.Decode`, which now expects a run address of the form `environment/runId`, where a catalog cursor is a plain sort key and wants `DecodeText`, as the same file's other cursor site uses. The other nine backends and the in-memory store pass all 29 conformance tests. The integration suites are outside the gate, which is how this went unseen | none | **Fixed 2026-09-21.** A functional defect and not a security one. The Mongo test project passes 348 of 348 against a real Mongo. Every other caller of the address decoder is a state store, which pages over run addresses, so this was the only misuse |
| V-44 | The catalog stores assume that writes to one version are serialized by the control plane, and `UpdatePackageAsync` reads, checks and then writes with no condition on the write in most backends. Two native builds of different targets for one version, finishing together on two workers, can each pass the check against the package as it was and the later write then drops the earlier addition. Nothing stored is changed by this, and the loser's retry attaches its binary, so it is a lost update and not an integrity failure. A conditional write in each backend would close it | 0030, 0056 | Reported |
