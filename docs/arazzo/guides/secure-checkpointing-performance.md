...
The MAC must not require running the RFC 8785 content-hash path per write *and* per open — a parse, sort, and re-emit is 10–50× the cost of the hash itself. The way to avoid it is to make the region **opaque octets no backend parses**: deterministic TLV framing with length-prefixed regions in the pinned stored order (header, runner region, salt, nonce, tag, ciphertext, MAC, then the control-plane region), stored in a binary column or an opaque property. The runner writes its region once with a non-indented, validation-skipping writer in a fixed property order, and MACs exactly those bytes.
...
The tenant anchor is now the freshness mechanism (the payload chain is deleted — ADR 0065 decision 6), and it is a **second durable write on the checkpoint hot path**: `pending(epoch, sequence + 1, digest)` before the save, promoted to `committed` on acknowledgement in the steady state (a recovery-time promote instead requires the stored row to match on epoch and digest, per the ADR's state table) — the digest is one SHA-256 over the submitted bytes already contiguous in the buffer (ADR 0065 decision 6 defines the extent normatively — region, salt, nonce, tag, ciphertext, and MAC, never the server-owned region), and it is what makes the anchor a commitment to a checkpoint rather than to a number. Budget it — one tenant-side round trip per checkpoint, well under a millisecond for a co-located anchor store but not free, and its unavailability stalls checkpointing. Batch the promote of the previous checkpoint with the pending write of the next, so the steady state is one anchor round trip per checkpoint rather than two. The run's lease holder is its sole dispatcher and sole anchor writer, and the record is CAS'd as a whole, so the listener and trigger host write through it rather than racing it — the single-flight interlock is per run, not per component.

Derive the three per-generation subkeys (`envelope-mac`, `wait-index`, `checkpoint-token`) **once per key generation** into an immutable key ring over a single pinned buffer (pinned so rotation can reliably zero it), held in a frozen dictionary keyed by key id so prior generations stay openable. Only `data-key` is per encryption, because only it takes the run, sequence, and salt. Use the static `HashData` overloads — no HMAC instances, no allocation.

The row buffer's lifetime follows the byte-identical retry rule: it is owned by the in-flight save and returned to the pool only when that sequence is acknowledged, superseded, or abandoned. Budget pool pressure as in-flight concurrency times row size; at most one save per run is in flight, enforced by the per-run interlock held by the lease holder.
...
**Index the blind key, not text.** Replace the nullable channel and correlation text columns with a single fixed-width 32-byte binary column plus the environment and key-generation columns, and index `(status, environment_id, key_id, wait_key)` — the generation column is what lets both generations match during a re-key roll and lets the sweep tell which rows it has already re-derived. Drop the old three-column collated index. The lookup is equality, but a message arrival issues **two probes** — the exact `(channel, correlationId)` key and the domain-separated channel-only sentinel — because the old predicate treated a null stored correlation as a wildcard, and a single-probe design would silently stop waking channel-only waits. Do not "simplify" by deriving the key from the channel alone: that collapses correlation blinding entirely. While in the DDL, add the indexes the dispatch path lacks — claimable-by-environment, and `leases(owner, expires_at)`, since the revocation fence currently scans the table it is meant to fence.
...
- Enabling pre-encryption compression on a sealed environment, or by default anywhere. It shrinks payloads 3–5× but makes ciphertext length a function of plaintext *content* (the CRIME/BREACH class), and checkpoints are guest-driven. ADR 0065 decision 10 makes length-bucket padding mandatory for sealed environments, so the opt-in exists **only for unsealed environments** — treating it as an alternative to padding would reintroduce the leak on exactly the environments padding exists for.
