...
The row buffer's lifetime follows the byte-identical retry rule: it is owned by the in-flight save and returned to the pool only when that sequence is acknowledged, superseded, or abandoned. Budget pool pressure as in-flight concurrency times row size; at most one save per run is in flight, enforced by the client-side interlock in the checkpoint's author (the runner, or the listener for guest-originated checkpoints) and spanning its replicas.
