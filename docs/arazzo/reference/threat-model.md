...
| A hostile binding: the control plane creates an environment of its own, binds the tenant's runner to it and harvests plaintext from runs executed with the tenant's credentials, or re-keys a served environment under a key it holds | **Holds**. The runner serves only the environments its own allowlist names (ADR 0065 decision 10) and hands back any other claim before loading a byte; a keyed environment is served only while some generation the runner holds is advertised active under the pinned seal key or under one that reaches it along rotation links the predecessors signed (decision 12), re-checked once a minute, and is suspended otherwise; the newest such generation is the one written under, so a rotation is followed and a swapped key is not | The listener and serverless runner pin the fingerprint but hold no runner API client to check it against; a suspended environment is still claimed and released each sweep, which the dispatcher's seen-twice rule bounds; an environment administrator holding the outgoing private seal half registers a successor every pinned party follows, which is what a rotation is | |
...
| Runner allowlist: the key ring admits only the environments it names (default deny, clear or sealed), the runner client hands back every other claim, the sealing store refuses every other checkpoint on the runner, the listener and the serverless runner, and a keyed entry serves only while a held generation's advertised seal key is the pinned one or reaches it along verified rotation links, the newest such being written under | Holds; the listener and serverless runner pin without a wire check | `RunnerKeyRing.cs`, `SealingCheckpointStore.cs`, `Runner.Client/ArazzoRunnerClient.AdmitsAsync`, `RunnerRunAdvance.cs`, `Runner.Server/ArazzoRunnerEnvironmentsHandler.cs` |
| Re-key sweep: a runner holding a successor generation carries every resting run of its sealed environments from the older generation to the one it writes under (claimed through `claimForRekey`, free leases only, terminal runs excluded, paged and quota-limited), re-deriving blinded waits from the clear wait inside the payload, so a compromised generation is emptied of live runs and can be retired and shredded | Holds; a finished run is never re-sealed, so its payload stays under the generation it finished under until purge or shredding; the listener and serverless runner run no sweep | `Runner.Client/RunnerRekeySweep.cs`, `Runner.Server/RunnerRunCoordinator.ClaimForRekeyAsync`, `WorkflowRun.ResealAsync`, `WorkflowCheckpointSerializer.cs` |
| Rotation chain: a registration into an environment that holds a generation carries the predecessor's ES256 signature over the framed rotation tuple or is refused; the link is stored on the generation, kept through retirement and advertised to runners and initiators, which follow a rotation only along links they verify back to their pinned fingerprint | Holds; the tuple carries no server challenge, the registration's own possession proof is the freshness bound | `Environments/EnvironmentKeyRotation.cs`, `ControlPlane.Server/ArazzoControlPlaneEnvironmentKeysHandler.cs`, `Runner.Client/ArazzoRunnerClient.CheckSealKeyAsync`, `ControlPlane.Cli/Commands.cs` |
