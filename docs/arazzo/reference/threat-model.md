...
| Checkpoint replay or stale write | **Holds** for the phase-A sequence rule. Single-row CAS, 409 on supersession, and the accepted sequence validated as persisted + 1 against a body sequence the ingress requires and checks against the header, so the rule no longer validates a number only the header carried | The rule is authoritative in the store but not yet MAC-bound to the runner-authored region (phase B), so a control plane that lies about acceptance is caught only once the unified MAC lands | H40 |
...
| Runner-API refusals audited | Designed | [ADR 0071](../adr/0071-authentication-event-telemetry.md) |
...
| UO-10 undetected breach | H11 | n/a | PART | n/a | PART | **One on mutations and on the control plane's reads, zero on the runner API.** A payload read is a record made before it is answered, a refused read is a record capped for each subject, and every other read is metered ([ADR 0070](../adr/0070-read-side-audit-three-tiers.md)), so a cross-tenant payload read and an enumeration are both reconstructable. A runner records the secrets it resolves on a chain of its own, where its host wires that. The runner API itself still emits nothing. Mutation audit is attributed to the canonical subject with owner group and environment, and is now durable evidence: a hash-linked chain under signed heads, outside the operational store, that a mutation cannot proceed without. It is still change-blind by decision, so it reconstructs who did what and not what changed |
...
| H11 | Crit | DIV | Runner API emits nothing, and there is no read audit anywhere | All | Open, narrowed: the read audit is built ([ADR 0070](../adr/0070-read-side-audit-three-tiers.md)), and the runner API still emits nothing |
...
detection rows they change in §8 move when the code does. GAP-6 is built and §8 says what it changed. GAP-7 is built: the control plane's three tiers, with its four payload disclosures and its refused reads on the chain and every other read metered, and a chain of the runner's own for the secrets it resolves; GAP-8 is not started.
