...
| Execution budget: fuel, wall clock and depth per run, deployment ceiling with a tightening environment override, runner-enforced and coordinator-verified on every save, a budget fault resumable only by a control-plane-authored, audited re-budget that the ceiling and the journal cap bound | Designed | [ADR 0068](../adr/0068-execution-budget-fuel-wall-clock-depth.md) |
