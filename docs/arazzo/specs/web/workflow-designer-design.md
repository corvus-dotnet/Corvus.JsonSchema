...
| `<arazzo-splitbar>` | A draggable resize bar between panes (2026-07-16): drives one CSS custom property (px) on a target element that the host grid consumes, so layouts stay declarative. Clamped `min`/`max`, `invert` for trailing panes, per-split persistence (`storage-key`), double-click / Enter reset, keyboard nudge, `role="separator"`. The designer places three: canvas ↔ inspector, canvas ↔ debug dock (drives the dock's FIXED height + tray-body max — fixed, not a cap, so the dock never resizes as step selections change content), tray ↔ session rail. |
