...
| **External schema reference** | An inputs-schema `$ref` of the form `schemas/<name>#<pointer>`: it resolves against the working copy's `jsonschema` attachment named `<name>` (never a `sourceDescription` — the Arazzo spec pins that enum), packaged alongside the sources at publish and registered with the generation resolver under the same relative URI. A reference to an unattached name is a validation error. |
