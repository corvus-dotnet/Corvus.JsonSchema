...
| **Working copy** | The mutable Arazzo document under edit in the workflow designer, with its scenarios. Durably saveable on its own, as many times as needed, without ever minting a catalog version — so development iteration does not pollute the catalog. Persisted in the designer workspace (and optionally a Git branch); *publish* mints a catalog version from it. Prefer this term over "draft save": **draft** is reserved for an unpromoted catalog version. |
| **Publish** | The act of minting a catalog version from a working copy, together with its scenarios and the evidence of their successful execution. A published-but-unpromoted version is a draft. (Serving a version at an endpoint is *hosting*, not publishing — see normalization E.) |
...
| **Schema editor** | The typed JSON-Schema authoring form (`<arazzo-schema-editor>`) for a workflow's `inputs` and the components library's input schemas, in place of a raw textarea. Edits a *subset* of JSON Schema as typed property rows and first-class `oneOf`/`anyOf`/`allOf` combiner nodes, stays lossless over the rest via **advanced nodes**, offers a Form | JSON toggle over the **guarded JSON editor**, a live typed preview, and a shared-type reference picker. |
| **Advanced node** | A schema construct the schema editor's visual tier summarises rather than fully forms — a keyword it does not render, `not`, `patternProperties`, a `$ref` carrying siblings, and the like. Shown as a labelled row that preserves the raw subschema and stays JSON-editable, so a round-trip never destroys or reorders it. |
| **Guarded JSON editor** | A parse-gated raw-JSON textarea whose edits apply only when the buffer parses, holding the last valid value otherwise (`wireGuardedJson`). Per host, a blank buffer either deletes the value or holds the last valid one. The single shared idiom behind the schema editor's JSON tier, the payload editor, and the document inspector. |
...
| **Debug run** | The durable, forward-only server-side entity a debug session drives (REST `debugRuns` tag, §18): started from a working copy, run/paused/stepped, resumable, and cancellable/deletable. Backed by a real executor whose transport is either a scenario's mock or a live dev environment; forward-only means time-travel is a replay over the recorded trace, not a rewind of the run. |
| **Trigger injection** | Delivering a message trigger into a debug run that is `Suspended` on a message-wait (REST `injectDebugRunMessage`): the payload is handed straight to the suspended run's correlated wait to resume it — nothing is published to a broker — so a message-driven workflow can be driven interactively without an out-of-band producer. |
...
| **Comparison** (workflow) | A structured diff of two workflow versions (`diffWorkflowPair`): steps, flow edges, and workflow-level surfaces classified as added / removed / changed / renamed / moved on top of the graph projection and the document model's identity-aware structural diff. Rendered by `<arazzo-workflow-compare>` as side-by-side, overlay, or text. |
| **Change classification** | The label a comparison assigns each element: *added* (present only on the right), *removed* (present only on the left), *changed* (present on both, content differs), *renamed* (a step matched by binding under a new id), *moved* (a step reordered, content identical — never *changed*). |
| **Rename (step)** | A step matched across versions by binding equality (same `operationId` / `operationPath` / `channelPath`+action / `workflowId`) under a different `stepId`. Presented as one changed step with a rename note, never a remove plus an add. |
| **Comparison mode** | The comparison's presentation, per dialog instance: **Side by side** (two surfaces), **Overlay** (one union canvas, the base version solid and the other side's exclusives translucent as *ghosts*), or **Text** (a CodeMirror MergeView over the two serialized documents). |
| **Merge target** | The side of a comparison, opened with `mergeTarget: true`, whose document the host is editing (the working copy). It turns the comparison into an interactive merge and sources its document from the live model, never a snapshot. |
| **Take / Keep** | The interactive-merge verbs on a change-list entry when there is a merge target. *Take* adopts the other side's state for that one entry (the component emits `change-accepted`/`merge-text-applied`; the host applies it to its model, undoably). *Keep* marks the entry reviewed for the session — the working copy IS the merge state, so a change not taken is kept by definition; nothing is persisted. |
...
| **Merged grantee search** | The grantee-search default when a principal directory is configured: the directory (every searchable kind) unioned with the observed-identity typeahead, directory-preferred on a `(kind, value)` collision, as one bounded unpaged result set. A directory failure degrades a merged search to observed results; only an explicit `source=directory` search surfaces it (502). |
...
| **Registered source** | An entry in the control plane's `/sources` registry: a named source document registered once and referenced by workflows, with per-environment credentials. A `jsonschema` source registers a JSON Schema document for **external schema references**. |
| **External schema reference** | An inputs-schema `$ref` to an attached `jsonschema` document (never a `sourceDescription` — the Arazzo spec pins that enum). The PREFERRED base is the document's absolute root `$id` (its canonical identity); the virtual relative path `schemas/<name>#<pointer>` is the fallback for an `$id`-less document. Packaged alongside the sources at publish and registered with the generation resolver under both identities. A reference matching no attached document is a validation error. |
