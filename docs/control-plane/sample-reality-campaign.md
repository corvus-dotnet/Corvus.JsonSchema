...
- **W4c DONE** (commit `5271248485`): `ExampleSeed.cs` in the sample — `IExampleSeed` + `ExampleSeedContext`
  ...
  - **W4c follow-up (remaining real bootstrap)** — still inline in `Program.cs`, to fold into
    `DefaultDeploymentBootstrap` next: schema-prep (`PrepareAsync`), OIDC/BFF/claims wiring, Vault AppRole trust.
    These are *real* (every deployment does them), so they belong in the bootstrap library, not the example seed.
