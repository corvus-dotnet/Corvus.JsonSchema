...
public static class SampleRegistry
{
    ...
    public static IReadOnlyList<(string Name, string Content, bool IsRoot)> LoadSampleFiles(string sampleId)
    {
        ...
        string prefix = $"Corvus.Text.Json.AsyncApi.Playground.Samples.{resourceId}.";
        string[] allResources = assembly.GetManifestResourceNames();

        string[] sampleResources = Array.FindAll(allResources, r =>
            r.StartsWith(prefix, StringComparison.Ordinal) && !r.EndsWith(".cs", StringComparison.Ordinal));

        if (sampleResources.Length == 0)
        {
            string? content = LoadEmbeddedResource($"Corvus.Text.Json.AsyncApi.Playground.Samples.{resourceId}.json");
            if (content is not null)
            {
                results.Add(("asyncapi.json", content, true));
            }
        ...
        foreach (string resourceName in sampleResources.OrderBy(r => r))
        {
            ...
            if (content is not null)
            {
                bool isRoot = fileName.StartsWith("asyncapi", StringComparison.OrdinalIgnoreCase)
    ...
    /// <summary>
    /// Loads the user code for a sample (from the embedded usercode.cs file).
    /// Returns null if no user code file exists for the sample.
    /// </summary>
    public static string? LoadSampleUserCode(string sampleId)
    {
        string resourceId = sampleId.Replace('-', '_');
        string resourceName = $"Corvus.Text.Json.AsyncApi.Playground.Samples.{resourceId}.usercode.cs";
