...
/// <summary>
/// Provides Roslyn-backed IntelliSense (completions and signature help) for the
/// custom functions C# editor. Maintains an AdhocWorkspace with a Document that
/// includes global usings, the <see cref="FunctionsWrapper"/> template, and the
/// user's dictionary initializer body — then delegates to Roslyn's CompletionService.
/// </summary>
public class IntelliSenseService : IDisposable
{
    ...
    // Offset from the start of the combined source to the first character of user code.
    private int userCodeStartOffset;
    ...
    {
        string combinedSource = this.BuildCombinedSource(userCode);

        if (this.workspace is null || this.projectId is null || this.userDocId is null)
        {
            this.BuildWorkspace(combinedSource);
        }
        else
        {
            Solution solution = this.workspace.CurrentSolution;
            SourceText newText = SourceText.From(combinedSource);
    ...
    private string BuildCombinedSource(string userCode)
    {
        (string wrappedSource, int userOffset) = FunctionsWrapper.ForIntelliSense(userCode);

        // Full source = preamble + newline + wrapper (with user code embedded)
        string combinedSource = WorkspaceService.Preamble + "\n" + wrappedSource;

        // User code offset in the combined source accounts for the preamble prefix
        this.userCodeStartOffset = WorkspaceService.Preamble.Length + 1 + userOffset;

        return combinedSource;
    }

    private void BuildWorkspace(string combinedSource)
    {
        this.workspace?.Dispose();
        ...
        solution = solution.AddDocument(
            this.userDocId,
            "Functions.cs",
            SourceText.From(combinedSource));
    ...
    private int GetUserCodeStartLine(SourceText sourceText)
    {
        ...
        {
            if (sourceText.Lines[i].Start >= this.userCodeStartOffset)
            {
                return i;
            }
        }
