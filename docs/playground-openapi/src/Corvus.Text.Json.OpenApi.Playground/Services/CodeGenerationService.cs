...
using PlaygroundGeneratedFile = Corvus.Text.Json.OpenApi.Playground.Models.GeneratedFile;

namespace Corvus.Text.Json.OpenApi.Playground.Services;

/// <summary>
/// Drives the V5 OpenAPI code generation pipeline entirely in WASM.
/// </summary>
public class CodeGenerationService
{
    ...
    /// <summary>
    /// Generate C# client and model code from OpenAPI spec files.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                OpenApi30CodeGenerator generator = new("Playground", schemaTypeMap);
                clientFiles = generator.Generate(specRoot, filter, referenceResolver);
            }
            else
            {
                OpenApi31CodeGenerator generator = new("Playground", schemaTypeMap);
                clientFiles = generator.Generate(specRoot, filter, referenceResolver);
            }
            ...
            {
                bool isRequestResponse = clientFile.FileName.EndsWith("Request.cs", StringComparison.Ordinal)
                    || clientFile.FileName.EndsWith("Response.cs", StringComparison.Ordinal);
