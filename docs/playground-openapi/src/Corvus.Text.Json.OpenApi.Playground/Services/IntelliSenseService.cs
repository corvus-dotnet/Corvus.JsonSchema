...
public class IntelliSenseService
{
    ...
    private IReadOnlyCollection<GeneratedFile>? generatedFiles;
    ...
    /// </summary>
    public void UpdateGeneratedCode(IReadOnlyCollection<GeneratedFile> files)
    {
        ...
        int column)
    {
        try
        {
            ...
            CompletionList? completions = await completionService.GetCompletionsAsync(document, position);
    ...
    private void BuildWorkspace(string userCode)
    {
        ...
        if (this.generatedFiles is not null)
        {
            ...
            foreach (var file in this.generatedFiles)
            {
                ...
                solution = solution.AddDocument(docId, $"Generated_{i}.cs", SourceText.From(file.Content));
                i++;
            }
        }
        ...
        solution = solution.AddDocument(this.userDocId, "Program.cs", SourceText.From(userSource));
        this.workspace.TryApplyChanges(solution);
    }
