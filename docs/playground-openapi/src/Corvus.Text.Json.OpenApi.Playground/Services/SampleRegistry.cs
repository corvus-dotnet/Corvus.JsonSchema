...
public static class SampleRegistry
{
    ...
    public static IReadOnlyList<(string Name, string Content, bool IsRoot)> LoadSampleFiles(string sampleId)
    {
        ...
        string prefix = $"Corvus.Text.Json.OpenApi.Playground.Samples.{resourceId}.";
        ...
        {
            string? content = LoadEmbeddedResource($"Corvus.Text.Json.OpenApi.Playground.Samples.{resourceId}.json");
    ...
    {
        string resourceId = sampleId.Replace('-', '_');
        string resourceName = $"Corvus.Text.Json.OpenApi.Playground.Samples.{resourceId}.usercode.cs";
