...
export function evaluatePerson(value: unknown, ev: Ev): boolean {
  ...
  if (__isObj(value)) {
    if (!Object.prototype.hasOwnProperty.call(value, "familyName")) { return false; }
    if (!Object.prototype.hasOwnProperty.call(value, "givenName")) { return false; }
  }
  if (__isObj(value)) {
    ...
    for (const k in o) {
      i++;
      if (k === "birthDate") { if (!evaluateBirthDate(o[k], NOEV)) { return false; } ev.markProp(i); }
      else if (k === "familyName") { if (!evaluateFamilyName(o[k], NOEV)) { return false; } ev.markProp(i); }
      else if (k === "givenName") { if (!evaluateGivenName(o[k], NOEV)) { return false; } ev.markProp(i); }
      else if (k === "height") { if (!evaluateHeight(o[k], NOEV)) { return false; } ev.markProp(i); }
      else if (k === "otherNames") { if (!evaluateOtherNames(o[k], NOEV)) { return false; } ev.markProp(i); }
    }
  }
  return true;
}
...
export function evaluateFamilyName(value: unknown, ev: Ev): boolean {
  if (!(typeof value === "string")) { return false; }
  return true;
}

export function evaluateGivenName(value: unknown, ev: Ev): boolean {
  if (!(typeof value === "string")) { return false; }
  return true;
}
...
export function evaluateOtherNames(value: unknown, ev: Ev): boolean {
  if (!(typeof value === "string")) { return false; }
  return true;
}
