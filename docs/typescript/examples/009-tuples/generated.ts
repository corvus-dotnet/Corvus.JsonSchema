...
export const Point3D = {
  evaluate: (v: unknown, results?: Results): boolean => evaluatePoint3D(v instanceof Uint8Array ? decodeAndParse(v) : v, fresh(), "", "", results ?? null),
  parse: (v: Uint8Array | string): Point3D => (v instanceof Uint8Array ? decodeAndParse(v) : JSON.parse(v)) as Point3D,
  build: buildPoint3D,
  buildCanonical: buildCanonicalPoint3D,
  patch: patchPoint3D,
  produce: producePoint3D,
  applyPatch: (doc: Uint8Array | Point3D, patch: JsonPatch): Uint8Array => canonicalize(applyPatch(doc instanceof Uint8Array ? decodeAndParse(doc) : doc, patch)),
  applyMergePatch: (doc: Uint8Array | Point3D, mergePatch: unknown): Uint8Array => canonicalize(applyMergePatch(doc instanceof Uint8Array ? decodeAndParse(doc) : doc, mergePatch)),
  createPatch: (source: Uint8Array | Point3D, target: Uint8Array | Point3D): JsonPatchOp[] => createPatch(source instanceof Uint8Array ? decodeAndParse(source) : source, target instanceof Uint8Array ? decodeAndParse(target) : target),
  createMergePatch: (source: Uint8Array | Point3D, target: Uint8Array | Point3D): unknown => createMergePatch(source instanceof Uint8Array ? decodeAndParse(source) : source, target instanceof Uint8Array ? decodeAndParse(target) : target),
};
export const Coord = {
...
export const PrefixItems2 = {
  evaluate: (v: unknown, results?: Results): boolean => evaluatePrefixItems2(v instanceof Uint8Array ? decodeAndParse(v) : v, fresh(), "", "", results ?? null),
};
export const PrefixItems3 = {
  evaluate: (v: unknown, results?: Results): boolean => evaluatePrefixItems3(v instanceof Uint8Array ? decodeAndParse(v) : v, fresh(), "", "", results ?? null),
};
