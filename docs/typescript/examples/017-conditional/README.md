...
TypeScript can't express "if method is card then cardNumber is required" as a single object type without splitting it into a union. The generator keeps the base shape (`cardNumber?`) and enforces the rule in `Payment.evaluate`. If you want the *type* to capture the two cases, model them as a `oneOf` (a discriminated union) instead.
