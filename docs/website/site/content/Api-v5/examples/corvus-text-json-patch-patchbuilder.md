Build an RFC 6902 JSON Patch document using the fluent builder API. Create a builder by calling `BeginPatch(workspace)` on a mutable element, chain operations, then call `GetPatchAndDispose()` to produce a `JsonPatchDocument`.
...
```csharp
JsonPatchDocument reorgPatch = root.BeginPatch(workspace)
    .Move("/tags"u8, "/labels"u8)
    .Copy("/name"u8, "/display_name"u8)
    .GetPatchAndDispose();

root.TryApplyPatch(reorgPatch);
```
