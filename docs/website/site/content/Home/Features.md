...
`ParsedJsonDocument<T>` uses `ArrayPool<byte>` to parse JSON with minimal GC impact. Just 136B per-document allocation vs 1,528 bytes for equivalent `JsonNode` operations — 91% less memory. Generated types are thin struct wrappers — creating a typed value from a parsed document is essentially free.
