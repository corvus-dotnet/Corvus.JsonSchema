// Self-contained, CI-runnable codegen-aware compliance harness for the corvus-py Python JSON Schema engine.
// A faithful port of the TypeScript compliance harness (prototypes/ts-bench/compliance/Program.cs): it walks
// every dialect (draft4/6/7/2019-09/2020-12) of the JSON-Schema-Test-Suite and, for EACH test-group, runs the
// Python code generator and writes one importable PACKAGE (module-per-type directory, `__init__.py` re-exports
// `evaluate`) plus a manifest.json recording { module, dialect, file, group, error, tests:[{ data, valid, desc }] }
// (data = the instance's raw JSON source text). Errored groups record `error` + a null module. suite-runner.py
// imports each package's `evaluate` and tallies pass/fail per dialect.
//
// The generator is set up EXACTLY as the TS harness / the C# test runner: FakeWebDocumentResolver (offline
// remotes) + metaschema docs + all analysers + rebaseAsRoot. CreateDefault normally; CreateWithFormatAssertion
// for optional/format/*, optional/content, and optional/format-assertion. Depends ONLY on the public codegen
// libraries.
//
...
//   <outDir>        where the packages + manifest.json are written (default: out-suite)
using System.Text;
...
internal static class SuiteHarness
{
    ...
    public static async Task Run(string testsBaseDir, string outDir)
    {
        ...
        Console.WriteLine($"Built {built} packages, {errored} errored, {totalTests} cases across {Dialects.Length} dialects -> {outDir}/manifest.json");
        return;
        ...
        async Task ProcessFile(string filePath, string dialect, IVocabulary fallback, string label, bool assertFormat)
        {
            ...
            foreach (JsonElement group in doc.RootElement.EnumerateArray())
            {
                ...
                string moduleName = Sanitize($"g_{dialect}_{label}_{i}");
                i++;
                ...
                {
                    string boolPkg = Path.Combine(outDir, moduleName);
                    Directory.CreateDirectory(boolPkg);
                    File.WriteAllText(Path.Combine(boolPkg, "__init__.py"), $"def evaluate(value, results=None):\n    return {(schema.ValueKind == JsonValueKind.True ? "True" : "False")}\n");
                ...
                try
                {
                    ...
                    PythonLanguageProvider provider = assertFormat ? PythonLanguageProvider.CreateWithFormatAssertion() : PythonLanguageProvider.CreateDefault();
                    provider.RootHint = root;
                    IReadOnlyCollection<GeneratedCodeFile> files = builder.GenerateCodeUsing(provider, CancellationToken.None, root);

                    string pkgDir = Path.Combine(outDir, moduleName);
                    Directory.CreateDirectory(pkgDir);
                    string export = provider.RootEvaluateExport(root);
                    foreach (GeneratedCodeFile file in files)
                    {
                        string content = file.FileName == "__init__.py" ? file.FileContent + export : file.FileContent;
                        File.WriteAllText(Path.Combine(pkgDir, file.FileName), content);
                    }
        ...
        // A valid Python package name: non-identifier characters (the dialect's hyphen, a filename's '-', '.')
        // become '_'. The trailing group index keeps names unique within a file.
