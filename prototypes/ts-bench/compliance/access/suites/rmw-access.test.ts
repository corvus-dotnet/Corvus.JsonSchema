...
import { Sample } from "./out-rmw/generated.js";
...
function rt(src: string, changes: Partial<Sample>): string {
  return dec.decode(Sample.patch(enc.encode(src), changes));
}
...
// 6) pretty-printed source (whitespace between tokens)
{
  ...
  const out = dec.decode(Sample.patch(enc.encode(pretty), { seq: 99 }));
...
{
  const o1 = JSON.parse(dec.decode(Sample.patch(enc.encode(rich), { seq: "now-a-string" } as unknown as Partial<Sample>)));
  eq("9 seq int->string", o1.seq, "now-a-string");
  const o2 = JSON.parse(dec.decode(Sample.patch(enc.encode(rich), { active: null } as unknown as Partial<Sample>)));
...
// 14) upsert: setting a member absent from the source ADDS it (set = replace-or-add)
{
  ...
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(noSeq), { seq: 5 })));
...
// 15) BOM-prefixed source: BOM copied through, member still located
{
  ...
  const outBytes = Sample.patch(bom, { seq: 99 });
...
// 16) replacement with large / structural size deltas: retained-around members (before AND after) intact
{
  ...
  const shrink = JSON.parse(dec.decode(Sample.patch(enc.encode(rich), { address: 42 } as unknown as Partial<Sample>)));
  ...
  const grow2 = JSON.parse(dec.decode(Sample.patch(enc.encode(rich), { seq: { nested: [1, 2] } } as unknown as Partial<Sample>)));
...
const rtRem = (src: string, ch: Partial<Sample>, rm: ReadonlyArray<keyof Sample>): string =>
  dec.decode(Sample.patch(enc.encode(src), ch, rm));
...
{
  const noSeq = JSON.stringify({ id: "x", a: 5 });
  throws("21 throw remove missing", () => Sample.patch(enc.encode(noSeq), {}, ["seq"]));
}
// 22) removal under pretty-print
{
  const pretty = JSON.stringify(JSON.parse(rich), null, 2);
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(pretty), {}, ["active"])));
...
// 24) add members absent from the source
{
  ...
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(partial), { note: "added", active: true })));
  ok("24 add absent props", p.note === "added" && p.active === true && p.id === "x" && p.seq === 1);
}
// 25) replace existing + add absent in one call
{
  const partial = JSON.stringify({ id: "x", seq: 1 });
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(partial), { seq: 99, note: "new" })));
...
// 26) add appends at end; multibyte sibling retained byte-wise
{
  ...
  const out = dec.decode(Sample.patch(enc.encode(noNote), { note: "メモ" }));
  const p = JSON.parse(out);
...
{
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode("{}"), { seq: 5 })));
  ok("27 add to empty", p.seq === 5);
}
// 28) add + remove in one call
{
  const src = JSON.stringify({ id: "x", seq: 1, ratio: 0.5 });
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(src), { note: "n" }, ["ratio"])));
...
// 29) add complex (nested object + array) values
{
  ...
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(src), { address: { street: "S", city: "C", zip: "Z" }, tags: ["t1", "t2"] })));
...
// 30) pretty-printed add (assert parsed)
{
  ...
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(pretty), { note: "n" })));
...
// 31) patch as overlay: replace existing + add new, leave the rest
{
  ...
  const p = JSON.parse(dec.decode(Sample.patch(enc.encode(base), { seq: 99, note: "added" })));
...
// 32) build round-trips and equals native JSON.stringify(props) (caller key order, NOT canonicalised)
{
  ...
  const out = dec.decode(Sample.build(props));
  const p = JSON.parse(out);
...
// 33) build empty -> {}
{
  ok("33 build empty", dec.decode(Sample.build({})) === "{}");
}
// 34) build with complex (nested object + array) values
{
  const p = JSON.parse(dec.decode(Sample.build({ id: "x", address: { street: "S", city: "C", zip: "Z" }, tags: ["t"] })));
...
{
  const built = Sample.build({ id: "x", seq: 1 });
  const p = JSON.parse(dec.decode(Sample.patch(built, { seq: 99, note: "n" })));
...
{
  const out = dec.decode(Sample.patch(enc.encode(rich), {}, undefined, { tags: { append: ["δ"] } }));
  const p = JSON.parse(out);
...
{
  const ins = JSON.parse(dec.decode(Sample.patch(enc.encode(rich), {}, undefined, { tags: { insert: { 1: ["X"] } } })));
  ok("37 nested insert", JSON.stringify(ins.tags) === JSON.stringify(["alpha", "X", "β", "γ"]));
  const setd = JSON.parse(dec.decode(Sample.patch(enc.encode(rich), {}, undefined, { tags: { set: { 0: "ZERO" } } })));
  ok("37 nested set", JSON.stringify(setd.tags) === JSON.stringify(["ZERO", "β", "γ"]));
  const remd = JSON.parse(dec.decode(Sample.patch(enc.encode(rich), {}, undefined, { tags: { removeAt: [1] } })));
...
{
  const out = dec.decode(Sample.patch(enc.encode(rich), { seq: 99 }, ["ratio"], { tags: { append: ["new"] } }));
  const p = JSON.parse(out);
...
{
  const out = dec.decode(Sample.patch(enc.encode(rich), {}, undefined, { tags: { append: ["d", "e"] } }));
  const p = JSON.parse(out);
