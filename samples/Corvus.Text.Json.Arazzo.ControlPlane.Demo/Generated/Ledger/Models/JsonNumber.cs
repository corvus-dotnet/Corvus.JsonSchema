...
#endif
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator long(JsonNumber value) => value._parent.TryGetValue(value._idx, out long result) ? result : throw new FormatException();

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator double(JsonNumber value) => value._parent.TryGetValue(value._idx, out double result) ? result : throw new FormatException();

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator Corvus.Numerics.BigNumber(JsonNumber value) => value._parent.TryGetValue(value._idx, out Corvus.Numerics.BigNumber result) ? result : throw new FormatException();

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator System.Numerics.BigInteger(JsonNumber value) => value._parent.TryGetValue(value._idx, out System.Numerics.BigInteger result) ? result : throw new FormatException();
    ...
    /// <c>True</c> if the values are equal.
    /// </returns>
    public static bool operator ==(in JsonNumber left, in JsonElement right)
    {
        return left.Equals(right);
    }
    ...
    {
        return JsonNumber.From(instance);
    }
    ...
    ///   A value could not be read from the reader.
    /// </exception>
    public static bool TryParseValue(ref Utf8JsonReader reader, out JsonNumber? result)
    {
        return JsonElementHelpers.TryParseValue<JsonNumber>(ref reader, out result);
    }
    ...
    {
        return
            (obj is IJsonElement value && Equals(new JsonNumber(value.ParentDocument, value.ParentDocumentIndex))) ||
...
#if NET
    static JsonNumber IJsonElement<JsonNumber>.CreateInstance(IJsonDocument parentDocument, int parentDocumentIndex) => new(parentDocument, parentDocumentIndex);
#endif

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    private string DebuggerDisplay => $"JsonNumber: ValueKind = {ValueKind} : \"{ToString()}\"";
    ...
    /// <summary>
    /// Gets a <see cref="JsonNumber"/> which can be safely stored beyond the lifetime of the
    /// original document.
    /// </summary>
    /// <returns>
    /// A <see cref="JsonNumber"/> which can be safely stored beyond the lifetime of the
    /// original document.
    /// </returns>
    /// <remarks>
    /// <para>
    ...
    {
        CheckValidInstance();
        return _parent.CloneElement<JsonNumber>(_idx);
    }
    ...
    /// </summary>
    /// <returns>
    /// An immutable <see cref="JsonNumber"/> that lives for the lifetime of its
