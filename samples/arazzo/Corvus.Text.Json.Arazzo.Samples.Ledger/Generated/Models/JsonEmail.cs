...
#endif
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator string(JsonEmail value) => value._parent.GetString(value._idx, JsonTokenType.String) ?? throw new FormatException();
    ...
    /// <c>True</c> if the values are equal.
    /// </returns>
    public static bool operator ==(in JsonEmail left, in JsonElement right)
    {
        return left.Equals(right);
    }
    ...
    {
        return JsonEmail.From(instance);
    }
    ...
    ///   A value could not be read from the reader.
    /// </exception>
    public static bool TryParseValue(ref Utf8JsonReader reader, out JsonEmail? result)
    {
        return JsonElementHelpers.TryParseValue<JsonEmail>(ref reader, out result);
    }
    ...
    {
        return
            (obj is IJsonElement value && Equals(new JsonEmail(value.ParentDocument, value.ParentDocumentIndex))) ||
...
#if NET
    static JsonEmail IJsonElement<JsonEmail>.CreateInstance(IJsonDocument parentDocument, int parentDocumentIndex) => new(parentDocument, parentDocumentIndex);
#endif

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    private string DebuggerDisplay => $"JsonEmail: ValueKind = {ValueKind} : \"{ToString()}\"";
    ...
    /// <summary>
    /// Gets a <see cref="JsonEmail"/> which can be safely stored beyond the lifetime of the
    /// original document.
    /// </summary>
    /// <returns>
    /// A <see cref="JsonEmail"/> which can be safely stored beyond the lifetime of the
    /// original document.
    /// </returns>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    public JsonEmail Clone()
    {
        CheckValidInstance();
        return _parent.CloneElement<JsonEmail>(_idx);
    }
    ...
    /// </summary>
    /// <returns>
    /// An immutable <see cref="JsonEmail"/> that lives for the lifetime of its
