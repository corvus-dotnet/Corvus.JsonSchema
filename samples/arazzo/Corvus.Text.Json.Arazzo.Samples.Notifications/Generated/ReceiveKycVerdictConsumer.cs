...
/// </summary>
public sealed class ReceiveKycVerdictConsumer : IAsyncDisposable
{
    private readonly IMessageTransport transport;
    private readonly IReceiveKycVerdictHandler handler;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReceiveKycVerdictConsumer"/> class.
    /// </summary>
    ...
    public ReceiveKycVerdictConsumer(IMessageTransport transport, IReceiveKycVerdictHandler handler, ValidationMode validationMode = ValidationMode.Basic, IMessageErrorPolicy? errorPolicy = null, IMessageAuthenticationProvider? authProvider = null)
    {
    ...
    public async ValueTask StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await this.transport.SubscribeAsync<Corvus.Text.Json.Arazzo.Samples.Notifications.Models.KycVerdictPayload>(ChannelAddressUtf8, this.HandleMessageAsync, cancellationToken).ConfigureAwait(false);
        }
        catch
        {
    ...
    private async ValueTask HandleMessageAsync(Corvus.Text.Json.Arazzo.Samples.Notifications.Models.KycVerdictPayload payload, Corvus.Text.Json.JsonElement headers, CancellationToken cancellationToken)
    {
        ...
        if (failure is null)
        {
            try
            {
                await this.handler.HandleKycVerdictAsync(payload, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                failure = ex;
