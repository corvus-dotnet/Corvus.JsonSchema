...
/// <summary>
/// Consumer for the <c>receiveKycVerdict</c> operation on channel <c>kyc.verdict</c>.
/// </summary>
public sealed class ReceiveKycVerdictWithDeliveryContextConsumer : IAsyncDisposable
{
    private readonly IMessageDeliveryContextTransport transport;
    private readonly IReceiveKycVerdictWithDeliveryContextHandler handler;
    ...
    private const string DeadLetterChannel = "dead-letter.kyc.verdict";
    private static readonly byte[] DeadLetterChannelUtf8 = "dead-letter.kyc.verdict"u8.ToArray();

    /// <summary>
    /// Initializes a new instance of the <see cref="ReceiveKycVerdictWithDeliveryContextConsumer"/> class.
    /// </summary>
    ...
    public ReceiveKycVerdictWithDeliveryContextConsumer(IMessageDeliveryContextTransport transport, IReceiveKycVerdictWithDeliveryContextHandler handler, ValidationMode validationMode = ValidationMode.Basic, IMessageErrorPolicy? errorPolicy = null, IMessageAuthenticationProvider? authProvider = null)
    {
    ...
    public async ValueTask StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await this.transport.SubscribeWithDeliveryContextAsync<Corvus.Text.Json.Arazzo.Samples.Notifications.Models.KycVerdictPayload>(ChannelAddressUtf8, this.HandleMessageAsync, cancellationToken).ConfigureAwait(false);
        }
        catch
        {
    ...
    private async ValueTask HandleMessageAsync(Corvus.Text.Json.Arazzo.Samples.Notifications.Models.KycVerdictPayload payload, MessageDeliveryContext deliveryContext, CancellationToken cancellationToken)
    {
        ...
        if (failure is null)
        {
            try
            {
                await this.handler.HandleKycVerdictAsync(payload, deliveryContext, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                failure = ex;
