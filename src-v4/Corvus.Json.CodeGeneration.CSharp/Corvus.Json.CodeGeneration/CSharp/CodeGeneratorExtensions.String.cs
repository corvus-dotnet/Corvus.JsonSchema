    ...
    private static CodeGenerator AppendFormattableMethods(this CodeGenerator generator, TypeDeclaration typeDeclaration)
    {
        ...
        generator
            ...
            .PushIndent()
                ...
                .AppendSeparatorLine()
                .AppendLineIndent("string formatted = this.ToString(format.IsEmpty ? null : format.ToString(), provider);")
                .AppendLineIndent("int requiredLength = global::System.Text.Encoding.UTF8.GetByteCount(formatted);")
                ...
                .AppendLineIndent("}")
                .AppendSeparatorLine()
                .AppendLineIndent("bytesWritten = global::System.Text.Encoding.UTF8.GetBytes(formatted.AsSpan(), utf8Destination);")
                .AppendLineIndent("return true;")
            .PopIndent()
            .AppendLineIndent("}")
            .AppendLine("#endif");
