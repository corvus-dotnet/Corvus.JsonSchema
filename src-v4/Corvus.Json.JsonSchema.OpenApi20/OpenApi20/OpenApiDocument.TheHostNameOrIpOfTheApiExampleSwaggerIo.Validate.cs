...
public readonly partial struct OpenApiDocument
{
    ...
    /// </para>
    /// </remarks>
    public readonly partial struct TheHostNameOrIpOfTheApiExampleSwaggerIo
    {
        /// <inheritdoc/>
        public ValidationContext Validate(in ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
        {
            ...
            if (level > ValidationLevel.Basic)
            {
                ...
                result = result.PushSchemaLocation("http://swagger.io/v2/schema.json#/properties/host");
            }
        ...
        public static partial class CorvusValidation
        {
            ...
            internal static ValidationContext StringValidationHandler(
                in TheHostNameOrIpOfTheApiExampleSwaggerIo value,
                JsonValueKind valueKind,
                in ValidationContext validationContext,
                ValidationLevel level = ValidationLevel.Flag)
            {
                ...
                static bool StringValidator(ReadOnlySpan<char> input, in Corvus.Json.Validate.ValidationContextWrapper context, out ValidationContext result)
                {
                    ...
                    if (Pattern.IsMatch(input))
                    {
                        ...
                        {
                            result = result.WithResult(isValid: true, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} matched '^[^{{}}/ :\\\\]+(?::\\d+)?$'");
                        }
                    }
                    else
                    {
                        ...
                        {
                            result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} did not match '^[^{{}}/ :\\\\]+(?::\\d+)?$'");
                        }
                        else
                        {
                            result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), "Validation pattern - The value did not match '^[^{{}}/ :\\\\]+(?::\\d+)?$'");
                        }
                    }
...
#else
            private static Regex CreatePattern() => new("^[^{}/ :\\\\]+(?::[0-9]+)?$", RegexOptions.Compiled);
#endif
        }
    }
}
