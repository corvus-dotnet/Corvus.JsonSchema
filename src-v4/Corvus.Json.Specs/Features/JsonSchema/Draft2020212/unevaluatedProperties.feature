...
Feature: unevaluatedProperties draft2020-12
    In order to use json-schema
    As a developer
    I want to support unevaluatedProperties in draft2020-12
...
Scenario Outline: unevaluatedProperties with adjacent bool additionalProperties
/* Schema: 
{
            "$schema": "https://json-schema.org/draft/2020-12/schema",
...
Scenario Outline: unevaluatedProperties with adjacent non-bool additionalProperties
/* Schema: 
{
            "$schema": "https://json-schema.org/draft/2020-12/schema",
...
*/
    ...
    Examples:
        ...
        # { "foo": "foo", "bar": "bar", "baz": "baz" }
        | #/008/tests/001/data | false | with additional properties                                                       |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo" }
        | #/009/tests/000/data | true  | with no additional properties                                                    |
        # { "foo": "foo", "bar": "bar" }
        | #/009/tests/001/data | true  | with additional properties                                                       |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo" }
        | #/010/tests/000/data | true  | with no nested unevaluated properties                                            |
        # { "foo": "foo", "bar": "bar" }
        | #/010/tests/001/data | true  | with nested unevaluated properties                                               |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo", "bar": "bar" }
        | #/011/tests/000/data | true  | when one matches and has no unevaluated properties                               |
        # { "foo": "foo", "bar": "bar", "baz": "not-baz" }
        | #/011/tests/001/data | false | when one matches and has unevaluated properties                                  |
        # { "foo": "foo", "bar": "bar", "baz": "baz" }
        | #/011/tests/002/data | true  | when two match and has no unevaluated properties                                 |
        # { "foo": "foo", "bar": "bar", "baz": "baz", "quux": "not-quux" }
        | #/011/tests/003/data | false | when two match and has unevaluated properties                                    |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo", "bar": "bar" }
        | #/012/tests/000/data | true  | with no unevaluated properties                                                   |
        # { "foo": "foo", "bar": "bar", "quux": "quux" }
        | #/012/tests/001/data | false | with unevaluated properties                                                      |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo", "bar": "bar" }
        | #/013/tests/000/data | false | with unevaluated properties                                                      |
...
*/
    ...
    Examples:
        ...
        # { "baz": "baz" }
        | #/015/tests/002/data | true  | when if is false and has no unevaluated properties                               |
        # { "foo": "else", "baz": "baz" }
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "then", "bar": "bar" }
        | #/016/tests/000/data | true  | when if is true and has no unevaluated properties                                |
        # { "foo": "then", "bar": "bar", "baz": "baz" }
        | #/016/tests/001/data | false | when if is true and has unevaluated properties                                   |
        # { "baz": "baz" }
        | #/016/tests/002/data | false | when if is false and has no unevaluated properties                               |
        # { "foo": "else", "baz": "baz" }
        | #/016/tests/003/data | false | when if is false and has unevaluated properties                                  |
...
Scenario Outline: unevaluatedProperties with $dynamicRef
/* Schema: 
{
            "$schema": "https://json-schema.org/draft/2020-12/schema",
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo", "bar": "bar" }
        | #/021/tests/000/data | true  | with no unevaluated properties                                                   |
        # { "foo": "foo", "bar": "bar", "baz": "baz" }
        | #/021/tests/001/data | false | with unevaluated properties                                                      |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": 1 }
        | #/023/tests/000/data | false | always fails                                                                     |
...
*/
    ...
    Examples:
        ...
        # { "foo": "foo", "bar": "bar" }
        | #/025/tests/001/data | true  | with nested unevaluated properties                                               |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo" }
        | #/026/tests/000/data | false | with no nested unevaluated properties                                            |
        # { "foo": "foo", "bar": "bar" }
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo" }
        | #/027/tests/000/data | true  | with no nested unevaluated properties                                            |
        # { "foo": "foo", "bar": "bar" }
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo" }
        | #/028/tests/000/data | false | with no nested unevaluated properties                                            |
        # { "foo": "foo", "bar": "bar" }
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "foo" }
        | #/029/tests/000/data | true  | with no nested unevaluated properties                                            |
        # { "foo": "foo", "bar": "bar" }
        | #/029/tests/001/data | false | with nested unevaluated properties                                               |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": { "bar": "test" } }
        | #/030/tests/000/data | true  | no extra properties                                                              |
        # { "foo": { "bar": "test", "faz": "test" } }
        | #/030/tests/001/data | false | uncle keyword evaluation is not significant                                      |
...
*/
    ...
    Examples:
        ...
        # { "foo": 1 }
        | #/031/tests/001/data | true  | in place applicator siblings, bar is missing                                     |
        # { "bar": 1 }
        | #/031/tests/002/data | false | in place applicator siblings, foo is missing                                     |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": 1, "bar": 1 }
        | #/032/tests/000/data | false | base case: both properties present                                               |
        # { "foo": 1 }
        | #/032/tests/001/data | false | in place applicator siblings, bar is missing                                     |
        # { "bar": 1 }
        | #/032/tests/002/data | true  | in place applicator siblings, foo is missing                                     |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # {}
        | #/033/tests/000/data | true  | Empty is valid                                                                   |
        # { "x": {} }
        | #/033/tests/001/data | true  | Single is valid                                                                  |
        # { "x": {}, "y": {} }
        | #/033/tests/002/data | false | Unevaluated on 1st level is invalid                                              |
        # { "x": { "x": {} } }
        | #/033/tests/003/data | true  | Nested is valid                                                                  |
        # { "x": { "x": {}, "y": {} } }
        | #/033/tests/004/data | false | Unevaluated on 2nd level is invalid                                              |
        # { "x": { "x": { "x": {} } } }
        | #/033/tests/005/data | true  | Deep nested is valid                                                             |
        # { "x": { "x": { "x": {}, "y": {} } } }
        | #/033/tests/006/data | false | Unevaluated on 3rd level is invalid                                              |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # {}
        | #/034/tests/000/data | false | Empty is invalid (no x or y)                                                     |
        # { "a": 1, "b": 1 }
        | #/034/tests/001/data | false | a and b are invalid (no x or y)                                                  |
        # { "x": 1, "y": 1 }
        | #/034/tests/002/data | false | x and y are invalid                                                              |
        # { "a": 1, "x": 1 }
        | #/034/tests/003/data | true  | a and x are valid                                                                |
        # { "a": 1, "y": 1 }
        | #/034/tests/004/data | true  | a and y are valid                                                                |
        # { "a": 1, "b": 1, "x": 1 }
        | #/034/tests/005/data | true  | a and b and x are valid                                                          |
        # { "a": 1, "b": 1, "y": 1 }
        | #/034/tests/006/data | true  | a and b and y are valid                                                          |
        # { "a": 1, "b": 1, "x": 1, "y": 1 }
        | #/034/tests/007/data | false | a and b and x and y are invalid                                                  |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # {}
        | #/035/tests/000/data | false | Empty is invalid                                                                 |
        # { "a": 1 }
        | #/035/tests/001/data | true  | a is valid                                                                       |
        # { "b": 1 }
        | #/035/tests/002/data | true  | b is valid                                                                       |
        # { "c": 1 }
        | #/035/tests/003/data | true  | c is valid                                                                       |
        # { "d": 1 }
        | #/035/tests/004/data | true  | d is valid                                                                       |
        # { "a": 1, "b": 1 }
        | #/035/tests/005/data | false | a + b is invalid                                                                 |
        # { "a": 1, "c": 1 }
        | #/035/tests/006/data | false | a + c is invalid                                                                 |
        # { "a": 1, "d": 1 }
        | #/035/tests/007/data | false | a + d is invalid                                                                 |
        # { "b": 1, "c": 1 }
        | #/035/tests/008/data | false | b + c is invalid                                                                 |
        # { "b": 1, "d": 1 }
        | #/035/tests/009/data | false | b + d is invalid                                                                 |
        # { "c": 1, "d": 1 }
        | #/035/tests/010/data | false | c + d is invalid                                                                 |
        # { "xx": 1 }
        | #/035/tests/011/data | true  | xx is valid                                                                      |
        # { "xx": 1, "foox": 1 }
        | #/035/tests/012/data | true  | xx + foox is valid                                                               |
        # { "xx": 1, "foo": 1 }
        | #/035/tests/013/data | false | xx + foo is invalid                                                              |
        # { "xx": 1, "a": 1 }
        | #/035/tests/014/data | false | xx + a is invalid                                                                |
        # { "xx": 1, "b": 1 }
        | #/035/tests/015/data | false | xx + b is invalid                                                                |
        # { "xx": 1, "c": 1 }
        | #/035/tests/016/data | false | xx + c is invalid                                                                |
        # { "xx": 1, "d": 1 }
        | #/035/tests/017/data | false | xx + d is invalid                                                                |
        # { "all": 1 }
        | #/035/tests/018/data | true  | all is valid                                                                     |
        # { "all": 1, "foo": 1 }
        | #/035/tests/019/data | true  | all + foo is valid                                                               |
        # { "all": 1, "a": 1 }
        | #/035/tests/020/data | false | all + a is invalid                                                               |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # True
        | #/036/tests/000/data | true  | ignores booleans                                                                 |
        # 123
        | #/036/tests/001/data | true  | ignores integers                                                                 |
        # 1.0
        | #/036/tests/002/data | true  | ignores floats                                                                   |
        # []
        | #/036/tests/003/data | true  | ignores arrays                                                                   |
        # foo
        | #/036/tests/004/data | true  | ignores strings                                                                  |
        # 
        | #/036/tests/005/data | true  | ignores null                                                                     |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # {"foo": null}
        | #/037/tests/000/data | true  | allows null valued properties                                                    |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # {"a": 1}
        | #/038/tests/000/data | true  | allows only number properties                                                    |
        # {"a": "b"}
        | #/038/tests/001/data | false | string property is invalid                                                       |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": "a" }
        | #/039/tests/000/data | true  | valid in case if is evaluated                                                    |
        # { "bar": "a" }
        | #/039/tests/001/data | false | invalid in case if is evaluated                                                  |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # {"foo": ""}
        | #/040/tests/000/data | false | unevaluatedProperties doesn't consider dependentSchemas                          |
        # {"bar": ""}
        | #/040/tests/001/data | false | unevaluatedProperties doesn't see bar when foo2 is absent                        |
        # { "foo2": "", "bar": ""}
        | #/040/tests/002/data | true  | unevaluatedProperties sees bar when foo2 is present                              |
...
*/
    ...
    Examples:
        | inputDataReference   | valid | description                                                                      |
        # { "foo": { "bar": "foo" }, "bar": "bar" }
        | #/041/tests/000/data | false | with an unevaluated property that exists at another location                     |
