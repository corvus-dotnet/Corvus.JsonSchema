...
public class UriTemplateSteps
{
    ...
    public void WhenIApplyTheVariablesToTheTemplateVar(string template)
    {
        try
        {
            var uriTemplate = new UriTemplate(template, createParameterParser: false, parameters: this.scenarioContext.Get<ImmutableDictionary<string, JsonAny>>(VariablesKey));
