// <copyright file="OpenApiCallbackServerCommand.cs" company="Endjin Limited">
...
/// <summary>
/// Spectre.Console.Cli command for generating server stubs from OpenAPI webhooks and callbacks.
/// </summary>
/// <remarks>
/// <para>
/// This generates the same output as <c>openapi-server</c> (handler interfaces, parameter structs,
/// result types, endpoint registration) but walks webhook and callback path-items instead of the
/// main <c>paths</c> object.
/// </para>
/// <para>
/// Use this when building the <em>client</em> side of an API: the service expects you to expose
/// server endpoints that it will call at the URLs specified in callbacks/webhooks.
/// </para>
/// </remarks>
internal sealed class OpenApiCallbackServerCommand : AsyncCommand<OpenApiGenerateSettings>
{
    /// <inheritdoc/>
    protected override async Task<int> ExecuteAsync(CommandContext context, OpenApiGenerateSettings settings, CancellationToken cancellationToken)
    {
        ...
        {
            SchemaReference[] schemaRefs = OpenApi32CodeGenerator.CollectWebhookAndCallbackSchemaPointers(specRoot, out var parameterNames, filter, referenceResolver);
            ...
            AnsiConsole.MarkupLine($"[green]Callback/webhook schemas:[/] {schemaRefs.Length}");
            ...
            files = generator.GenerateCallbackServer(specRoot, filter, referenceResolver);
        }
        else if (specVersion is "3.1" or not "3.0")
        {
            SchemaReference[] schemaRefs = OpenApi31CodeGenerator.CollectWebhookAndCallbackSchemaPointers(specRoot, out var parameterNames, filter, referenceResolver);
        ...
        {
            SchemaReference[] schemaRefs = OpenApi30CodeGenerator.CollectWebhookAndCallbackSchemaPointers(specRoot, out var parameterNames, filter, referenceResolver);
        ...
        AnsiConsole.MarkupLine($"[green]Callback server files:[/] {files.Count}");
