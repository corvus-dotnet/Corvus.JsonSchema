...
// restore runs this entry once: argv[1] names the sidecar's invocation endpoint, and the remaining
// ARAZZO_SOURCE__<name>=<url> arguments seed the deployment's source bindings (argv is frozen into the snapshot at
// evolve, so it carries only deployment-stable values — the per-run invocation is fetched over the allowlisted guest
// network). The executor is the signed executor.dll this project references, constructed directly (no loader, no
// reflection) so the whole guest is Native-AOT compiled.
using System;
using System.Collections;
using System.Collections.Generic;
using System.Net.Http;
...
try
{
    ...
    else
    {
        ...
        // so every connection pools over the host-proxied socket layer.
