...
/// isolated-worker app; the micro-guest target (ADR 0063) is likewise runtime-less and compiles to a fully static
/// Native-AOT <c>guest</c> for the Unikraft ELF loader, with the guest-side runtime knobs the spike proved baked in.
/// </summary>
public sealed class AotHostAppAssembler
{
    ...
    public AssembledHostApp Assemble(ReadOnlyMemory<byte> executorAssembly, ReadOnlyMemory<byte> executorManifest, string runtimeIdentifier, AotHostAppOptions options)
    {
        ...
        else if (options.Target == ServerlessTarget.MicroGuest)
        {
            files.Add(Text(ProgramFileName, MicroGuestProgram(manifest.EntryType)));
        }
        else
        {
...
""";
    ...
    private static string MicroGuestProgram(string entryType) =>
$$"""
// Generated by the workflow AOT builder (ADR 0055/0063): the baked per-(environment, version) micro-guest. Each snapshot
// restore runs this entry once: argv[1] names the sidecar's invocation endpoint, and the remaining name=value
// arguments seed the deployment's source bindings (ARAZZO_SOURCE__<name>) and this sandbox's guest token
// (ARAZZO_GUEST_TOKEN, minted by the sidecar at evolve), which the guest surface requires on the fetch and the outcome so
// only this sandbox reads its own invocation (P1-10). argv is frozen into the snapshot at evolve, so it carries only
// deployment-stable values — the per-run invocation is fetched over the allowlisted guest network. The executor is the
// signed executor.dll this project references, constructed directly (no loader, no reflection) so the whole guest is
// Native-AOT compiled.
using System;
using System.Collections;
using System.Collections.Generic;
...
try
{
    if (args.Length == 0 || !Uri.TryCreate(args[0], UriKind.Absolute, out Uri? endpoint))
    {
        Console.Error.WriteLine("micro-guest: the first baked argument must be the sidecar's absolute invocation endpoint URL.");
    }
    else
    {
        // The source bindings ride the baked argv the way a cloud function's ride its deploy-time app settings: seed
        // them into the process environment so the shared Bind() below reads both identically.
        for (int i = 1; i < args.Length; i++)
        {
            int separator = args[i].IndexOf('=');
            if (separator > 0)
            {
                Environment.SetEnvironmentVariable(args[i][..separator], args[i][(separator + 1)..]);
            }
        }

        // The guest surface answers only the sandbox whose token the request carries; the token rides the frozen argv
        // with the source bindings. Without it there is nothing to fetch, so this is the second thing checked.
        string? guestToken = Environment.GetEnvironmentVariable("ARAZZO_GUEST_TOKEN");
        if (string.IsNullOrEmpty(guestToken))
        {
            Console.Error.WriteLine("micro-guest: the baked arguments carry no ARAZZO_GUEST_TOKEN; the sidecar mints one at evolve.");
            NativeExit(1);
        }

        // One handler for the guest's whole HTTP surface — the invocation fetch, the checkpoints, and the outcome —
        // so every connection pools over the host-proxied socket layer. The sidecar client carries the guest token;
        // the checkpoint calls go through the handler with their own token, never this one.
        using var handler = new SocketsHttpHandler();
        using var client = new HttpClient(handler, disposeHandler: false);
        client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", guestToken);
...
{
    // The guest console reaches the host's log; the sidecar reports the missing outcome as the invocation's failure.
    Console.Error.WriteLine($"micro-guest: {ex}");
}

// Exit the micro-VM directly: the runtime's shutdown (its thread joins) hangs under the unikernel, and every durable
// effect is already checkpointed, so there is nothing left to flush (ADR 0063).
NativeExit(exitCode);
...
""";
    ...
    private static string Project(string runtimeIdentifier, string executorAssemblyName, AotHostAppOptions options) =>
        ...
        {
            ServerlessTarget.AzureFunctions => AzureProject(runtimeIdentifier, executorAssemblyName, options),
            ServerlessTarget.MicroGuest => MicroGuestProject(runtimeIdentifier, executorAssemblyName, options),
            _ => LambdaProject(runtimeIdentifier, executorAssemblyName, options),
        };
...
""";
    ...
    private static string MicroGuestProject(string runtimeIdentifier, string executorAssemblyName, AotHostAppOptions options) =>
$"""
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <OutputType>Exe</OutputType>
    ...
    <LangVersion>preview</LangVersion>
    <PublishAot>true</PublishAot>
    <!-- The guest binary the micro-guest deployer bakes into the initrd at the kernel's exec path (ADR 0063). -->
    <AssemblyName>guest</AssemblyName>
    <RuntimeIdentifier>{runtimeIdentifier}</RuntimeIdentifier>
    <!-- Fully static: the Unikraft ELF loader runs one self-contained musl executable — no dynamic loader, no shared
         objects — linked with lld as the alpine builder image provides. -->
    <StaticExecutable>true</StaticExecutable>
    <LinkerFlavor>lld</LinkerFlavor>{MicroGuestInstructionSet(runtimeIdentifier)}
    <StripSymbols>true</StripSymbols>
    <InvariantGlobalization>true</InvariantGlobalization>
    <ManagePackageVersionsCentrally>false</ManagePackageVersionsCentrally>
  </PropertyGroup>
  <ItemGroup>
    <!-- The two guest-side knobs the ADR 0063 spike proved: the .NET 10 GC's startup address-space reservation is
         fatal on the no-paging guest kernel, so the heap is hard-limited to fit the micro-VM's memory; and the guest's
         host-proxied socket layer is IPv4-only, so the default dual-mode IPv6 socket is disabled. -->
    <RuntimeHostConfigurationOption Include="System.GC.HeapHardLimit" Value="{options.MicroGuestHeapHardLimitBytes.ToString(CultureInfo.InvariantCulture)}" />
    <RuntimeHostConfigurationOption Include="System.Net.DisableIPv6" Value="true" />
  </ItemGroup>
  <ItemGroup>
    <!-- The vendor-neutral serverless core the baked entry feeds the fetched invocation to. -->
  ...
  <ItemGroup>
    ...
    <!-- Root the executor for ILC: its types are reached only through the entry type constructed in Program, so the
         AOT trimmer would otherwise drop the assembly's metadata and the entry type fails to load at startup. -->
    <TrimmerRootAssembly Include="{executorAssemblyName}" />
  </ItemGroup>
</Project>
""";

    // The conservative x64 ISA baseline for the micro-VM's CPUID (the hypervisor does not expose the host's full
    // feature set to the guest); a non-x64 target takes ILC's own baseline for its architecture.
    private static string MicroGuestInstructionSet(string runtimeIdentifier) =>
        runtimeIdentifier.EndsWith("-x64", StringComparison.Ordinal)
            ? $"{Environment.NewLine}    <IlcInstructionSet>x86-64</IlcInstructionSet>"
            : string.Empty;
...
public sealed record AotHostAppOptions
{
    ...
    /// <summary>Gets the value baked as the guest's <c>System.GC.HeapHardLimit</c>, in bytes (micro-guest target). The
    /// .NET 10 GC's startup address-space reservation is fatal on the no-paging guest kernel, so the heap is hard-limited
    /// to fit inside the micro-VM's memory (ADR 0063); size it comfortably below the sandbox memory the sidecar boots.
    /// Defaults to 32 MiB (a 64 MiB micro-VM).</summary>
    public long MicroGuestHeapHardLimitBytes { get; init; } = 32 * 1024 * 1024;
}