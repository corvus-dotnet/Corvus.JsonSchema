...
/// <summary>
/// Request type for the GetNativeBuild operation.
/// </summary>
...
public readonly struct GetNativeBuildRequest : IApiRequest<GetNativeBuildRequest>
{
    ...
    /// </summary>
    public Corvus.Text.Json.Arazzo.Durability.ControlPlane.Cli.Client.Models.JsonString RuntimeIdentifier { get; init; }

    /// <summary>
    /// Initializes a new instance of the <see cref="GetNativeBuildRequest"/> struct.
    /// </summary>
    ...
    /// <inheritdoc/>
    public static ReadOnlySpan<byte> PathTemplateUtf8 => "/catalog/{baseWorkflowId}/versions/{versionNumber}/nativeBuilds/{environment}/{runtimeIdentifier}"u8;
    ...
    public void WriteResolvedPath(IBufferWriter<byte> writer)
    {
        ...
        writer.Write("/"u8);
        using UnescapedUtf8JsonString utf8RuntimeIdentifier = ((JsonElement)this.RuntimeIdentifier).GetUtf8String();
        Span<byte> escRuntimeIdentifier = stackalloc byte[utf8RuntimeIdentifier.Span.Length * 3];
        if (Utf8Uri.TryEscapeDataString(utf8RuntimeIdentifier.Span, escRuntimeIdentifier, out int ewRuntimeIdentifier))
        {
            writer.Write(escRuntimeIdentifier[..ewRuntimeIdentifier]);
        }
    }
    ...
    public void Validate(ValidationMode mode = ValidationMode.Basic)
    {
        ...
        else if (mode == ValidationMode.Detailed)
        {
            ...
            using JsonSchemaResultsCollector collectorRuntimeIdentifier = JsonSchemaResultsCollector.Create(JsonSchemaResultsLevel.Detailed);
            if (!this.RuntimeIdentifier.EvaluateSchema(collectorRuntimeIdentifier))
            {
                ThrowHelper.ThrowRequestParameterValidationFailed("runtimeIdentifier", SchemaValidationDetail.FormatResults(collectorRuntimeIdentifier));
            }
        ...
        else
        {
            ...
            if (!this.RuntimeIdentifier.EvaluateSchema())
            {
                ThrowHelper.ThrowRequestParameterValidationFailed("runtimeIdentifier");
            }
