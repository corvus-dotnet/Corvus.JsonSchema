...
public readonly partial struct GitHubCommitList
{
    ...
    public static ParsedJsonDocument<GitHubCommitList> Create(
        scoped in Source value, int initialCapacity = 30)
    {
        ...
        try
        {
            ...
            Debug.Assert(cvb.MemberCount == 1);
            ((IMutableJsonDocument)documentBuilder).SetAndDispose(ref cvb);
            return documentBuilder.ToParsedJsonDocument<GitHubCommitList>();
        }
        finally
        {
    ...
    /// <param name="initialValueBufferSize">The initial size in bytes of the value buffer.</param>
    /// <returns>A <see cref="ParsedJsonDocument{T}"/> containing the given value. The caller must dispose it.</returns>
    public static ParsedJsonDocument<GitHubCommitList> Create<TContext>(
    ...
    public static ParsedJsonDocument<GitHubCommitList> Create(in Corvus.Text.Json.Arazzo.Durability.ControlPlane.Cli.Client.Models.GitHubCommitList.GitHubCommitArray.Source commits, in Corvus.Text.Json.Arazzo.Durability.ControlPlane.Cli.Client.Models.JsonBoolean.Source hasMore = default, int initialCapacity = 30)
    {
        ParsedJsonDocumentBuilder documentBuilder = ParsedJsonDocumentBuilder.Rent();
        try
        {
    ...
    /// <param name="initialCapacity">The (optional) estimate of the capacity to reserve for the document.</param>
    /// <returns>A <see cref="ParsedJsonDocument{T}"/> containing the given property values. The caller must dispose it.</returns>
    public static ParsedJsonDocument<GitHubCommitList> Create<TContext>(in TContext context, in Corvus.Text.Json.Arazzo.Durability.ControlPlane.Cli.Client.Models.GitHubCommitList.GitHubCommitArray.Source<TContext> commits, in Corvus.Text.Json.Arazzo.Durability.ControlPlane.Cli.Client.Models.JsonBoolean.Source hasMore = default, int initialCapacity = 30)
        #if NET9_0_OR_GREATER
        where TContext : allows ref struct
        #endif
    {
