...
#endif
{
    ...
    /// </para>
    /// <para>
    /// The fault&#39;s error type. The platform records these fixed values itself, and any other value is a step&#39;s own failure, recorded as it happened: `budget-fuel` (the run made as many step attempts as its budget&#39;s max steps allows), `budget-deadline` (the run outlived its budget&#39;s wall clock), `budget-depth` (the run nested sub-workflows past its budget&#39;s depth limit), `executor-unhandled` (the executor failed in a way nothing in the workflow handled; the failure is in the executor&#39;s trace and not on the run), `executor-unresolvable` (the workflow version&#39;s executor was refused: missing, not runnable, or failed verification), `transport-unbound` (a source the workflow calls has no usable binding in the run&#39;s environment). A run faulted on its budget is resumed by raising its environment&#39;s limit and resuming, which re-budgets the run (see `rebudget` on the run), and is refused with 409 `budget-exhausted` while the run is still outside the budget it would be given; the other three are resumed once their cause is fixed. Any run can instead be re-run (`rerunRun`).
    /// </para>
    /// </remarks>
    public Corvus.Text.Json.Arazzo.Durability.ControlPlane.Cli.Client.Models.JsonString Error
    {
        get
        {
            if (_parent.TryGetNamedPropertyValue(_idx, JsonPropertyNames.ErrorUtf8, out Corvus.Text.Json.Arazzo.Durability.ControlPlane.Cli.Client.Models.JsonString value))
            {
                return value;
            }
