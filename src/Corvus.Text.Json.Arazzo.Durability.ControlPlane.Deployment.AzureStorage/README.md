# Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.AzureStorage

Azure Storage deployment provisioning for an Arazzo durability control plane. It provisions a deployment in
...
```csharp
await AzureStorageControlPlaneDeployment.ProvisionAsync(connectionString, options);
```
...
- **Coupled to Azure Storage.** Schema creation is inherently backend-specific — each backend's `PrepareAsync`
  has a different signature — so an Azure Storage deployment owns the Azure Storage schema. Other backends get their
...
- **Identity-provider agnostic.** This package never references Keycloak, OIDC, or any ASP.NET wiring. It
  ...
  (Keycloak, Entra ID, Auth0, …) separately in its composition root. Azure Storage **and** *whatever IdP*.
...
| `…ControlPlane.Deployment.AzureStorage` (this) | Azure Storage schema for the full control-plane store set + the bootstrap above, in one call. |
