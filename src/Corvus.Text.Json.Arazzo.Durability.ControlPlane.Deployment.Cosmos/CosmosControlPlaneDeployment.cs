// <copyright file="CosmosControlPlaneDeployment.cs" company="Endjin Limited">
...
/// <summary>
/// Provisions an Arazzo durability control-plane deployment backed by Azure Cosmos DB: it creates the schema for every store
...
/// <remarks>
/// <para>
/// This type is coupled to Azure Cosmos DB because schema creation is inherently backend-specific — each backend's
...
/// </para>
/// </remarks>
public static class CosmosControlPlaneDeployment
{
    /// <summary>
    ...
    /// </summary>
    /// <param name="connectionString">The Azure Cosmos DB connection string for the shared control-plane database.</param>
    ...
    public static async ValueTask ProvisionSchemaAsync(string connectionString, string databaseName = "arazzo", CancellationToken cancellationToken = default)
    {
        ...
        await CosmosDraftRunStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosDraftRunTraceStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);

        // Environments + working copies + workflow administrators.
        await CosmosEnvironmentStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosWorkspaceWorkflowStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosWorkflowAdministratorStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);

        // Security policy + access requests (§16.5).
        await CosmosSecurityPolicyStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosAccessRequestStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        ...
        await CosmosAvailabilityStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosAvailabilityRequestStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosSourceStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosEnvironmentAdministratorStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
        await CosmosObservedIdentityStore.PrepareAsync(connectionString, databaseName, cancellationToken).ConfigureAwait(false);
    }
}