# Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.Cosmos

Azure Cosmos DB deployment provisioning for an Arazzo durability control plane. It provisions a deployment in
...
```csharp
await CosmosControlPlaneDeployment.ProvisionAsync(connectionString, options);
```

The optional `databaseName` parameter (defaults to `"arazzo"`) selects the Cosmos database to provision into.
...
- **Coupled to Azure Cosmos DB.** Schema creation is inherently backend-specific — each backend's `PrepareAsync`
  has a different signature — so an Azure Cosmos DB deployment owns the Azure Cosmos DB schema. Other backends get their
...
- **Identity-provider agnostic.** This package never references Keycloak, OIDC, or any ASP.NET wiring. It
  ...
  (Keycloak, Entra ID, Auth0, …) separately in its composition root. Azure Cosmos DB **and** *whatever IdP*.
...
| `…ControlPlane.Deployment.Cosmos` (this) | Azure Cosmos DB schema for the full control-plane store set + the bootstrap above, in one call. |
| host composition | The runtime store graph, the HTTP surface, and the identity-provider wiring. |

`ProvisionSchemaAsync(connectionString)` is available on its own for hosts whose composition interleaves
