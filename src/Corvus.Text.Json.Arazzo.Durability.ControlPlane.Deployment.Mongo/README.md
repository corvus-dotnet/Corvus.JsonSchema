# Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.Mongo

MongoDB deployment provisioning for an Arazzo durability control plane. It provisions a deployment in
...
```csharp
await MongoControlPlaneDeployment.ProvisionAsync(connectionString, options);
```
...
- **Coupled to MongoDB.** Schema creation is inherently backend-specific — each backend's `PrepareAsync`
  has a different signature — so a MongoDB deployment owns the MongoDB schema. Other backends get their
...
- **Identity-provider agnostic.** This package never references Keycloak, OIDC, or any ASP.NET wiring. It
  ...
  (Keycloak, Entra ID, Auth0, …) separately in its composition root. MongoDB **and** *whatever IdP*.
- **Idempotent.** Every operation is index/collection setup or an upsert, so it is safe to run on every
  startup.
...
| `…ControlPlane.Deployment.Mongo` (this) | MongoDB schema for the full control-plane store set + the bootstrap above, in one call. |
