# Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.MySql

MySQL deployment provisioning for an Arazzo durability control plane. It provisions a deployment in
...
```csharp
await MySqlControlPlaneDeployment.ProvisionAsync(connectionString, options);
```
...
- **Coupled to MySQL.** Schema creation is inherently backend-specific — each backend's `PrepareAsync`
  has a different signature — so a MySQL deployment owns the MySQL schema. Other backends get their
...
- **Identity-provider agnostic.** This package never references Keycloak, OIDC, or any ASP.NET wiring. It
  ...
  (Keycloak, Entra ID, Auth0, …) separately in its composition root. MySQL **and** *whatever IdP*.
...
| `…ControlPlane.Deployment.MySql` (this) | MySQL schema for the full control-plane store set + the bootstrap above, in one call. |
