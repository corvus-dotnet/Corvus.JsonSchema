# Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.SqlServer

SQL Server deployment provisioning for an Arazzo durability control plane. It provisions a deployment in
...
```csharp
await SqlServerControlPlaneDeployment.ProvisionAsync(connectionString, options);
```
...
- **Coupled to SQL Server.** Schema creation is inherently backend-specific — each backend's `PrepareAsync`
  has a different signature — so a SQL Server deployment owns the SQL Server schema. Other backends get their
...
- **Identity-provider agnostic.** This package never references Keycloak, OIDC, or any ASP.NET wiring. It
  ...
  (Keycloak, Entra ID, Auth0, …) separately in its composition root. SQL Server **and** *whatever IdP*.
- **Idempotent.** Every operation is idempotent schema creation / upsert, so it is safe to run on every
  startup.
...
| `…ControlPlane.Deployment.SqlServer` (this) | SQL Server schema for the full control-plane store set + the bootstrap above, in one call. |
