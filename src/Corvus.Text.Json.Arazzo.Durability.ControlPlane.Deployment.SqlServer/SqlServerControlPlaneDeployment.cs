// <copyright file="SqlServerControlPlaneDeployment.cs" company="Endjin Limited">
...
namespace Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.SqlServer;

/// <summary>
/// Provisions an Arazzo durability control-plane deployment backed by SQL Server: it creates the schema for every store
...
/// <remarks>
/// <para>
/// This type is coupled to SQL Server because schema creation is inherently backend-specific — each backend's
...
/// </para>
/// </remarks>
public static class SqlServerControlPlaneDeployment
{
    /// <summary>
    ...
    /// </summary>
    /// <param name="connectionString">The SQL Server connection string for the shared control-plane database.</param>
    ...
    public static async ValueTask ProvisionAsync(string connectionString, DeploymentBootstrapOptions options, CancellationToken cancellationToken = default, GovernanceAuditor? auditor = null)
    {
        ...
        await using SqlServerSecurityPolicyStore securityStore = await SqlServerSecurityPolicyStore.ConnectAsync(connectionString, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Creates the schema for every store the Arazzo control plane owns (idempotent schema creation).
    ...
    public static async ValueTask ProvisionSchemaAsync(string connectionString, CancellationToken cancellationToken = default)
    {
        ...
        await SqlServerDraftRunStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerDraftRunTraceStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);

        // Environments + working copies + workflow administrators.
        await SqlServerEnvironmentStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerWorkspaceWorkflowStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerWorkflowAdministratorStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);

        // Security policy + access requests (§16.5).
        await SqlServerSecurityPolicyStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerAccessRequestStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        ...
        await SqlServerAvailabilityStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerAvailabilityRequestStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerSourceStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerEnvironmentAdministratorStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqlServerObservedIdentityStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
    }
}