# Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.Sqlite

SQLite deployment provisioning for an Arazzo durability control plane. It provisions a deployment in
...
```csharp
await SqliteControlPlaneDeployment.ProvisionAsync(connectionString, options);
```
...
- **Coupled to SQLite.** Schema creation is inherently backend-specific — each backend's `PrepareAsync`
  has a different signature — so a SQLite deployment owns the SQLite schema. Other backends get their
...
- **Identity-provider agnostic.** This package never references Keycloak, OIDC, or any ASP.NET wiring. It
  ...
  (Keycloak, Entra ID, Auth0, …) separately in its composition root. SQLite **and** *whatever IdP*.
- **Idempotent.** Every operation is `CREATE TABLE IF NOT EXISTS` / upsert, so it is safe to run on every
  startup.
...
| `…ControlPlane.Deployment.Sqlite` (this) | SQLite schema for the full control-plane store set + the bootstrap above, in one call. |
