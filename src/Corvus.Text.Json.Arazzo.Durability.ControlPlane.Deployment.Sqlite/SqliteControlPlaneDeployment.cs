// <copyright file="SqliteControlPlaneDeployment.cs" company="Endjin Limited">
...
namespace Corvus.Text.Json.Arazzo.Durability.ControlPlane.Deployment.Sqlite;

/// <summary>
/// Provisions an Arazzo durability control-plane deployment backed by SQLite: it creates the schema for every store
...
/// <remarks>
/// <para>
/// This type is coupled to SQLite because schema creation is inherently backend-specific — each backend's
...
/// </para>
/// </remarks>
public static class SqliteControlPlaneDeployment
{
    /// <summary>
    ...
    /// </summary>
    /// <param name="connectionString">The SQLite connection string for the shared control-plane database.</param>
    ...
    public static async ValueTask ProvisionAsync(string connectionString, DeploymentBootstrapOptions options, CancellationToken cancellationToken = default)
    {
        ...
        await using SqliteSecurityPolicyStore securityStore = await SqliteSecurityPolicyStore.ConnectAsync(connectionString, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public static async ValueTask ProvisionSchemaAsync(string connectionString, CancellationToken cancellationToken = default)
    {
        ...
        await SqliteDraftRunStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteDraftRunTraceStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);

        // Environments + working copies + workflow administrators.
        await SqliteEnvironmentStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteWorkspaceWorkflowStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteWorkflowAdministratorStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);

        // Security policy + access requests (§16.5).
        await SqliteSecurityPolicyStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteAccessRequestStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        ...
        await SqliteAvailabilityStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteAvailabilityRequestStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteSourceStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteEnvironmentAdministratorStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
        await SqliteObservedIdentityStore.PrepareAsync(connectionString, cancellationToken).ConfigureAwait(false);
    }
}