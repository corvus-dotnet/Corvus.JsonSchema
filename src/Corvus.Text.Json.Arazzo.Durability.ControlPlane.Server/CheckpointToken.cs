...
/// the run in the request URL. It is a symmetric HMAC over the run's full <c>(environment, runId)</c> address
/// (ADR 0065 decision 9) and an expiry, so it needs no cloud identity provider, it is opaque to the function (which
/// never interprets it), and a leaked token authenticates only its own run at its own address and only until it
/// expires. The token is <c>{expiryUnixSeconds}.{base64url(HMAC-SHA256(secret, "environment:runId:expiry"))}</c> —
/// the address is bound by the signature but never transmitted, since the checkpoint endpoint already knows it from
/// the URL.
/// </summary>
/// <remarks>
/// <para>
/// The signed message is framed by its grammar rather than by length prefixes, and that is sound rather than merely
/// untroublesome so far: the environment-name grammar admits no colon, so the first colon ends it, and the expiry is
/// a digits-only suffix after the final colon, so no other triple of environment, run id and expiry can produce an
/// identical message however the run id is spelled. A run id containing the separator therefore borrows nothing,
...
public static class CheckpointToken
{
    ...
    /// <param name="address">The <c>(environment, runId)</c> address of the run the token authorises checkpoints for (ADR 0065 decision 9).</param>
    ...
    public static string Issue(ReadOnlySpan<byte> secret, in WorkflowRunAddress address, DateTimeOffset expiry)
    {
        ArgumentException.ThrowIfNullOrEmpty(address.RunId.Value);
        ...
        if (!TrySign(secret, address, expiryUnixSeconds, signature, out int written))
        {
            throw new ArgumentException("The checkpoint token's signature could not be computed.", nameof(address));
        }
    ...
    /// <paramref name="address"/>, and not yet expired at <paramref name="now"/>.
    /// </summary>
    ...
    /// <param name="address">The run's <c>(environment, runId)</c> address from the request URL the token must authorise.</param>
    /// <param name="now">The current time, for the expiry check.</param>
    /// <returns><see langword="true"/> if the token authorises checkpoints for the run at that address.</returns>
    public static bool TryValidate(ReadOnlySpan<byte> secret, string? token, in WorkflowRunAddress address, DateTimeOffset now)
    {
        if (secret.Length < MinimumSecretBytes || string.IsNullOrEmpty(token) || string.IsNullOrEmpty(address.RunId.Value))
        {
            return false;
        }
        ...
        // Recompute the signature over the address in the URL and the token's expiry, and compare in constant time. A
        // token minted for another run — or for the same run id at another environment — yields a different signature,
        // so it does not validate here.
        Span<char> expected = stackalloc char[SignatureChars];
        if (!TrySign(secret, address, expiryUnixSeconds, expected, out int written))
        {
            return false;
        }
    ...
    private static bool TrySign(ReadOnlySpan<byte> secret, in WorkflowRunAddress address, long expiryUnixSeconds, Span<char> destination, out int written)
    {
        int maximumMessage = Encoding.UTF8.GetMaxByteCount(address.Environment.Length + 1 + address.RunId.Value.Length) + 1 + CanonicalExpiryChars;
        ...
        try
        {
            ...
            int position = Encoding.UTF8.GetBytes(address.Environment, message);
            message[position++] = (byte)':';
            position += Encoding.UTF8.GetBytes(address.RunId.Value, message[position..]);
            message[position++] = (byte)':';
