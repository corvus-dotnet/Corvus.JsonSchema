...
/// <summary>
/// Result type for the GetAccessGrantsReach operation.
/// </summary>
public readonly struct GetAccessGrantsReachResult
{
    private GetAccessGrantsReachResult(int statusCode, JsonElement body = default, string? contentType = null)
    {
    ...
    /// <returns>A <see cref="GetAccessGrantsReachResult"/> with status 200.</returns>
    public static GetAccessGrantsReachResult Ok(Corvus.Text.Json.Arazzo.Durability.ControlPlane.Server.Models.AccessGrantsReachPage.Source body, JsonWorkspace workspace) => new(200, Corvus.Text.Json.Arazzo.Durability.ControlPlane.Server.Models.AccessGrantsReachPage.CreateBuilder(workspace, body, 30).RootElement, "application/json");
    /// <summary>
    ...
    /// <param name="workspace">The workspace for building the response value.</param>
    /// <returns>A <see cref="GetAccessGrantsReachResult"/> with status 200.</returns>
    public static GetAccessGrantsReachResult Ok<TContext>(Corvus.Text.Json.Arazzo.Durability.ControlPlane.Server.Models.AccessGrantsReachPage.Source<TContext> body, JsonWorkspace workspace)
    #if NET9_0_OR_GREATER
        where TContext : allows ref struct
    #endif
        => new(200, Corvus.Text.Json.Arazzo.Durability.ControlPlane.Server.Models.AccessGrantsReachPage.CreateBuilder(workspace, in body, 30).RootElement, "application/json");
    ...
    /// <param name="workspace">The workspace for building the response value.</param>
    /// <returns>A <see cref="GetAccessGrantsReachResult"/> with status 400.</returns>
    public static GetAccessGrantsReachResult BadRequest<TContext>(Corvus.Text.Json.Arazzo.Durability.ControlPlane.Server.Models.ProblemDetails.Source<TContext> body, JsonWorkspace workspace)
    #if NET9_0_OR_GREATER
        where TContext : allows ref struct
    #endif
    ...
    public bool ValidateBody()
    {
        ...
        {
            200 => Corvus.Text.Json.Arazzo.Durability.ControlPlane.Server.Models.AccessGrantsReachPage.From(this.Body).EvaluateSchema(),
