...
/// <summary>
/// Result type for the GetCatalogPackage operation.
/// </summary>
public readonly struct GetCatalogPackageResult
{
    private GetCatalogPackageResult(int statusCode, JsonElement body = default, string? contentType = null, bool hasBinaryBody = false, Func<Stream, CancellationToken, ValueTask>? binaryWriter = null)
    {
    ...
    /// <returns>A <see cref="GetCatalogPackageResult"/> with status 200.</returns>
    public static GetCatalogPackageResult Ok(ReadOnlyMemory<byte> body, string? contentType = "application/octet-stream") => new(200, default, contentType, hasBinaryBody: true, binaryWriter: (stream, cancellationToken) => stream.WriteAsync(body, cancellationToken));
    ...
    /// <param name="contentType">The content type for the response body.</param>
    /// <returns>A <see cref="GetCatalogPackageResult"/> with status 200.</returns>
    public static GetCatalogPackageResult Ok(Func<Stream, CancellationToken, ValueTask> writeBody, string? contentType = "application/octet-stream") => new(200, default, contentType, hasBinaryBody: true, binaryWriter: writeBody);
    ...
    /// <param name="workspace">The workspace for building the response value.</param>
    /// <returns>A <see cref="GetCatalogPackageResult"/> with status 404.</returns>
    public static GetCatalogPackageResult NotFound<TContext>(Corvus.Text.Json.Arazzo.Durability.ControlPlane.Server.Models.ProblemDetails.Source<TContext> body, JsonWorkspace workspace)
