...
#endif
{
    ...
    internal JsonIri(IJsonDocument parent, int idx)
    {
        Debug.Assert(idx >= 0);
        _parent = parent;
        _idx = idx;
    ...
    public bool TryGetValue(out Utf8IriValue value) { CheckValidInstance(); return Utf8IriValue.TryGetValue(_parent, _idx, out value); }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator Utf8IriValue(JsonIri value) => Utf8IriValue.TryGetValue(value._parent, value._idx, out Utf8IriValue result) ? result : throw new FormatException();

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator string(JsonIri value) => value._parent.GetString(value._idx, JsonTokenType.String) ?? throw new FormatException();
    ...
    /// <c>True</c> if the values are equal.
    /// </returns>
    public static bool operator ==(in JsonIri left, in JsonElement right)
    {
        return left.Equals(right);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator JsonElement(JsonIri instance)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator JsonIri(JsonElement instance)
    {
        return JsonIri.From(instance);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static JsonIri From<T>(in T instance)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static JsonIri ParseValue(ReadOnlySpan<byte> utf8Json, JsonDocumentOptions options = default)
    {
        #pragma warning disable CS0618 // Type or member is obsolete
        return JsonElementHelpers.ParseValue<JsonIri>(utf8Json, options);
    ...
    [Obsolete("Use ParsedJsonDocument<T>.Parse() for pooled-memory parsing, or Clone() for a standalone copy. ParseValue allocates without pooling.")]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static JsonIri ParseValue(ReadOnlySpan<char> json, JsonDocumentOptions options = default)
    {
        #pragma warning disable CS0618 // Type or member is obsolete
        return JsonElementHelpers.ParseValue<JsonIri>(json, options);
        #pragma warning restore CS0618
    }
    ...
    [Obsolete("Use ParsedJsonDocument<T>.Parse() for pooled-memory parsing, or Clone() for a standalone copy. ParseValue allocates without pooling.")]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static JsonIri ParseValue(string json, JsonDocumentOptions options = default)
    {
        #pragma warning disable CS0618 // Type or member is obsolete
        return JsonElementHelpers.ParseValue<JsonIri>(json, options);
        #pragma warning restore CS0618
    ...
    {
        #pragma warning disable CS0618 // Type or member is obsolete
        return JsonElementHelpers.ParseValue<JsonIri>(ref reader);
        #pragma warning restore CS0618
    }
    ...
    ///   A value could not be read from the reader.
    /// </exception>
    public static bool TryParseValue(ref Utf8JsonReader reader, out JsonIri? result)
    {
        return JsonElementHelpers.TryParseValue<JsonIri>(ref reader, out result);
    }
    ...
    {
        return
            (obj is IJsonElement value && Equals(new JsonIri(value.ParentDocument, value.ParentDocumentIndex))) ||
    ...
    /// the IRI with percent-encoded sequences decoded for human readability.
    /// </para>
    /// <para>
    /// When <paramref name="format"/> is <c>"c"</c> or <c>"C"</c>, produces the canonical form of
    /// the IRI with all required characters percent-encoded.
    /// </para>
    /// <para>
    ...
    {
        CheckValidInstance();
        if (!string.IsNullOrEmpty(format) && (format is "g" or "G" or "c" or "C") && TryGetValue(out Utf8IriValue uriValue))
        {
            using (uriValue)
            {
                bool isDisplay = format is "g" or "G";
                Utf8Iri uri = uriValue.Iri;
                if (global::Corvus.Text.Json.Internal.JsonElementHelpers.TryFormatIri(uri, isDisplay, out string? result))
                    return result;
            }
        }
    ...
    public bool TryFormat(Span<char> destination, out int charsWritten, ReadOnlySpan<char> format, IFormatProvider? provider)
    {
        ...
        if (!format.IsEmpty && format.Length == 1)
        {
            ...
            if ((isDisplay || isCanonical) && TryGetValue(out Utf8IriValue uriValue))
            {
                using (uriValue)
                {
                    Utf8Iri uri = uriValue.Iri;
                    return global::Corvus.Text.Json.Internal.JsonElementHelpers.TryFormatIri(uri, isDisplay, destination, out charsWritten);
                }
            }
        }
    ...
    /// <para>
    /// When <paramref name="format"/> is <c>"g"</c> or <c>"G"</c>, writes the display form of
    /// the IRI as UTF-8 with percent-encoded sequences decoded for human readability.
    /// </para>
    /// <para>
    /// When <paramref name="format"/> is <c>"c"</c> or <c>"C"</c>, writes the canonical form of
    /// the IRI as UTF-8 with all required characters percent-encoded.
    /// </para>
    /// <para>
    /// When <paramref name="format"/> is empty, or an unrecognised format string is provided,
...
#if NET
    static JsonIri IJsonElement<JsonIri>.CreateInstance(IJsonDocument parentDocument, int parentDocumentIndex) => new(parentDocument, parentDocumentIndex);
#endif

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    private string DebuggerDisplay => $"JsonIri: ValueKind = {ValueKind} : \"{ToString()}\"";
    ...
    /// <summary>
    /// Gets a <see cref="JsonIri"/> which can be safely stored beyond the lifetime of the
    /// original document.
    /// </summary>
    /// <returns>
    /// A <see cref="JsonIri"/> which can be safely stored beyond the lifetime of the
    /// original document.
    /// </returns>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    public JsonIri Clone()
    {
        CheckValidInstance();
        return _parent.CloneElement<JsonIri>(_idx);
    }
    ...
    /// </summary>
    /// <returns>
    /// An immutable <see cref="JsonIri"/> that lives for the lifetime of its
