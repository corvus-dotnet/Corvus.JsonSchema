...
/// </summary>
public sealed class ReceiveAccessDecisionWithDeliveryContextConsumer : IAsyncDisposable
{
    private readonly IMessageDeliveryContextTransport transport;
    private readonly IReceiveAccessDecisionWithDeliveryContextHandler handler;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReceiveAccessDecisionWithDeliveryContextConsumer"/> class.
    /// </summary>
    ...
    public ReceiveAccessDecisionWithDeliveryContextConsumer(IMessageDeliveryContextTransport transport, IReceiveAccessDecisionWithDeliveryContextHandler handler, ValidationMode validationMode = ValidationMode.Basic, IMessageErrorPolicy? errorPolicy = null, IMessageAuthenticationProvider? authProvider = null)
    {
    ...
    public async ValueTask StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await this.transport.SubscribeWithDeliveryContextAsync<Corvus.Text.Json.Arazzo.Durability.ControlPlane.SystemWorkflows.Models.AccessDecisionPayload>(ChannelAddressUtf8, this.HandleMessageAsync, cancellationToken).ConfigureAwait(false);
        }
        catch
        {
    ...
    private async ValueTask HandleMessageAsync(Corvus.Text.Json.Arazzo.Durability.ControlPlane.SystemWorkflows.Models.AccessDecisionPayload payload, MessageDeliveryContext deliveryContext, CancellationToken cancellationToken)
    {
        ...
        if (failure is null)
        {
            try
            {
                await this.handler.HandleAccessDecisionAsync(payload, deliveryContext, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                failure = ex;
