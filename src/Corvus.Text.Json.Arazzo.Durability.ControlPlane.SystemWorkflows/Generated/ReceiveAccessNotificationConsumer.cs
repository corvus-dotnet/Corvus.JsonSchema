...
/// </summary>
public sealed class ReceiveAccessNotificationConsumer : IAsyncDisposable
{
    private readonly IMessageTransport transport;
    private readonly IReceiveAccessNotificationHandler handler;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReceiveAccessNotificationConsumer"/> class.
    /// </summary>
    ...
    public ReceiveAccessNotificationConsumer(IMessageTransport transport, IReceiveAccessNotificationHandler handler, ValidationMode validationMode = ValidationMode.Basic, IMessageErrorPolicy? errorPolicy = null, IMessageAuthenticationProvider? authProvider = null)
    {
    ...
    public async ValueTask StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await this.transport.SubscribeAsync<Corvus.Text.Json.Arazzo.Durability.ControlPlane.SystemWorkflows.Models.AccessNotificationPayload>(ChannelAddressUtf8, this.HandleMessageAsync, cancellationToken).ConfigureAwait(false);
        }
        catch
        {
    ...
    private async ValueTask HandleMessageAsync(Corvus.Text.Json.Arazzo.Durability.ControlPlane.SystemWorkflows.Models.AccessNotificationPayload payload, Corvus.Text.Json.JsonElement headers, CancellationToken cancellationToken)
    {
        ...
        if (failure is null)
        {
            try
            {
                await this.handler.HandleAccessNotificationAsync(payload, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                failure = ex;
