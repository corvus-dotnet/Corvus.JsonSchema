...
public sealed class CosmosEnvironmentAdministratorStore : IEnvironmentAdministratorStore, IAsyncDisposable
{
    ...
    public async ValueTask<EnvironmentAdministeredPage> ListAdministeredAsync(IReadOnlyList<string> adminDigests, int limit, global::Corvus.Text.Json.Arazzo.Durability.JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        // Cross-partition keyset page under membership (§16.5.4): an environment whose mirrored adminDigests contains ANY of
        // the caller's subset digests. A document matches once regardless of how many digests hit, so no DISTINCT. The lazy
        // stream iterator is drained only until one row beyond the page, so the read is bounded.
        ...
        string where = after is null
            ? $" WHERE ({contains})"
            : $" WHERE ({contains}) AND c.environmentName > @after";
