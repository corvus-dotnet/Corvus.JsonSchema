...
/// <summary>
/// Centralized exception-throwing helpers for the Cosmos DB durability backend.
/// </summary>
/// <remarks>
/// <para>
/// Guard-position helpers are <c>Throw*</c> methods marked <see cref="DoesNotReturnAttribute"/> so the JIT can optimize call-site code after a throw; expression-position helpers are <c>Get*Exception</c> factories the caller throws (so a value-producing path stays well-typed). All exception messages come from the embedded <c>Resources/Strings.resx</c> resource file via <c>SR</c>.
/// </para>
/// </remarks>
internal static class ThrowHelper
{
    ...
    /// <summary>Throws when a runner-authorization create lost a race but the winning record could not be re-read.</summary>
    [DoesNotReturn]
    [StackTraceHidden]
    public static void ThrowRunnerAuthorizationCreateConflictNotReread()
        => throw new InvalidOperationException(SR.RunnerAuthorizationCreateConflictNotReread);

    /// <summary>Creates the exception for an availability-entry create that lost a race but whose winning entry could not be re-read, for the caller to throw.</summary>
    /// <returns>The exception to throw.</returns>
    public static InvalidOperationException GetAvailabilityCreateConflictNotRereadException()
        => new(SR.AvailabilityCreateConflictNotReread);
