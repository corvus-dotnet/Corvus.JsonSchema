// <copyright file="MongoSourceStore.cs" company="Endjin Limited">
...
/// <summary>
/// A MongoDB-backed <see cref="ISourceStore"/> (design §7.6): registered sources persisted as documents. Each
/// source is stored as its <see cref="RegisteredSource"/> document in a binary <c>doc</c> field, keyed by a composite
/// <c>_id</c> (<c>{ n: name, t: tags-discriminator }</c>) so reach-isolated sources that share a name coexist while
...
/// concurrent.
/// </remarks>
public sealed class MongoSourceStore : ISourceStore, IAsyncDisposable
{
    ...
    private readonly IMongoCollection<BsonDocument> sources;

    private MongoSourceStore(IMongoClient client, string databaseName, bool ownsClient, TimeProvider timeProvider)
    {
        this.client = client;
        this.ownsClient = ownsClient;
        this.timeProvider = timeProvider;
        IMongoDatabase database = client.GetDatabase(databaseName);
        this.sources = database.GetCollection<BsonDocument>("sources");
    }
    ...
    public static async ValueTask PrepareAsync(string connectionString, string databaseName = "arazzo", CancellationToken cancellationToken = default)
    {
        ...
        await using var store = new MongoSourceStore(client, databaseName, ownsClient: true, TimeProvider.System);
    ...
    public static async ValueTask PrepareAsync(IMongoClient client, string databaseName = "arazzo", CancellationToken cancellationToken = default)
    {
        ...
        await using var store = new MongoSourceStore(client, databaseName, ownsClient: false, TimeProvider.System);
        await store.EnsureIndexesAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    public static ValueTask<MongoSourceStore> ConnectAsync(string connectionString, string databaseName = "arazzo", TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connectionString);
        cancellationToken.ThrowIfCancellationRequested();
        var client = new MongoClient(connectionString);
        return new ValueTask<MongoSourceStore>(new MongoSourceStore(client, databaseName, ownsClient: true, timeProvider ?? TimeProvider.System));
    }
    ...
    public static ValueTask<MongoSourceStore> ConnectAsync(IMongoClient client, string databaseName = "arazzo", TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(client);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<MongoSourceStore>(new MongoSourceStore(client, databaseName, ownsClient: false, timeProvider ?? TimeProvider.System));
    }
    ...
    public async ValueTask<ParsedJsonDocument<RegisteredSource>> AddAsync(RegisteredSource draft, string actor, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await this.sources.InsertOneAsync(document, options: null, cancellationToken).ConfigureAwait(false);
        }
    ...
    public async ValueTask<SourcePage> ListAsync(AccessContext context, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // row is the lookahead that signals a continuation token — the row key of the last *included* source.
            using IAsyncCursor<BsonDocument> mongoCursor = await this.sources.Find(filter).Sort(sort).Limit(pageSize + 1).ToCursorAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<ParsedJsonDocument<RegisteredSource>?> UpdateAsync(string name, RegisteredSource draft, WorkflowEtag expectedEtag, string actor, AccessContext context, CancellationToken cancellationToken)
    {
        ...
        UpdateDefinition<BsonDocument> update = Builders<BsonDocument>.Update
            .Set("etag", SourceSerialization.EtagOf(json).Value!)
        ...
        await this.sources.UpdateOneAsync(
    ...
    public async ValueTask<bool> DeleteAsync(string name, WorkflowEtag expectedEtag, AccessContext context, CancellationToken cancellationToken)
    {
        ...
        await this.sources.DeleteOneAsync(
            Builders<BsonDocument>.Filter.Eq("_id", Key(name, tags!)),
            cancellationToken).ConfigureAwait(false);
        return true;
    }
    ...
    public async ValueTask<(int Count, bool Capped)> CountAsync(AccessContext context, int cap, CancellationToken cancellationToken)
    {
        ...
        long total = await this.sources.CountDocumentsAsync(
            filter,
    ...
    // string id — the unique _id then rejects an exact duplicate source on insert.
    ...
    {
        List<BsonDocument> documents = await this.sources
    ...
    private async ValueTask EnsureIndexesAsync(CancellationToken cancellationToken)
    {
        ...
        await this.sources.Indexes.CreateManyAsync([byName, bySecurityTags], cancellationToken).ConfigureAwait(false);
    }
}