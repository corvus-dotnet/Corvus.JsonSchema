...
public sealed class MySqlRunnerRegistry : IRunnerRegistry, IAsyncDisposable
{
    private const string SchemaSql =
        ...
        CREATE TABLE IF NOT EXISTS runner_hosted_versions (
            ...
            isolation_model VARCHAR(50) NULL,
            environment VARCHAR(63) NOT NULL,
            PRIMARY KEY (runner_id, base_workflow_id, version_number),
            INDEX ix_runner_hosted_versions_version (base_workflow_id, version_number, environment),
    ...
    public async ValueTask<bool> IsVersionHostedAsync(string baseWorkflowId, int versionNumber, string environment, RunIsolationModel requiredIsolation, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = "SELECT EXISTS(SELECT 1 FROM runner_hosted_versions WHERE base_workflow_id = @baseWorkflowId AND version_number = @versionNumber AND environment = @environment AND (@requiredIsolation = 'InProcess' OR isolation_model = 'Isolated'));";
    ...
    public async ValueTask<RunnerRegistryPage> ListAsync(AccessContext context, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        // Reach (§14.2) is a per-row ABAC predicate, not expressible as SQL, so runner_id COLLATE utf8mb4_bin drives an
        // ordered range SEEK past the cursor and rows are streamed + reach-filtered in flight — the reader is consumed only
        // until the page fills, never the full SELECT + parse of every registration the in-memory fallback does. (No LIMIT:
