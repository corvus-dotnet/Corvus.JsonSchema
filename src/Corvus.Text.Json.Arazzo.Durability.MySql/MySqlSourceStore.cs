// <copyright file="MySqlSourceStore.cs" company="Endjin Limited">
...
/// <summary>
/// A MySQL-backed <see cref="ISourceStore"/> (design §7.6): registered sources persisted relationally. Each
/// source is stored as its <see cref="RegisteredSource"/> document in a <c>LONGBLOB</c> column, keyed by (Name, and a
...
/// </remarks>
public sealed class MySqlSourceStore : ISourceStore, IAsyncDisposable
{
    private readonly MySqlDataSource dataSource;
    private readonly bool ownsDataSource;
    private readonly TimeProvider timeProvider;

    private MySqlSourceStore(MySqlDataSource dataSource, bool ownsDataSource, TimeProvider timeProvider)
    {
    ...
    public static ValueTask<MySqlSourceStore> ConnectAsync(string connectionString, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connectionString);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<MySqlSourceStore>(
            new MySqlSourceStore(new MySqlDataSource(connectionString), ownsDataSource: true, timeProvider ?? TimeProvider.System));
    }
    ...
    public static ValueTask<MySqlSourceStore> ConnectAsync(MySqlDataSource dataSource, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(dataSource);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<MySqlSourceStore>(
            new MySqlSourceStore(dataSource, ownsDataSource: false, timeProvider ?? TimeProvider.System));
    }
    ...
    public async ValueTask<ParsedJsonDocument<RegisteredSource>> AddAsync(RegisteredSource draft, string actor, CancellationToken cancellationToken)
    {
        ...
        insert.CommandText = "INSERT INTO Sources (Name, TagsHash, Tags, Etag, Document) VALUES (@n, @h, @t, @etag, @doc);";
    ...
    public async ValueTask<SourcePage> ListAsync(AccessContext context, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // evaluates, but applied in the database), so out-of-reach rows never leave it. LIMIT bounds it.
            ...
            var sql = new StringBuilder("SELECT Name, TagsHash, Document FROM Sources");
    ...
    public async ValueTask<bool> DeleteAsync(string name, WorkflowEtag expectedEtag, AccessContext context, CancellationToken cancellationToken)
    {
        ...
        delete.CommandText =
            "DELETE FROM Sources WHERE Name = @n AND TagsHash = @h; " +
            "DELETE FROM SourceSecurityTags WHERE Name = @n AND TagsHash = @h;";
    ...
    private static async Task SyncSecurityTagsAsync(MySqlConnection connection, MySqlTransaction transaction, string name, string tagsHash, SecurityTagSet managementTags, CancellationToken cancellationToken)
    {
        ...
        foreach (SecurityTag tag in managementTags.ToList())
        {
            await using MySqlCommand insert = connection.CreateCommand();
            insert.Transaction = transaction;
            insert.CommandText = "INSERT INTO SourceSecurityTags (Name, TagsHash, TagKey, TagValue) VALUES (@n, @h, @key, @value);";
    ...
    // The fixed-length, deterministic row-addressing key for a source: a SHA-256 hex digest over the identity
    ...
    // discriminator (the raw Tags value, re-hashed for row addressing). A source outside reach is invisible.
    ...
    private const string SchemaSql =
        ...
        CREATE TABLE IF NOT EXISTS SourceSecurityTags (
            ...
            INDEX IX_SourceSecurityTags_owner (Name, TagsHash),
            INDEX IX_SourceSecurityTags_kv (TagKey, TagValue)
        );
        """;
}