...
public sealed class MySqlWorkflowCatalogStore : IWorkflowCatalogStore, ISupportsRowSecurityFilter, IAsyncDisposable
{
    ...
    public async ValueTask<CatalogPage> QueryAsync(CatalogQuery query, CancellationToken cancellationToken)
    {
        ...
        // and keep the representative (RepRank = 1); keyset-page by base workflow id alone (base compare/order pinned
        // COLLATE utf8mb4_bin for byte-ordinal paging). Otherwise page every matching version by (base, version).
    ...
    public async ValueTask<bool> UpdatePackageAsync(string baseWorkflowId, int versionNumber, ReadOnlyMemory<byte> updatedPackage, CancellationToken cancellationToken)
    {
        ...
        CatalogPackage.EnsureAddsOnlyNativeArtifacts(storedHash, stored, updatedPackage);
    ...
    public async ValueTask<(int Count, bool Capped)> CountAsync(CatalogQuery query, int cap, CancellationToken cancellationToken)
    {
        ...
        long total = Convert.ToInt64(await select.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false), CultureInfo.InvariantCulture);
    ...
    private static string BuildFilterWhere(MySqlCommand command, in CatalogQuery query)
    {
