// <copyright file="MySqlWorkflowDeploymentStore.cs" company="Endjin Limited">
...
/// <summary>
/// A MySQL-backed <see cref="IWorkflowDeploymentStore"/> — serverless deployments (ADR 0055) persisted relationally. Each
/// deployment is stored as its <see cref="WorkflowDeployment"/> schema document in a <c>LONGBLOB</c> column, with the
/// filterable fields (status, target tuple) and the etag mirrored into columns for querying and the optimistic-concurrency
/// check. The id is derived from the target tuple, so <see cref="EnqueueAsync"/> is an idempotent
/// <c>INSERT ... ON DUPLICATE KEY UPDATE</c> upsert. Mirrors <see cref="MySqlNativeBuildJobStore"/>, keyed by the
/// target-derived id.
/// </summary>
...
public sealed class MySqlWorkflowDeploymentStore : IWorkflowDeploymentStore, IAsyncDisposable
{
    private readonly MySqlDataSource dataSource;
    private readonly bool ownsDataSource;
    private readonly TimeProvider timeProvider;

    private MySqlWorkflowDeploymentStore(MySqlDataSource dataSource, bool ownsDataSource, TimeProvider timeProvider)
    {
    ...
    public static ValueTask<MySqlWorkflowDeploymentStore> ConnectAsync(string connectionString, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connectionString);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<MySqlWorkflowDeploymentStore>(
            new MySqlWorkflowDeploymentStore(new MySqlDataSource(connectionString), ownsDataSource: true, timeProvider ?? TimeProvider.System));
    }
    ...
    public static ValueTask<MySqlWorkflowDeploymentStore> ConnectAsync(MySqlDataSource dataSource, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(dataSource);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<MySqlWorkflowDeploymentStore>(
            new MySqlWorkflowDeploymentStore(dataSource, ownsDataSource: false, timeProvider ?? TimeProvider.System));
    }
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>> EnqueueAsync(WorkflowDeployment draft, string actor, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            upsert.CommandText =
                ...
                "CreatedAt = VALUES(CreatedAt), Etag = VALUES(Etag), LeaseExpiresAt = VALUES(LeaseExpiresAt), Document = VALUES(Document);";
    ...
    private static void AppendFilters(List<string> conditions, MySqlCommand command, WorkflowDeploymentQuery query)
    {
    ...
    private const string SchemaSql =
        ...
        CREATE TABLE IF NOT EXISTS WorkflowDeployments (
            ...
            LeaseExpiresAt VARCHAR(33) NULL,
            Document LONGBLOB NOT NULL,
            INDEX IX_WorkflowDeployments_Status (Status),
            INDEX IX_WorkflowDeployments_Target (BaseWorkflowId, VersionNumber, Environment, RuntimeIdentifier),
            INDEX IX_WorkflowDeployments_Queue (Status, CreatedAt, Id),
            INDEX IX_WorkflowDeployments_Created (CreatedAt, Id)
        );
        """;
}