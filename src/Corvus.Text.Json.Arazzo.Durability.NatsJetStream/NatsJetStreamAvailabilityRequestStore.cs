// <copyright file="NatsJetStreamAvailabilityRequestStore.cs" company="Endjin Limited">
...
/// <summary>
/// A NATS JetStream-backed <see cref="IAvailabilityRequestStore"/> — availability ("promotion") requests (design §7.8)
/// persisted in a single KV bucket. Each request is stored as its <see cref="AvailabilityRequest"/> schema document under a
/// namespaced, Base64Url-encoded key. The KV listing is unordered, so the unpaged
/// <see cref="ListAsync(AvailabilityRequestQuery, CancellationToken)"/> materialises every request and filters / orders
/// client-side; the paged seam adds a keyset index of marker keys for oldest-first paging. The etag travels inside the
/// document (not the KV revision), so optimistic concurrency is a read-compare-write driven by
/// <see cref="AvailabilityRequestSerialization.SerializeDecision"/>, exactly as the SQLite and in-memory backends do.
/// Mirrors <see cref="NatsJetStreamAccessRequestStore"/>, parameterised by environment.
/// </summary>
public sealed class NatsJetStreamAvailabilityRequestStore : IAvailabilityRequestStore, IAsyncDisposable
{
    private const string Bucket = "arazzo_availability_requests";
    ...
    private static readonly IComparer<ParsedJsonDocument<AvailabilityRequest>> ByCreatedAtThenId =
    ...
    private NatsJetStreamAvailabilityRequestStore(NatsConnection? ownedConnection, INatsKVStore store, TimeProvider timeProvider)
    {
    ...
    /// <summary>Provisions the availability-request KV bucket (requires stream-management rights); run once at deploy time.</summary>
    ...
    /// <summary>Provisions the availability-request KV bucket over a caller-supplied connection.</summary>
    ...
    public static async ValueTask<NatsJetStreamAvailabilityRequestStore> ConnectAsync(string url, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(url);
        ...
        try
        {
            ...
            return new NatsJetStreamAvailabilityRequestStore(connection, store, timeProvider ?? TimeProvider.System);
        }
        catch
        {
    ...
    public static async ValueTask<NatsJetStreamAvailabilityRequestStore> ConnectAsync(INatsConnection connection, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connection);
        var kv = new NatsKVContext(new NatsJSContext(connection));
        INatsKVStore store = await kv.GetStoreAsync(Bucket, cancellationToken).ConfigureAwait(false);
        return new NatsJetStreamAvailabilityRequestStore(ownedConnection: null, store, timeProvider ?? TimeProvider.System);
    }
    ...
    public async ValueTask<ParsedJsonDocument<AvailabilityRequest>?> GetAsync(string id, CancellationToken cancellationToken)
    {
        ...
        NatsKVEntry<byte[]>? entry = await this.TryGetAsync(RequestPrefix + Enc(id), cancellationToken).ConfigureAwait(false);
        return entry is { Value: { } bytes } ? ParsedJsonDocument<AvailabilityRequest>.Parse(bytes.AsMemory()) : null;
    }
    ...
    public async ValueTask<ParsedJsonDocument<AvailabilityRequest>?> DecideAsync(string id, AvailabilityRequestDecision decision, WorkflowEtag expectedEtag, string actor, CancellationToken cancellationToken)
    {
        ...
        WorkflowEtag etag = NewEtag();
        using ParsedJsonDocument<AvailabilityRequest> current = ParsedJsonDocument<AvailabilityRequest>.Parse(bytes.AsMemory());
