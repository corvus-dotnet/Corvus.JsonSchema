...
public sealed class NatsJetStreamSourceStore : ISourceStore, IAsyncDisposable
{
    ...
    private NatsJetStreamSourceStore(NatsConnection? ownedConnection, INatsKVStore store, INatsKVStore labels, TimeProvider timeProvider)
    {
        this.labels = labels;
    ...
    public static async ValueTask PrepareAsync(string url, CancellationToken cancellationToken = default)
    {
        ...
        await kv.CreateStoreAsync(new NatsKVConfig(NatsSecurityLabels.SourceLabelBucket), cancellationToken).ConfigureAwait(false);
    }
    ...
    public static async ValueTask<NatsJetStreamSourceStore> ConnectAsync(string url, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            INatsKVStore labels = await kv.GetStoreAsync(NatsSecurityLabels.SourceLabelBucket, cancellationToken).ConfigureAwait(false);
            return new NatsJetStreamSourceStore(connection, store, labels, timeProvider ?? TimeProvider.System);
        }
        catch
        {
    ...
    public static async ValueTask<NatsJetStreamSourceStore> ConnectAsync(INatsConnection connection, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ...
        INatsKVStore store = await kv.GetStoreAsync(Bucket, cancellationToken).ConfigureAwait(false);
        INatsKVStore labels = await kv.GetStoreAsync(NatsSecurityLabels.SourceLabelBucket, cancellationToken).ConfigureAwait(false);
        return new NatsJetStreamSourceStore(ownedConnection: null, store, labels, timeProvider ?? TimeProvider.System);
    }
    ...
    public async ValueTask<SourcePage> ListAsync(AccessContext context, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        // the index could not narrow and the keys-only scan runs as it always did; an empty one means no source
        ...
        // source.{Enc(name)}.{Enc(discriminator)}, so decoding its two Base64Url parts recovers the ordering tuple
