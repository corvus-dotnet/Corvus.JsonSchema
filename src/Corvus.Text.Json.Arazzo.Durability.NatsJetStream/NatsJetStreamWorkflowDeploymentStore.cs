// <copyright file="NatsJetStreamWorkflowDeploymentStore.cs" company="Endjin Limited">
...
/// <summary>
/// A NATS JetStream-backed <see cref="IWorkflowDeploymentStore"/> — serverless workflow deployments (ADR 0055) persisted in a
/// single KV bucket. Each deployment is stored as its <see cref="WorkflowDeployment"/> schema document under a namespaced,
/// Base64Url-encoded key. The KV listing is unordered, so the unpaged
/// <see cref="ListAsync(WorkflowDeploymentQuery, CancellationToken)"/> materialises every deployment and filters / orders
/// client-side; the paged seam adds a keyset index of marker keys for oldest-first paging. The id is derived from the target
/// tuple, so <see cref="EnqueueAsync"/> is an idempotent put, and the store-owned etag travels inside the document. Mirrors
/// <see cref="NatsJetStreamAvailabilityRequestStore"/>, keyed by the target-derived id rather than a random id.
/// </summary>
/// <remarks>The claim walks the keyset index oldest-first and transitions the first Queued deployment to Deploying under the
/// KV's native revision compare-and-set (<c>UpdateAsync</c> with the read revision); a lost CAS means a concurrent worker
/// claimed it, so the claim retries the next-oldest — two workers never claim the same deployment.</remarks>
public sealed class NatsJetStreamWorkflowDeploymentStore : IWorkflowDeploymentStore, IAsyncDisposable
{
    private const string Bucket = "arazzo_workflow_deployments";
    private const string DeploymentPrefix = "dep.";

    // A keyset index for native oldest-first paging: one marker key per deployment of the form
    ...
    // documents are then fetched. The "idx." prefix never collides with the "dep." record keys.
    ...
    private static readonly IComparer<ParsedJsonDocument<WorkflowDeployment>> ByCreatedAtThenId =
    ...
    private NatsJetStreamWorkflowDeploymentStore(NatsConnection? ownedConnection, INatsKVStore store, TimeProvider timeProvider)
    {
    ...
    /// <summary>Provisions the workflow-deployment KV bucket (requires stream-management rights); run once at deploy time.</summary>
    ...
    /// <summary>Provisions the workflow-deployment KV bucket over a caller-supplied connection.</summary>
    ...
    public static async ValueTask<NatsJetStreamWorkflowDeploymentStore> ConnectAsync(string url, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(url);
        ...
        try
        {
            ...
            return new NatsJetStreamWorkflowDeploymentStore(connection, store, timeProvider ?? TimeProvider.System);
        }
        catch
        {
    ...
    public static async ValueTask<NatsJetStreamWorkflowDeploymentStore> ConnectAsync(INatsConnection connection, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connection);
        var kv = new NatsKVContext(new NatsJSContext(connection));
        INatsKVStore store = await kv.GetStoreAsync(Bucket, cancellationToken).ConfigureAwait(false);
        return new NatsJetStreamWorkflowDeploymentStore(ownedConnection: null, store, timeProvider ?? TimeProvider.System);
    }
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>> EnqueueAsync(WorkflowDeployment draft, string actor, CancellationToken cancellationToken)
    {
        ...
        NatsKVEntry<byte[]>? existing = await this.TryGetAsync(DeploymentPrefix + Enc(id), cancellationToken).ConfigureAwait(false);
        if (existing is { Value: { } priorBytes })
        {
            using ParsedJsonDocument<WorkflowDeployment> prior = ParsedJsonDocument<WorkflowDeployment>.Parse(priorBytes.AsMemory());
        ...
        await this.store.PutAsync(DeploymentPrefix + Enc(id), json, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>?> GetAsync(string id, CancellationToken cancellationToken)
    {
        ...
        NatsKVEntry<byte[]>? entry = await this.TryGetAsync(DeploymentPrefix + Enc(id), cancellationToken).ConfigureAwait(false);
        return entry is { Value: { } bytes } ? ParsedJsonDocument<WorkflowDeployment>.Parse(bytes.AsMemory()) : null;
    }
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>?> ClaimNextQueuedAsync(string claimedBy, TimeSpan leaseTtl, CancellationToken cancellationToken)
    {
        ...
        // One `now` guards the lease-expiry test (which orphaned Deploying deployments are reclaimable) and stamps the new
        ...
        // Try the oldest CLAIMABLE candidate first (Queued, or an orphaned Deploying whose lease has expired — ADR 0056),
        // transitioning it to Deploying under the KV native revision CAS (UpdateAsync with the read revision); on a
        ...
        foreach ((string _, string id) in keys)
        {
            ...
            using ParsedJsonDocument<WorkflowDeployment> current = ParsedJsonDocument<WorkflowDeployment>.Parse(bytes.AsMemory());
            if (!current.RootElement.IsClaimable(now))
            {
                continue; // completed, or Deploying with a live lease
            }

            WorkflowEtag etag = NewEtag();
            byte[] claimed = WorkflowDeploymentSerialization.SerializeClaimed(current.RootElement, claimedBy, now, now + leaseTtl, etag);
            try
            {
                await this.store.UpdateAsync(DeploymentPrefix + Enc(id), claimed, entry.Value.Revision, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>?> RenewLeaseAsync(string id, WorkflowEtag expectedEtag, TimeSpan leaseTtl, CancellationToken cancellationToken)
    {
        ...
        WorkflowEtag etag = NewEtag();
        byte[] json = WorkflowDeploymentSerialization.SerializeRenewal(current.RootElement, id, expectedEtag, this.timeProvider.GetUtcNow() + leaseTtl, etag);
        await this.store.PutAsync(DeploymentPrefix + Enc(id), json, cancellationToken: cancellationToken).ConfigureAwait(false);
        return PersistedJson.ToPooledDocument<WorkflowDeployment>(json);
    }
    ...
    public async ValueTask<PooledDocumentList<WorkflowDeployment>> ListAsync(WorkflowDeploymentQuery query, CancellationToken cancellationToken)
    {
        ...
        await foreach (string key in this.store.GetKeysAsync(cancellationToken: cancellationToken).ConfigureAwait(false))
        {
            if (!key.StartsWith(DeploymentPrefix, StringComparison.Ordinal))
            {
                continue;
            }
            ...
            ParsedJsonDocument<WorkflowDeployment> document = ParsedJsonDocument<WorkflowDeployment>.Parse(bytes.AsMemory());
    ...
    public async ValueTask<(int Count, bool Capped)> CountAsync(WorkflowDeploymentQuery query, int cap, CancellationToken cancellationToken)
    {
        ...
        await foreach (string key in this.store.GetKeysAsync(cancellationToken: cancellationToken).ConfigureAwait(false))
        {
            ...
            using ParsedJsonDocument<WorkflowDeployment> document = ParsedJsonDocument<WorkflowDeployment>.Parse(bytes.AsMemory());
    ...
    // The keyset index key for a deployment: "idx.{Base64Url(createdAtIso)}.{Base64Url(id)}", with createdAt as the
