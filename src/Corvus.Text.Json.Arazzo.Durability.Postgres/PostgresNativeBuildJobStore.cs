// <copyright file="PostgresNativeBuildJobStore.cs" company="Endjin Limited">
...
/// <summary>
/// A PostgreSQL-backed <see cref="INativeBuildJobStore"/> — Native-AOT build jobs (ADR 0055) persisted relationally. Each job
/// is stored as its <see cref="NativeBuildJob"/> schema document in a <c>bytea</c> column, with the filterable fields (status,
/// target tuple) and the etag mirrored into columns for querying and the optimistic-concurrency check. The id is derived from
/// the target tuple, so <see cref="EnqueueAsync"/> is an idempotent <c>INSERT ... ON CONFLICT</c> upsert. Mirrors
/// <see cref="PostgresAvailabilityRequestStore"/>, keyed by the target-derived id rather than a random id.
/// </summary>
...
public sealed class PostgresNativeBuildJobStore : INativeBuildJobStore, IAsyncDisposable
{
    private readonly NpgsqlDataSource dataSource;
    private readonly bool ownsDataSource;
    private readonly TimeProvider timeProvider;

    private PostgresNativeBuildJobStore(NpgsqlDataSource dataSource, bool ownsDataSource, TimeProvider timeProvider)
    {
    ...
    public static ValueTask<PostgresNativeBuildJobStore> ConnectAsync(string connectionString, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connectionString);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<PostgresNativeBuildJobStore>(
            new PostgresNativeBuildJobStore(NpgsqlDataSource.Create(connectionString), ownsDataSource: true, timeProvider ?? TimeProvider.System));
    }
    ...
    public static ValueTask<PostgresNativeBuildJobStore> ConnectAsync(NpgsqlDataSource dataSource, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(dataSource);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<PostgresNativeBuildJobStore>(
            new PostgresNativeBuildJobStore(dataSource, ownsDataSource: false, timeProvider ?? TimeProvider.System));
    }
    ...
    public async ValueTask<ParsedJsonDocument<NativeBuildJob>?> ClaimNextQueuedAsync(string claimedBy, TimeSpan leaseTtl, CancellationToken cancellationToken)
    {
        ...
        // orphaned Building whose lease has expired (ADR 0056). A concurrent claim skips this locked row and takes the next,
        // so two workers never claim the same job; a crashed worker holds no lock, so its orphan is lockable and reclaimed.
        string? id = null;
        byte[]? document = null;
    ...
    private static void AppendFilters(List<string> conditions, NpgsqlCommand command, NativeBuildJobQuery query)
    {
    ...
    private const string SchemaSql =
        ...
        CREATE INDEX IF NOT EXISTS IX_NativeBuildJobs_Created ON NativeBuildJobs (CreatedAt, Id);
        """;
}