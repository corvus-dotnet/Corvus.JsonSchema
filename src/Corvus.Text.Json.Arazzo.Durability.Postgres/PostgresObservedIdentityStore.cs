// <copyright file="PostgresObservedIdentityStore.cs" company="Endjin Limited">
...
/// <summary>
/// A PostgreSQL-backed <see cref="IObservedIdentityStore"/> (design §16.5.4): the distinct grantees the control plane
...
/// key columns are declared <c>COLLATE "C"</c> so ordering, the prefix lower bound, and the keyset are byte-ordinal —
/// the order the contract pages by — independent of the database's locale collation; the prefix upper bound is an
/// in-memory <c>StartsWith</c>. Each operation opens a pooled connection; a sighting runs in one transaction.</para>
/// </remarks>
public sealed class PostgresObservedIdentityStore : IObservedIdentityStore, IAsyncDisposable
{
    private readonly NpgsqlDataSource dataSource;
    private readonly bool ownsDataSource;
    private readonly TimeProvider timeProvider;

    private PostgresObservedIdentityStore(NpgsqlDataSource dataSource, bool ownsDataSource, TimeProvider timeProvider)
    {
    ...
    public static ValueTask<PostgresObservedIdentityStore> ConnectAsync(string connectionString, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connectionString);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<PostgresObservedIdentityStore>(
            new PostgresObservedIdentityStore(NpgsqlDataSource.Create(connectionString), ownsDataSource: true, timeProvider ?? TimeProvider.System));
    }
    ...
    public static ValueTask<PostgresObservedIdentityStore> ConnectAsync(NpgsqlDataSource dataSource, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(dataSource);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<PostgresObservedIdentityStore>(
            new PostgresObservedIdentityStore(dataSource, ownsDataSource: false, timeProvider ?? TimeProvider.System));
    }
    ...
    private const string SchemaSql =
        ...
        CREATE TABLE IF NOT EXISTS ObservedIdentities (
            SubjectKind TEXT COLLATE "C" NOT NULL,
            SubjectValue TEXT COLLATE "C" NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS ix_observedidentitysecuritytags_kv ON ObservedIdentitySecurityTags (TagKey, TagValue);
        """;
}