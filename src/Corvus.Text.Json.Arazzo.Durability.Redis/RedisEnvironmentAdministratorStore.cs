...
public sealed class RedisEnvironmentAdministratorStore : IEnvironmentAdministratorStore, IAsyncDisposable
{
    ...
    public async ValueTask<EnvironmentAdministeredPage> ListAdministeredAsync(IReadOnlyList<string> adminDigests, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        // The keyset cursor (the environment name to page strictly after) reifies once here, never per row. Each digest's
        // sorted-set members are the administered environment names (score 0), so a ZRANGEBYLEX is an ordinal keyset.
        ...
        // (dedupe + ordinal order via the SortedSet) yields the global smallest pageSize+1 — an environment in more than one
