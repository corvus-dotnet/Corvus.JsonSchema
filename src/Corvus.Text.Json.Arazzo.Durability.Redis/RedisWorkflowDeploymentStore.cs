// <copyright file="RedisWorkflowDeploymentStore.cs" company="Endjin Limited">
...
/// <summary>
/// A Redis-backed <see cref="IWorkflowDeploymentStore"/> — serverless deployments (ADR 0055) persisted for a distributed host.
/// Each deployment is stored verbatim as its <see cref="WorkflowDeployment"/> Corvus.Text.Json document under a per-record key,
/// with a single set holding every deployment id for enumeration and a keyset sorted-set index for oldest-first paging. The id
/// is derived from the target tuple, so <see cref="EnqueueAsync"/> is an idempotent overwrite, and the etag travels inside the
/// document. Mirrors <see cref="RedisAvailabilityRequestStore"/>, keyed by the target-derived id rather than a random id.
/// </summary>
/// <remarks>The claim walks the keyset index oldest-first and transitions the first Queued deployment to Deploying under an
/// optimistic compare-and-set (a Redis transaction conditioned on the record's bytes being unchanged); a lost CAS means a
/// concurrent worker claimed it, so the claim retries the next-oldest — two workers never claim the same deployment.</remarks>
public sealed class RedisWorkflowDeploymentStore : IWorkflowDeploymentStore, IAsyncDisposable
{
    private const string DeploymentPrefix = "arazzo:workflowdeployment:";
    private const string DeploymentIndexKey = "arazzo:workflowdeployments";
    ...
    private const string DeploymentByCreatedKey = "arazzo:workflowdeployments:bycreated";
    ...
    private static readonly IComparer<ParsedJsonDocument<WorkflowDeployment>> ByCreatedThenId =
    ...
    private RedisWorkflowDeploymentStore(IConnectionMultiplexer connection, TimeProvider timeProvider, bool ownsConnection)
    {
    ...
    /// <summary>Opens a workflow-deployment store over the given Redis configuration.</summary>
    ...
    public static async ValueTask<RedisWorkflowDeploymentStore> ConnectAsync(string configuration, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(configuration);
        cancellationToken.ThrowIfCancellationRequested();
        IConnectionMultiplexer connection = await ConnectionMultiplexer.ConnectAsync(configuration).ConfigureAwait(false);
        return new RedisWorkflowDeploymentStore(connection, timeProvider ?? TimeProvider.System, ownsConnection: true);
    }

    /// <summary>Creates a workflow-deployment store over an existing connection (the caller keeps ownership).</summary>
    /// <param name="connection">The Redis connection.</param>
    /// <param name="timeProvider">The time source for audit timestamps; defaults to <see cref="TimeProvider.System"/>.</param>
    /// <returns>The store.</returns>
    public static RedisWorkflowDeploymentStore Connect(IConnectionMultiplexer connection, TimeProvider? timeProvider = null)
    {
        ArgumentNullException.ThrowIfNull(connection);
        return new RedisWorkflowDeploymentStore(connection, timeProvider ?? TimeProvider.System, ownsConnection: false);
    }
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>> EnqueueAsync(WorkflowDeployment draft, string actor, CancellationToken cancellationToken)
    {
        ...
        using (Lease<byte>? existing = await this.database.StringGetLeaseAsync(DeploymentPrefix + id).ConfigureAwait(false))
        {
            if (existing is { Length: > 0 })
            {
                using ParsedJsonDocument<WorkflowDeployment> prior = ParsedJsonDocument<WorkflowDeployment>.Parse(existing.Memory);
                await this.database.SortedSetRemoveAsync(DeploymentByCreatedKey, KeysetMember(prior.RootElement.CreatedAtValue, id)).ConfigureAwait(false);
            }
        }
        ...
        try
        {
            ...
            await this.database.StringSetAsync(DeploymentPrefix + id, utf8).ConfigureAwait(false);
            await this.database.SetAddAsync(DeploymentIndexKey, id).ConfigureAwait(false);
            await this.database.SortedSetAddAsync(DeploymentByCreatedKey, KeysetMember(now, id), 0).ConfigureAwait(false);
            return doc;
        }
        catch
        {
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>?> GetAsync(string id, CancellationToken cancellationToken)
    {
        ...
        using Lease<byte>? lease = await this.database.StringGetLeaseAsync(DeploymentPrefix + id).ConfigureAwait(false);
        return lease is { Length: > 0 } ? PersistedJson.ToPooledDocument<WorkflowDeployment>(lease.Span) : null;
    }
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>?> ClaimNextQueuedAsync(string claimedBy, TimeSpan leaseTtl, CancellationToken cancellationToken)
    {
        ...
        // candidate (Queued, or an orphaned Deploying whose lease has expired — ADR 0056), transition it to Deploying under an
        ...
        // it retries the next-oldest — two workers never claim the same deployment.
        RedisValue[] members = await this.database.SortedSetRangeByValueAsync(DeploymentByCreatedKey).ConfigureAwait(false);
        foreach (RedisValue value in members)
        {
            ...
            RedisValue currentValue = await this.database.StringGetAsync(DeploymentPrefix + id).ConfigureAwait(false);
            ...
            using ParsedJsonDocument<WorkflowDeployment> current = ParsedJsonDocument<WorkflowDeployment>.Parse(currentBytes.AsMemory());
            if (!current.RootElement.IsClaimable(now))
            {
                continue; // completed, or Deploying with a live lease — the claim never changes createdAt/id, so the member stays valid
            }
            ...
            transaction.AddCondition(Condition.StringEqual(DeploymentPrefix + id, currentValue));
            _ = transaction.StringSetAsync(DeploymentPrefix + id, claimed);
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>?> CompleteAsync(string id, WorkflowDeploymentCompletion completion, WorkflowEtag expectedEtag, CancellationToken cancellationToken)
    {
        ...
        using ParsedJsonDocument<WorkflowDeployment> current = ParsedJsonDocument<WorkflowDeployment>.Parse(lease.Memory);
    ...
    public async ValueTask<PooledDocumentList<WorkflowDeployment>> ListAsync(WorkflowDeploymentQuery query, CancellationToken cancellationToken)
    {
        ...
        foreach (RedisValue member in await this.database.SetMembersAsync(DeploymentIndexKey).ConfigureAwait(false))
        {
            ...
            using Lease<byte>? lease = await this.database.StringGetLeaseAsync(DeploymentPrefix + (string)member!).ConfigureAwait(false);
            if (lease is not { Length: > 0 })
            {
                continue;
            }

            ParsedJsonDocument<WorkflowDeployment> document = PersistedJson.ToPooledDocument<WorkflowDeployment>(lease.Span);
    ...
    public async ValueTask<WorkflowDeploymentPage> ListAsync(WorkflowDeploymentQuery query, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        // filter until the page fills (reads ~ pageSize / selectivity), never every deployment's document.
        RedisValue[] members = await this.database.SortedSetRangeByValueAsync(DeploymentByCreatedKey).ConfigureAwait(false);
    ...
    public async ValueTask<(int Count, bool Capped)> CountAsync(WorkflowDeploymentQuery query, int cap, CancellationToken cancellationToken)
    {
        ...
        foreach (RedisValue member in await this.database.SetMembersAsync(DeploymentIndexKey).ConfigureAwait(false))
        {
            ...
            using ParsedJsonDocument<WorkflowDeployment> document = PersistedJson.ToPooledDocument<WorkflowDeployment>(lease.Span);
    ...
    // The keyset index member for a deployment: "{createdAtIso}\0{id}", with createdAt as the fixed-width ISO-8601 "o" UTC
