...
public interface IApiClaimsHandler
{
    ...
    /// <summary>
    /// Handles POST /rekeyClaims — Claim the resting runs still sealed under an older key generation
    /// </summary>
    /// <param name="parameters">The operation parameters.</param>
    /// <param name="workspace">The workspace for building response values.</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>The operation result.</returns>
    ValueTask<ClaimForRekeyResult> HandleClaimForRekeyAsync(ClaimForRekeyParams parameters, JsonWorkspace workspace, CancellationToken cancellationToken = default);
}
