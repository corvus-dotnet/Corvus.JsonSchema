// <copyright file="SqlServerAvailabilityStore.cs" company="Endjin Limited">
...
/// <summary>
/// A SQL Server-backed <see cref="IAvailabilityStore"/> (design §7.8): the availability matrix (which workflow versions
/// are available in which environments) persisted relationally. Each entry is stored as its <see cref="AvailabilityEntry"/>
/// document in a <c>varbinary(max)</c> column, keyed by (BaseWorkflowId, VersionNumber, Environment). Availability has no
...
/// </summary>
/// <remarks>
/// Each operation opens a pooled connection, so the store is naturally concurrent. The two list axes are indexed keyset
/// range scans: by-version orders by Environment (the primary key prefix already covers it); by-environment orders by
/// (BaseWorkflowId, VersionNumber) over a secondary index.
/// </remarks>
public sealed class SqlServerAvailabilityStore : IAvailabilityStore, IAsyncDisposable
{
    private readonly string connectionString;
    private readonly TimeProvider timeProvider;

    private SqlServerAvailabilityStore(string connectionString, TimeProvider timeProvider)
    {
    ...
    public static ValueTask<SqlServerAvailabilityStore> ConnectAsync(string connectionString, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connectionString);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<SqlServerAvailabilityStore>(new SqlServerAvailabilityStore(connectionString, timeProvider ?? TimeProvider.System));
    }
    ...
    private static async ValueTask<byte[]?> ReadDocumentAsync(SqlConnection connection, string baseWorkflowId, int versionNumber, string environment, CancellationToken cancellationToken)
    {
        await using SqlCommand select = connection.CreateCommand();
    ...
    private const string SchemaSql =
        """
        IF OBJECT_ID(N'Availability', N'U') IS NULL
        BEGIN
            CREATE TABLE Availability (
                ...
                CONSTRAINT PK_Availability PRIMARY KEY (BaseWorkflowId, VersionNumber, Environment)
            );
            CREATE INDEX IX_Availability_Environment ON Availability (Environment, BaseWorkflowId, VersionNumber);
        END;
        """;
}