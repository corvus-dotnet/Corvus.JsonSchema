...
public sealed class SqlServerSecurityPolicyStore : ISecurityPolicyStore, IAsyncDisposable
{
    ...
    public async ValueTask<PooledDocumentList<SecurityBindingDocument>> ListBindingsForSubjectAsync(JsonString claimType, JsonString claimValue, CancellationToken cancellationToken)
    {
        ...
        // Native by-subject lookup: ordinal equality on the extracted ClaimType/ClaimValue columns. COLLATE
        // Latin1_General_BIN2 forces the byte-wise comparison the in-memory default uses (the database's default
        // collation is typically case-insensitive), so a case-variant subject cannot over-match. A value-less binding
        // has NULL ClaimValue, which never equals a value, so it is excluded. Ordered by the same (SortOrder, Id) as the
        // full read; only this subject's rows are read.
        await using SqlCommand select = connection.CreateCommand();
        select.CommandText =
            """
            SELECT Document FROM SecurityBindings
            WHERE ClaimType COLLATE Latin1_General_BIN2 = @claimType AND ClaimValue COLLATE Latin1_General_BIN2 = @claimValue
