...
public sealed class SqlServerWorkflowCatalogStore : IWorkflowCatalogStore, ISupportsRowSecurityFilter
{
    ...
    public async ValueTask<CatalogPage> QueryAsync(CatalogQuery query, CancellationToken cancellationToken)
    {
        ...
        // and keep the representative (RepRank = 1); keyset-page by base workflow id alone (base compare/order COLLATE
        // Latin1_General_BIN2 for byte-ordinal paging; TOP (@limit) caps the outer SELECT). Otherwise page by (base, version).
        select.CommandText = query.DistinctWorkflows
            ? "WITH ranked AS (\n  SELECT " + ColumnList +
              ...
              "SELECT TOP (@limit) " + ColumnList + " FROM ranked\n" +
              "WHERE RepRank = 1 AND (@after IS NULL OR BaseWorkflowId COLLATE Latin1_General_BIN2 > @after)\n" +
              "ORDER BY BaseWorkflowId COLLATE Latin1_General_BIN2;"
    ...
    public async ValueTask<ParsedJsonDocument<CatalogVersion>?> UpdateMetadataAsync(string baseWorkflowId, int versionNumber, CatalogMetadataPatch patch, CancellationToken cancellationToken)
    {
        ...
        await using SqlTransaction? transaction = reTag ? (SqlTransaction)await connection.BeginTransactionAsync(cancellationToken).ConfigureAwait(false) : null;
    ...
    public async ValueTask<bool> UpdatePackageAsync(string baseWorkflowId, int versionNumber, ReadOnlyMemory<byte> updatedPackage, CancellationToken cancellationToken)
    {
        ...
        // Stream the (potentially large) canonical package straight from its memory as the VARBINARY(MAX) parameter — no
        // GC copy of the whole package — exactly as the add path binds it.
        using ReadOnlyMemoryStream packageStream = ReadOnlyMemoryStream.Rent(updatedPackage);
        await using SqlCommand update = connection.CreateCommand();
        ...
        update.Parameters.AddWithValue("@baseWorkflowId", baseWorkflowId);
        update.Parameters.AddWithValue("@versionNumber", versionNumber);
        update.Parameters.Add(new SqlParameter("@package", SqlDbType.VarBinary, -1) { Value = packageStream });
    ...
    private static async ValueTask<ParsedJsonDocument<CatalogVersion>?> ReadOneAsync(SqlConnection connection, string baseWorkflowId, int versionNumber, CancellationToken cancellationToken)
    {
        await using SqlCommand select = connection.CreateCommand();
    ...
    private static ParsedJsonDocument<CatalogVersion> ReadVersion(SqlDataReader reader)
    ...
    public async ValueTask<(int Count, bool Capped)> CountAsync(CatalogQuery query, int cap, CancellationToken cancellationToken)
    {
        ...
        // SQL Server has no LIMIT: bound with TOP (@cap) then COUNT. Distinct mode counts distinct base workflows
        // (DISTINCT TOP returns up to @cap distinct base ids); otherwise matching versions. Same filter as the search.
        string inner = query.DistinctWorkflows
            ? "SELECT DISTINCT TOP (@cap) BaseWorkflowId FROM CatalogVersions WHERE " + filterWhere
            : "SELECT TOP (@cap) 1 AS x FROM CatalogVersions WHERE " + filterWhere;
    ...
    // TOP, or ORDER BY. QueryAsync embeds it (plus cursor/paging); CountAsync wraps it in a bounded COUNT — both
    // share this exact predicate so the reach filter cannot drift between search and count.
    private static string BuildFilterWhere(SqlCommand command, in CatalogQuery query)
    {
        ...
        command.Parameters.Add(NullableText("@workflowIdPrefixLike", query.WorkflowIdPrefix is { Length: > 0 } p ? EscapeLike(p) + "%" : null));
