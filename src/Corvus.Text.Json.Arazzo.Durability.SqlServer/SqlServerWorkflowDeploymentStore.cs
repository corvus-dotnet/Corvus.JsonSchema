// <copyright file="SqlServerWorkflowDeploymentStore.cs" company="Endjin Limited">
...
/// <summary>
/// A SQL Server-backed <see cref="IWorkflowDeploymentStore"/> — serverless workflow deployments (ADR 0055) persisted
/// relationally. Each deployment is stored as its <see cref="WorkflowDeployment"/> schema document in a <c>varbinary(max)</c>
/// column, with the filterable fields (status, target tuple) and the etag mirrored into columns for querying and the
/// optimistic-concurrency check. The creation instant is held sortably (ISO-8601 round-trip text) so the oldest-first ordering
/// is a plain <c>ORDER BY</c>. The id is derived from the target tuple, so <see cref="EnqueueAsync"/> is a race-safe
/// <c>MERGE</c> upsert. Mirrors <see cref="SqlServerAvailabilityRequestStore"/>, keyed by the target-derived id rather than a
/// random id.
/// </summary>
/// <remarks>Each operation opens a pooled connection, so the store is naturally concurrent; the claim is a single transaction
/// that reads the oldest queued row <c>WITH (UPDLOCK, READPAST, ROWLOCK)</c>, so two workers never claim the same deployment.</remarks>
public sealed class SqlServerWorkflowDeploymentStore : IWorkflowDeploymentStore, IAsyncDisposable
{
    private readonly string connectionString;
    private readonly TimeProvider timeProvider;

    private SqlServerWorkflowDeploymentStore(string connectionString, TimeProvider timeProvider)
    {
    ...
    public static ValueTask<SqlServerWorkflowDeploymentStore> ConnectAsync(string connectionString, TimeProvider? timeProvider = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(connectionString);
        cancellationToken.ThrowIfCancellationRequested();
        return new ValueTask<SqlServerWorkflowDeploymentStore>(new SqlServerWorkflowDeploymentStore(connectionString, timeProvider ?? TimeProvider.System));
    }
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>> EnqueueAsync(WorkflowDeployment draft, string actor, CancellationToken cancellationToken)
    {
        ...
        // race-safe MERGE that resets the deployment to Queued (SerializeNew omits startedAt/completedAt/failureReason/
        // functionUrl/claimedBy). A (re-)enqueue resets the deployment to Queued, so its lease is cleared (NULL); a Queued
        // deployment holds no lease.
        upsert.CommandText =
            """
            MERGE WorkflowDeployments WITH (HOLDLOCK) AS target
    ...
    public async ValueTask<ParsedJsonDocument<WorkflowDeployment>?> ClaimNextQueuedAsync(string claimedBy, TimeSpan leaseTtl, CancellationToken cancellationToken)
    {
        ...
        // an orphaned Deploying whose lease has expired (ADR 0056). UPDLOCK holds an update lock on the chosen row for the
        // transaction and READPAST makes a concurrent claim skip it and take the next, so two workers never claim the same
        // deployment; a crashed worker holds no lock, so its orphan is lockable and reclaimed.
        string? id = null;
        byte[]? document = null;
        await using (SqlCommand select = connection.CreateCommand())
        {
            ...
            select.CommandText =
                "SELECT TOP 1 Id, Document FROM WorkflowDeployments WITH (UPDLOCK, READPAST, ROWLOCK) " +
                "WHERE Status = @queued OR (Status = @deploying AND (LeaseExpiresAt IS NULL OR LeaseExpiresAt <= @now)) ORDER BY CreatedAt, Id;";
    ...
    public async ValueTask<WorkflowDeploymentPage> ListAsync(WorkflowDeploymentQuery query, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await using SqlConnection connection = await this.OpenAsync(cancellationToken).ConfigureAwait(false);
            await using SqlCommand select = connection.CreateCommand();
            var sql = new StringBuilder("SELECT TOP (@limit) Document FROM WorkflowDeployments");
            ...
            // The IX_WorkflowDeployments_Created index on (CreatedAt, Id) drives both the order and the seek; TOP bounds the
    ...
    public async ValueTask<(int Count, bool Capped)> CountAsync(WorkflowDeploymentQuery query, int cap, CancellationToken cancellationToken)
    {
        ...
        var inner = new StringBuilder("SELECT TOP (@cap) 1 AS x FROM WorkflowDeployments");
    ...
    private static void AppendFilters(List<string> conditions, SqlCommand command, WorkflowDeploymentQuery query)
    {
    ...
    private const string SchemaSql =
        """
        IF OBJECT_ID(N'WorkflowDeployments', N'U') IS NULL
        BEGIN
            ...
            CREATE INDEX IX_WorkflowDeployments_Status ON WorkflowDeployments (Status);
            CREATE INDEX IX_WorkflowDeployments_Target ON WorkflowDeployments (BaseWorkflowId, VersionNumber, Environment, RuntimeIdentifier);
            CREATE INDEX IX_WorkflowDeployments_Queue ON WorkflowDeployments (Status, CreatedAt, Id);
            CREATE INDEX IX_WorkflowDeployments_Created ON WorkflowDeployments (CreatedAt, Id);
        END;
        """;
}