...
public sealed class SqlServerWorkflowStateStore : IWorkflowStateStore, IWorkflowWaitIndex, IWorkflowDispatchIndex, IWorkflowLeaseAdministration, ISupportsRowSecurityFilter
{
    ...
    public async ValueTask<WorkflowLease?> AcquireLeaseAsync(WorkflowRunAddress address, string owner, TimeSpan ttl, CancellationToken cancellationToken)
    {
        ...
        // and every restart (ADR 0065 §6), and OUTPUT reports the granted value in the same statement that mints it — a
        // separate read could not tell this grant's epoch from the next one's.
        merge.CommandText =
            """
        ...
        object? granted = await merge.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<WorkflowRunPage> QueryAsync(WorkflowQuery query, CancellationToken cancellationToken)
    {
        ...
        select.CommandText =
            $"""
            SELECT TOP (@limit) environment, run_id, status, workflow_id, created_at, updated_at, due_at, awaiting_channel, awaiting_correlation_id, error_type, correlation_id, tags
            FROM workflow_runs
            WHERE {filter}
            ...
            """;
        select.Parameters.Add(NullableText("@after_environment", after?.Environment));
        select.Parameters.Add(NullableText("@after_id", after?.RunId.Value));
    ...
    private static string BuildVisibilityFilter(SqlCommand command, in WorkflowQuery query)
    {
        ...
        command.Parameters.Add(NullableText("@schedule_id", ScheduleHostedWorkflow.ScheduleWorkflowId));
        ...
        if (!query.Tags.IsEmpty)
        {
            ...
            // same way and found by CHARINDEX under a binary collation, since the column's own collation folds case: a
            // queried tag is an exact member, as the document and KV backends match it. An unanchored LIKE matched
            // "production" for "prod" (P1-16). No LIKE metacharacters, so nothing to escape.
            List<string> tags = query.Tags.ToList();
            for (int i = 0; i < tags.Count; i++)
            {
                string name = "@tag" + i.ToString(CultureInfo.InvariantCulture);
                sql.Append(" AND CHARINDEX(").Append(name).Append(", tags COLLATE Latin1_General_BIN2) > 0");
    ...
    private static void BindRun(SqlCommand command, in WorkflowRunAddress address, Stream checkpoint, in WorkflowRunIndexEntry index)
    {
