...
public sealed class SqliteRunnerRegistry : IRunnerRegistry, IAsyncDisposable
{
    ...
    public async ValueTask<RunnerRegistryPage> ListAsync(AccessContext context, int limit, JsonString pageToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            // Reach (§14.2) is a per-row ABAC predicate, not expressible as SQL, so the runner_id PRIMARY KEY B-tree drives
            // an ordered range SEEK past the cursor and rows are streamed + reach-filtered in flight — the reader is consumed
            // only until the page fills, never the full SELECT + parse of every registration the in-memory fallback does.
            // (No LIMIT: filtered-out rows don't count toward the page, so the page-fill + one-row look-ahead governs how far
            // the scan reads.)
            using SqliteCommand select = this.connection.CreateCommand();
            select.CommandText =
                """
            ...
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                RunnerRegistration runner = RunnerRegistration.FromJson(reader.GetFieldValue<byte[]>(0));
