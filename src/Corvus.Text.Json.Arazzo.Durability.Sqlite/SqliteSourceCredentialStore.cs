...
public sealed class SqliteSourceCredentialStore : ISourceCredentialStore, IAsyncDisposable
{
    ...
    public async ValueTask<ParsedJsonDocument<SourceCredentialBinding>> AddAsync(SourceCredentialBinding draft, string actor, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // correlated EXISTS. Atomic; a re-tagging update rebuilds the mirror in step (see UpdateAsync).
    ...
    public async ValueTask<ParsedJsonDocument<SourceCredentialBinding>?> UpdateAsync(string sourceName, string environment, SourceCredentialBinding draft, WorkflowEtag expectedEtag, string actor, AccessContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // A draft that supplies management tags re-tags the binding's reach scope (a store-level replace; an
            // omitted set is carried forward), so the queryable mirror the list/count reach predicate reads is rebuilt
            // in the same transaction — left stale, the listing keeps deciding by the old tags and drifts from the get.
            if (!draft.ManagementTagsValue.IsEmpty)
            {
                ...
                await SyncSecurityTagsAsync(this.connection, transaction, sourceName, environment, tags!, draft.ManagementTagsValue, cancellationToken).ConfigureAwait(false);
            }
