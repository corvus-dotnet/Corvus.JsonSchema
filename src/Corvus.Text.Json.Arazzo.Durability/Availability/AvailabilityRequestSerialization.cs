// <copyright file="AvailabilityRequestSerialization.cs" company="Endjin Limited">
...
/// <summary>
/// Shared, pooled serialization for the <see cref="IAvailabilityRequestStore"/> implementations: every backend persists a
/// request as the same Corvus.Text.Json document, so "write a new request" / "carry an existing request forward under a
/// decision" live here once rather than per backend. Each method builds through a pooled scratch buffer
...
/// Mirrors <see cref="Security.AccessRequestSerialization"/>.
/// </summary>
public static class AvailabilityRequestSerialization
{
    ...
    public static byte[] SerializeNew(string id, AvailabilityRequest draft, string actor, DateTimeOffset createdAt, WorkflowEtag etag)
    ...
    public static byte[] SerializeDecision(in AvailabilityRequest existing, string id, WorkflowEtag expectedEtag, AvailabilityRequestDecision decision, string actor, DateTimeOffset decidedAt, WorkflowEtag etag)
    {
        EnsureEtag(id, expectedEtag, existing.EtagValue);
        return PersistedJson.ToArray(
            (Current: existing, decision, actor, decidedAt, etag),
            static (Utf8JsonWriter writer, in (AvailabilityRequest Current, AvailabilityRequestDecision Dec, string Actor, DateTimeOffset At, WorkflowEtag Tag) c)
    ...
    /// <returns>The pooled document that owns the persisted bytes.</returns>
    /// <exception cref="AvailabilityRequestConflictException">The expected etag no longer matches.</exception>
    public static ParsedJsonDocument<AvailabilityRequest> SerializeDecisionDoc(in AvailabilityRequest existing, string id, WorkflowEtag expectedEtag, AvailabilityRequestDecision decision, string actor, DateTimeOffset decidedAt, WorkflowEtag etag)
    {
        EnsureEtag(id, expectedEtag, existing.EtagValue);
        return PersistedJson.ToPooledDocument<AvailabilityRequest, (AvailabilityRequest Current, AvailabilityRequestDecision Dec, string Actor, DateTimeOffset At, WorkflowEtag Tag)>(
    ...
    /// — for a decision concurrency check. The <c>byte[]</c> parameter keeps the method-group delegate conversion the
    ...
    {
        using ParsedJsonDocument<AvailabilityRequest> current = ParsedJsonDocument<AvailabilityRequest>.Parse(document.AsMemory());
        return current.RootElement.EtagValue;
    }

    /// <summary>Throws <see cref="AvailabilityRequestConflictException"/> when a non-<see cref="WorkflowEtag.None"/> expected etag no longer matches.</summary>
    ...
    public static void EnsureEtag(string id, WorkflowEtag expected, WorkflowEtag actual)
    {
        ...
        {
            throw new AvailabilityRequestConflictException(id, expected);
        }
    }
}