...
public sealed class DraftWorkflowResumer : IRunExecutionBackend, IDisposable
{
    ...
    [System.Diagnostics.CodeAnalysis.UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "The draft runner loads a per-capture compiled executor by design, and runs only in in-process (non-AOT, non-trimmed) runner hosts. AOT execution backends do not use this resumer (ADR 0055).")]
    [System.Diagnostics.CodeAnalysis.UnconditionalSuppressMessage("AOT", "IL3050", Justification = "The draft runner loads a per-capture compiled executor by design, and runs only in in-process (non-AOT) runner hosts. AOT execution backends do not use this resumer (ADR 0055).")]
