// <copyright file="EnvironmentContinuationToken.cs" company="Endjin Limited">
...
/// <summary>
/// Encodes and decodes the opaque continuation token used to keyset-page <see cref="IEnvironmentStore.ListAsync"/>
/// results (design §7.7). Stores page by the stable total order <c>(name, tie-breaker)</c> — the contractual primary
/// order is <c>name</c>; the tie-breaker (the management-tag discriminator) makes it total so two reach-isolated
/// environments that share a name page deterministically. The token carries the last row's two key parts so the next
/// request can seek past it; it is opaque and <b>backend-scoped</b> (only ever presented back to the store that issued
/// it, so each backend may choose whatever tie-breaker it can order on).
/// </summary>
/// <remarks>
...
/// </remarks>
public static class EnvironmentContinuationToken
{
    ...
    // Key parts are short (environment names, tag discriminators), so the assemble/decode scratch fits the stack in the
