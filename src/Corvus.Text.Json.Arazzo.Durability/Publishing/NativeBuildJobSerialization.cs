...
public static class NativeBuildJobSerialization
{
    ...
    /// <param name="etag">The new record etag.</param>
    /// <returns>The owned UTF-8 JSON bytes.</returns>
    public static byte[] SerializeClaimed(in NativeBuildJob existing, string claimedBy, DateTimeOffset startedAt, DateTimeOffset leaseExpiresAt, WorkflowEtag etag)
        => PersistedJson.ToArray(
            (Current: existing, claimedBy, startedAt, leaseExpiresAt, etag),
            static (Utf8JsonWriter writer, in (NativeBuildJob Current, string ClaimedBy, DateTimeOffset At, DateTimeOffset Lease, WorkflowEtag Tag) c)
    ...
    /// <param name="etag">The new record etag.</param>
    /// <returns>The pooled document that owns the persisted bytes.</returns>
    public static ParsedJsonDocument<NativeBuildJob> SerializeClaimedDoc(in NativeBuildJob existing, string claimedBy, DateTimeOffset startedAt, DateTimeOffset leaseExpiresAt, WorkflowEtag etag)
        => PersistedJson.ToPooledDocument<NativeBuildJob, (NativeBuildJob Current, string ClaimedBy, DateTimeOffset At, DateTimeOffset Lease, WorkflowEtag Tag)>(
    ...
    /// <summary>Checks the etag and serializes a lease-renewed copy of a Building job to owned JSON bytes — the heartbeat
    /// that keeps a running build's lease live (ADR 0056). The etag is the ownership token: a stale expected etag means the
    /// job was reclaimed, so the renewal conflicts.</summary>
    /// <param name="existing">The stored job, already parsed by the backend leaf (read synchronously here).</param>
    /// <param name="id">The job id (for a conflict message).</param>
    ...
    /// <returns>The owned UTF-8 JSON bytes.</returns>
    /// <exception cref="NativeBuildJobConflictException">The expected etag no longer matches.</exception>
    public static byte[] SerializeRenewal(in NativeBuildJob existing, string id, WorkflowEtag expectedEtag, DateTimeOffset leaseExpiresAt, WorkflowEtag etag)
    {
        EnsureEtag(id, expectedEtag, existing.EtagValue);
        return PersistedJson.ToArray(
            (Current: existing, leaseExpiresAt, etag),
            static (Utf8JsonWriter writer, in (NativeBuildJob Current, DateTimeOffset Lease, WorkflowEtag Tag) c)
                => c.Current.WriteRenewedLease(writer, c.Lease, c.Tag));
    }

    /// <summary>Checks the etag and serializes a lease-renewed copy of a Building job into a pooled buffer the returned
    /// document owns (no GC array), for a driver that binds a <see cref="ReadOnlyMemory{T}"/> / stream.</summary>
    /// <param name="existing">The stored job, already parsed by the backend leaf (read synchronously here).</param>
    /// <param name="id">The job id (for a conflict message).</param>
    /// <param name="expectedEtag">The expected current etag (<see cref="WorkflowEtag.None"/> renews unconditionally).</param>
    /// <param name="leaseExpiresAt">The new instant the lease expires.</param>
    ...
    /// <returns>The pooled document that owns the persisted bytes.</returns>
    /// <exception cref="NativeBuildJobConflictException">The expected etag no longer matches.</exception>
    public static ParsedJsonDocument<NativeBuildJob> SerializeRenewalDoc(in NativeBuildJob existing, string id, WorkflowEtag expectedEtag, DateTimeOffset leaseExpiresAt, WorkflowEtag etag)
    {
        EnsureEtag(id, expectedEtag, existing.EtagValue);
        return PersistedJson.ToPooledDocument<NativeBuildJob, (NativeBuildJob Current, DateTimeOffset Lease, WorkflowEtag Tag)>(
