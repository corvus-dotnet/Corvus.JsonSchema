// <copyright file="WorkflowDeploymentContinuationToken.cs" company="Endjin Limited">
...
/// results (ADR 0055). The list pages oldest-first by <c>(createdAt, id)</c> — ascending creation instant with the unique
/// <c>id</c> as the tie-breaker — so the token carries the last row's <c>(createdAt, id)</c> for the next request to seek
/// strictly past. It is opaque and <b>backend-scoped</b> (only ever presented back to the store that issued it). Mirrors
/// the availability-request continuation token.
/// </summary>
/// <remarks>
...
/// </remarks>
public static class WorkflowDeploymentContinuationToken
{
    ...
    public static int EncodeToUtf8(long utcTicks, ReadOnlySpan<byte> idUtf8, Span<byte> destination)
    {
        ...
        try
        {
            ...
            {
                ThrowHelper.ThrowWorkflowDeploymentInstantNotFormatted();
            }
    ...
    public static bool TryDecode(ReadOnlySpan<byte> tokenUtf8, Span<byte> destination, out long utcTicks, out ReadOnlySpan<byte> idUtf8)
    {
        ...
        {
            ThrowHelper.ThrowWorkflowDeploymentTokenInvalidBase64();
        }
        ...
        {
            ThrowHelper.ThrowWorkflowDeploymentTokenMalformed();
        }
