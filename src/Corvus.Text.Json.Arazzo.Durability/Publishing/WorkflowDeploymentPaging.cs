// <copyright file="WorkflowDeploymentPaging.cs" company="Endjin Limited">
...
/// <summary>
/// Shared in-memory keyset paging of deployments, backing the default
/// <see cref="IWorkflowDeploymentStore.ListAsync(WorkflowDeploymentQuery, int, JsonString, System.Threading.CancellationToken)"/>
/// over a store's existing filtered read (ADR 0055). The filter (status / target tuple) is applied by
/// <see cref="IWorkflowDeploymentStore.ListAsync(WorkflowDeploymentQuery, System.Threading.CancellationToken)"/>; this only
/// orders the result oldest-first by <c>(createdAt, id)</c>, seeks strictly past the cursor, and bounds to the limit. A
...
/// </summary>
internal static class WorkflowDeploymentPaging
{
    /// <summary>The page size used when a caller passes a non-positive limit (the public store-contract default).</summary>
    internal const int DefaultPageSize = WorkflowDeploymentPage.DefaultPageSize;
    ...
    /// <param name="limit">The maximum deployments in the page (non-positive uses <see cref="DefaultPageSize"/>).</param>
    ...
    internal static WorkflowDeploymentPage PageInMemory(PooledDocumentList<WorkflowDeployment> filtered, int limit, JsonString pageToken)
    {
        ...
        var ordered = new List<WorkflowDeployment>(filtered.Count);
        ...
        var page = new PooledDocumentList<WorkflowDeployment>(Math.Min(pageSize, ordered.Count));
        try
        {
            ...
            if (pageToken.IsNotUndefined())
            {
                ...
                cursorBuffer = ArrayPool<byte>.Shared.Rent(WorkflowDeploymentContinuationToken.GetMaxDecodedLength(tokenUtf8.Span.Length));
                hasCursor = WorkflowDeploymentContinuationToken.TryDecode(tokenUtf8.Span, cursorBuffer, out cursorTicks, out cursorId);
            }

            bool hasMore = false;
            foreach (WorkflowDeployment deployment in ordered)
            {
                if (hasCursor && !After(deployment, cursorTicks, cursorId))
                {
                ...
                page.Add(PersistedJson.ToPooledDocument<WorkflowDeployment>(JsonMarshal.GetRawUtf8Value(deployment).Memory.Span));
            }
    ...
    // True when (deployment.createdAt, deployment.id) sorts strictly after the cursor in the (createdAt asc, id ordinal asc) order.
    private static bool After(WorkflowDeployment deployment, long cursorTicks, ReadOnlySpan<byte> cursorId)
    {
        long ticks = deployment.CreatedAtValue.UtcTicks;
        if (ticks != cursorTicks)
        {
            return ticks > cursorTicks;
        }

        using UnescapedUtf8JsonString id = deployment.Id.GetUtf8String();
