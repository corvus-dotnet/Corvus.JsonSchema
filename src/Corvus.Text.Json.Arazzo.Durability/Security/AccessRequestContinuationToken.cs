// <copyright file="AccessRequestContinuationToken.cs" company="Endjin Limited">
...
/// results (design §16.5). The approval queue pages oldest-first by <c>(createdAt, id)</c> — ascending creation instant
/// with the unique <c>id</c> as the tie-breaker — so the token carries the last row's <c>(createdAt, id)</c> for the next
/// request to seek strictly past. It is opaque and <b>backend-scoped</b> (only ever presented back to the store that
/// issued it).
/// </summary>
/// <remarks>
...
/// </remarks>
public static class AccessRequestContinuationToken
{
    ...
    public static int EncodeToUtf8(long utcTicks, ReadOnlySpan<byte> idUtf8, Span<byte> destination)
    {
        ...
        try
        {
            ...
            {
                throw new FormatException("The access-request instant could not be formatted into the page token.");
            }
    ...
    public static bool TryDecode(ReadOnlySpan<byte> tokenUtf8, Span<byte> destination, out long utcTicks, out ReadOnlySpan<byte> idUtf8)
    {
        ...
        {
            throw new FormatException("The access-request page token is not valid base64url.");
        }
        ...
        {
            throw new FormatException("The access-request page token is malformed.");
        }
