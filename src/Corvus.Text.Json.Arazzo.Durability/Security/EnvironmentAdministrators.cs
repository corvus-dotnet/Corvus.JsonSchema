// <copyright file="EnvironmentAdministrators.cs" company="Endjin Limited">
...
/// <summary>
/// The explicit, mutable administration record for a deployment environment (design §7.7): the set of <em>administrator
/// identities</em> entitled to govern the environment — manage it and approve promotions into it. Generated from
/// <c>Schemas/EnvironmentAdministrators.json</c> and used as the domain value <em>and</em> the persisted form.
/// </summary>
/// <remarks>
...
/// <para>The record is materialized when the environment is created — creating one grants the creator administration
/// (§7.7) — and mutated by add-administrator / transfer / remove. The administrator set is never empty: the last
/// administrator cannot be removed. This mirrors <see cref="WorkflowAdministrators"/>, keyed by environment name.</para>
/// </remarks>
[JsonSchemaTypeGenerator("../Schemas/EnvironmentAdministrators.json")]
public readonly partial struct EnvironmentAdministrators
{
    /// <summary>Gets the environment name this record governs.</summary>
    public string EnvironmentNameValue => (string)this.EnvironmentName;
    ...
    /// <summary>Whether <paramref name="candidate"/> administers this environment (design §16.5.4, membership model): true
    /// iff one of the administrator identities is a <strong>subset</strong> of <paramref name="candidate"/> — the caller's
    /// whole stamped identity <em>contains</em> a named administrator identity. Membership supersedes the earlier exact
    ...
    /// <returns><see langword="true"/> if the candidate administers this environment.</returns>
    ...
    public static EnvironmentAdministrators FromJson(ReadOnlyMemory<byte> utf8) => ParseValue(utf8.Span);
    ...
    public byte[] ToJsonBytes()
        => PersistedJson.ToArray(this, static (Utf8JsonWriter writer, in EnvironmentAdministrators v) => v.WriteTo(writer));
    ...
    /// <param name="environmentName">The environment the record administers.</param>
    ...
    public static ParsedJsonDocument<EnvironmentAdministrators> CreateNew(string environmentName, IReadOnlyList<AdministratorIdentity> administrators, string actor, DateTimeOffset createdAt, WorkflowEtag etag)
        => Create(
    ...
    /// <param name="createdAt">The creation instant.</param>
    /// <param name="etag">The optimistic-concurrency token to assign.</param>
    public static void WriteNew(Utf8JsonWriter writer, string environmentName, IReadOnlyList<AdministratorIdentity> administrators, string actor, DateTimeOffset createdAt, WorkflowEtag etag)
    {
        writer.WriteStartObject();
        writer.WriteString(JsonPropertyNames.EnvironmentNameUtf8, environmentName);
    ...
    {
        writer.WriteStartObject();
        writer.WriteString(JsonPropertyNames.EnvironmentNameUtf8, this.EnvironmentNameValue);
        WriteAdministrators(writer, administrators);
