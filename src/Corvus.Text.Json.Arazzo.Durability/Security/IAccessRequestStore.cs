...
public interface IAccessRequestStore
{
    ...
    /// <summary>Counts requests matching a filter, bounded by <paramref name="cap"/> (design §16.5 — work badges and list
    /// footers): the same <paramref name="query"/> reach + filter as the paged list, but returning only a bounded total,
    /// never rows. The default implementation counts over <see cref="ListAsync(AccessRequestQuery, CancellationToken)"/>;
    ...
    /// <param name="query">The filter (all criteria optional) — the same query the list uses, carrying the §14.2 reach.</param>
    ...
    async ValueTask<(int Count, bool Capped)> CountAsync(AccessRequestQuery query, int cap, CancellationToken cancellationToken)
    {
        using PooledDocumentList<AccessRequest> filtered = await this.ListAsync(query, cancellationToken).ConfigureAwait(false);
