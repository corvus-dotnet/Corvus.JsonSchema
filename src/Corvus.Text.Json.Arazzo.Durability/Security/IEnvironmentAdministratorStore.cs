...
public interface IEnvironmentAdministratorStore
{
    ...
    /// <summary>Lists the environment names the caller's identity administers under the <strong>membership</strong> model
    /// (design §16.5.4): an environment appears iff one of its administrator identities is a <em>subset</em> of the caller's
    ...
    /// <c>environmentName</c>. An empty <paramref name="adminDigests"/> (the empty identity) administers nothing.</summary>
    ...
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>One keyset page of administered environment names (the DISTINCT union across the digests, ordered by <c>environmentName</c>), as a disposable the caller must dispose.</returns>
