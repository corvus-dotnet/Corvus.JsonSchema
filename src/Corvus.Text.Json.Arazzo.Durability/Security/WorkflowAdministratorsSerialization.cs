...
public static class WorkflowAdministratorsSerialization
{
    ...
    public static ParsedJsonDocument<WorkflowAdministrators> SerializeNewDoc(string baseWorkflowId, IReadOnlyList<SecurityTagSet> administrators, string actor, DateTimeOffset createdAt, WorkflowEtag etag)
        => PersistedJson.ToPooledDocument<WorkflowAdministrators, (string Id, IReadOnlyList<SecurityTagSet> Administrators, string Actor, DateTimeOffset At, WorkflowEtag Tag)>(
            (baseWorkflowId, administrators, actor, createdAt, etag),
            static (Utf8JsonWriter writer, in (string Id, IReadOnlyList<SecurityTagSet> Administrators, string Actor, DateTimeOffset At, WorkflowEtag Tag) c)
                => WorkflowAdministrators.WriteNew(writer, c.Id, c.Administrators, c.Actor, c.At, c.Tag));
    ...
    /// <param name="etag">The new record etag.</param>
    /// <returns>The owned UTF-8 JSON bytes.</returns>
    public static byte[] SerializeUpdated(in WorkflowAdministrators existing, IReadOnlyList<SecurityTagSet> administrators, string actor, DateTimeOffset updatedAt, WorkflowEtag etag)
        => PersistedJson.ToArray(
    ...
    /// <param name="etag">The new record etag.</param>
    /// <returns>The pooled document that owns the persisted bytes.</returns>
    public static ParsedJsonDocument<WorkflowAdministrators> SerializeUpdatedDoc(in WorkflowAdministrators existing, IReadOnlyList<SecurityTagSet> administrators, string actor, DateTimeOffset updatedAt, WorkflowEtag etag)
        => PersistedJson.ToPooledDocument<WorkflowAdministrators, (WorkflowAdministrators Current, IReadOnlyList<SecurityTagSet> Administrators, string Actor, DateTimeOffset At, WorkflowEtag Tag)>(
