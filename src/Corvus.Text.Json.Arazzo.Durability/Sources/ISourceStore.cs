...
public interface ISourceStore
{
    ...
    /// <summary>Counts the sources the caller's read reach admits, bounded by <paramref name="cap"/> (for list footers):
    /// the same reach as <see cref="ListAsync"/>, but returning only a bounded total, never rows. The default reads one
    /// bounded page of at most <paramref name="cap"/> + 1 visible rows and counts them; a backend that can push the reach
    /// into its query overrides it with a native bounded <c>COUNT</c>.</summary>
    /// <param name="context">The caller's row-access grant (use <see cref="AccessContext.System"/> for full reach).</param>
    ...
    {
        using SourcePage page = await this.ListAsync(context, cap + 1, default, cancellationToken).ConfigureAwait(false);
        int count = page.Sources.Count;
