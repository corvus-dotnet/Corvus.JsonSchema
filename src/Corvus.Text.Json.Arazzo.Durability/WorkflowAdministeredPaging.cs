// <copyright file="WorkflowAdministeredPaging.cs" company="Endjin Limited">
...
/// <summary>
/// Shared helpers for the reverse administration index (design §15.4) that every <see cref="IWorkflowAdministratorStore"/>
/// implementation uses, so the digest keys and the keyset page boundary are computed identically across all backends.
/// </summary>
public static class WorkflowAdministeredPaging
{
    /// <summary>Computes the distinct collision-probe digests of an administrator set — the reverse-index keys for a base
    /// id. The empty identity has no digest and is skipped. Two identities are set-equal iff their digests are equal
    /// (<see cref="SecurityIdentityDigest"/>), so these are exactly the digests the forward
    /// <see cref="WorkflowAdministrators.IsAdministeredBy"/> compares — the index can never over-grant or miss a match.</summary>
    /// <param name="administrators">The administrator identities written for a base id.</param>
    /// <returns>The distinct administrator digests (order as first seen), for the index write path.</returns>
    public static IReadOnlyList<string> DistinctDigests(IReadOnlyList<WorkflowAdministrators.AdministratorIdentity> administrators)
    {
    ...
    /// <param name="rows">The base ids read for the page, ordered by base id; at most <paramref name="pageSize"/> + 1.</param>
    ...
    public static WorkflowAdministeredPage ToPage(IReadOnlyList<string> rows, int pageSize)
    {
        ArgumentNullException.ThrowIfNull(rows);
        if (rows.Count <= pageSize)
        {
            return WorkflowAdministeredPage.Create(rows);
        }
        ...
        try
        {
            ...
            return WorkflowAdministeredPage.Create(page, scratch.AsSpan(0, written));
        }
        finally
        {
