// <copyright file="WorkspaceWorkflowContinuationToken.cs" company="Endjin Limited">
...
/// <summary>
/// Encodes and decodes the opaque continuation token used to keyset-page <see cref="IWorkspaceWorkflowStore.ListAsync"/> results
/// (design §7.6). Stores page by the stable total order <c>(id, tie-breaker)</c> — the contractual primary order is
/// <c>id</c>; the tie-breaker (the management-tag discriminator) makes it total so the order stays total even for backends whose tie-breaker is meaningful (the id is already unique here, so the tie-breaker is empty). The token carries the last row's two key parts so the next request can seek
...
/// </summary>
/// <remarks>
/// The token is <c>base64url(id \0 tie-breaker)</c>. The <see cref="ReadOnlySpan{T}"/> overloads are the warm seam:
...
/// </remarks>
public static class WorkspaceWorkflowContinuationToken
{
    ...
    // Key parts are short (working-copy ids, tag discriminators), so the assemble/decode scratch fits the stack in the
    ...
    /// <param name="name">The last row's working-copy id.</param>
    ...
    /// <c>(id, tie-breaker)</c> cursor to page strictly after — the warm path's bytes-native decode.</summary>
