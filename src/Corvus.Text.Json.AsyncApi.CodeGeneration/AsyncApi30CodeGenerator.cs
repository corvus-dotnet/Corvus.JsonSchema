...
public sealed class AsyncApi30CodeGenerator
{
    ...
    private readonly bool generateMessageDeliveryContext;
        ...
        bool generateMessageDeliveryContext = false)
    {
        ...
        this.generateMessageDeliveryContext = generateMessageDeliveryContext;
    }
        ...
        if (op.Messages.Count == 1)
        {
            ...
            if (msg.HeadersTypeName is not null)
            {
                if (this.generateMessageDeliveryContext)
                {
                    ...
                    w.WriteLine($"/// <param name=\"context\">Runtime metadata for the delivered message.</param>");
                    w.WriteLine($"/// <param name=\"cancellationToken\">A cancellation token.</param>");
                    w.WriteLine($"{returnType} {methodName}({payloadType} payload, {msg.HeadersTypeName} headers, MessageDeliveryContext context, CancellationToken cancellationToken = default);");
                }
                else
                {
                    w.WriteLine($"/// <param name=\"headers\">The deserialized message headers.</param>");
    ...
    private GeneratedFile EmitConsumer(OperationInfo op)
    {
        ...
        // subscribed with the protocol-specific metadata its specification declared.
        ...
        void EmitStartBody(bool async)
        {
            ...
            bool useDeliveryContext = this.generateMessageDeliveryContext;
            bool hasBindingContext = op.ChannelBindingsJson is not null || op.OperationBindingsJson is not null;
            if (useDeliveryContext || hasBindingContext)
            {
                w.WriteLine("MessageContext context = new()");
                w.OpenBrace();
                if (op.ChannelBindingsJson is not null)
        ...
        if (IsResponderOperation(op))
        {
            ...
            if (msg.HeadersTypeName is not null)
            {
                w.WriteLine(this.generateMessageDeliveryContext
                    ? $"return this.handler.{handlerMethod}(payload, {msg.HeadersTypeName}.From(headers), deliveryContext, cancellationToken);"
                    : $"return this.handler.{handlerMethod}(payload, {msg.HeadersTypeName}.From(headers), cancellationToken);");
            }
            else
            {
                w.WriteLine(this.generateMessageDeliveryContext
                    ? $"return this.handler.{handlerMethod}(payload, deliveryContext, cancellationToken);"
                    : $"return this.handler.{handlerMethod}(payload, cancellationToken);");
            }
        ...
        else if (op.Messages.Count == 1)
        {
            ...
            if (msg.HeadersTypeName is not null)
            {
                ...
                w.WriteLine(this.generateMessageDeliveryContext
                    ? $"await this.handler.{handlerMethod}(payload, h, deliveryContext, cancellationToken).ConfigureAwait(false);"
                    : $"await this.handler.{handlerMethod}(payload, h, cancellationToken).ConfigureAwait(false);");
            }
            else
            {
                w.WriteLine(this.generateMessageDeliveryContext
                    ? $"await this.handler.{handlerMethod}(payload, deliveryContext, cancellationToken).ConfigureAwait(false);"
                    : $"await this.handler.{handlerMethod}(payload, cancellationToken).ConfigureAwait(false);");
            }
