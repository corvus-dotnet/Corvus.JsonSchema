...
public readonly struct ConditionalCodeSpecification
{
    ...
    private static string GetCondition(FrameworkType condition)
    {
        return condition switch
        {
            ...
            _ => throw new InvalidOperationException(SR.Format(System.Globalization.CultureInfo.InvariantCulture, SR.UnsupportedFrameworkType, condition)),
        };
    }
        ...
        List<ConditionalCodeSpecification> net80OrGreater)
    {
        foreach (ConditionalCodeSpecification spec in conditionalSpecifications)
        {
            switch (spec.condition)
            {
                ...
                default:
                    throw new NotSupportedException(SR.Format(System.Globalization.CultureInfo.InvariantCulture, SR.UnsupportedFrameworkType, spec.condition));
            }
        }
    }
}