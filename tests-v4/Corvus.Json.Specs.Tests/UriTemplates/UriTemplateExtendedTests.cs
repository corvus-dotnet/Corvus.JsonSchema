...
public class UriTemplateExtendedTests
{
    ...
    [InlineData("{/id*}{?fields,first_name,last.name,token}", """["/person?fields=id,name,picture\u0026first_name=John\u0026last.name=Doe\u0026token=12345","/person?fields=id,picture,name\u0026first_name=John\u0026last.name=Doe\u0026token=12345","/person?fields=picture,name,id\u0026first_name=John\u0026last.name=Doe\u0026token=12345","/person?fields=picture,id,name\u0026first_name=John\u0026last.name=Doe\u0026token=12345","/person?fields=name,picture,id\u0026first_name=John\u0026last.name=Doe\u0026token=12345","/person?fields=name,id,picture\u0026first_name=John\u0026last.name=Doe\u0026token=12345"]""")]
    [InlineData("/search.{format}{?q,geocode,lang,locale,page,result_type}", """["/search.json?q=URI%20Templates\u0026geocode=37.76,-122.427\u0026lang=en\u0026page=5","/search.json?q=URI%20Templates\u0026geocode=-122.427,37.76\u0026lang=en\u0026page=5"]""")]
    [InlineData("/test{/Some%20Thing}", """["/test/foo"]""")]
    [InlineData("/set{?number}", """["/set?number=6"]""")]
    [InlineData("/loc{?long,lat}", """["/loc?long=37.76\u0026lat=-122.427"]""")]
    [InlineData("/base{/group_id,first_name}/pages{/page,lang}{?format,q}", """["/base/12345/John/pages/5/en?format=json\u0026q=URI%20Templates"]""")]
    [InlineData("/sparql{?query}", """["/sparql?query=PREFIX%20dc%3A%20%3Chttp%3A%2F%2Fpurl.org%2Fdc%2Felements%2F1.1%2F%3E%20SELECT%20%3Fbook%20%3Fwho%20WHERE%20%7B%20%3Fbook%20dc%3Acreator%20%3Fwho%20%7D"]""")]
    [InlineData("/go{?uri}", """["/go?uri=http%3A%2F%2Fexample.org%2F%3Furi%3Dhttp%253A%252F%252Fexample.org%252F"]""")]
    [InlineData("/service{?word}", """["/service?word=dr%C3%BCcken"]""")]
    [InlineData("/lookup{?Stra%C3%9Fe}", """["/lookup?Stra%C3%9Fe=Gr%C3%BCner%20Weg"]""")]
    [InlineData("{random}", """["%C5%A1%C3%B6%C3%A4%C5%B8%C5%93%C3%B1%C3%AA%E2%82%AC%C2%A3%C2%A5%E2%80%A1%C3%91%C3%92%C3%93%C3%94%C3%95%C3%96%C3%97%C3%98%C3%99%C3%9A%C3%A0%C3%A1%C3%A2%C3%A3%C3%A4%C3%A5%C3%A6%C3%A7%C3%BF"]""")]
    [InlineData("{?assoc_special_chars*}", """["?%C5%A1%C3%B6%C3%A4%C5%B8%C5%93%C3%B1%C3%AA%E2%82%AC%C2%A3%C2%A5%E2%80%A1%C3%91%C3%92%C3%93%C3%94%C3%95=%C3%96%C3%97%C3%98%C3%99%C3%9A%C3%A0%C3%A1%C3%A2%C3%A3%C3%A4%C3%A5%C3%A6%C3%A7%C3%BF"]""")]
    ...
    [Theory]
    [InlineData("{/empty_list}", """[""]""")]
    [InlineData("{/empty_list*}", """[""]""")]
    [InlineData("{?empty_list}", """[""]""")]
    [InlineData("{?empty_list*}", """[""]""")]
    [InlineData("{?empty_assoc}", """[""]""")]
    [InlineData("{?empty_assoc*}", """[""]""")]
    ...
    [Theory]
    [InlineData("{42}", """["The%20Answer%20to%20the%20Ultimate%20Question%20of%20Life%2C%20the%20Universe%2C%20and%20Everything"]""")]
    [InlineData("{?42}", """["?42=The%20Answer%20to%20the%20Ultimate%20Question%20of%20Life%2C%20the%20Universe%2C%20and%20Everything"]""")]
    [InlineData("{1337}", """["leet,as,it,can,be"]""")]
    [InlineData("{?1337*}", """["?1337=leet\u00261337=as\u00261337=it\u00261337=can\u00261337=be"]""")]
    [InlineData("{?german*}", """["?11=elf\u002612=zw%C3%B6lf","?12=zw%C3%B6lf\u002611=elf"]""")]
    ...
    [Theory]
    [InlineData("{?id,token,keys*}", """["?id=admin&token=12345&key1=val1&key2=val2","?id=admin&token=12345&key2=val2&key1=val1"]""")]
    [InlineData("{/id}{?token,keys*}", """["/admin?token=12345&key1=val1&key2=val2", "/admin?token=12345&key2=val2&key1=val1"]""")]
    [InlineData("{?id,token}{&keys*}", """["?id=admin&token=12345&key1=val1&key2=val2","?id=admin&token=12345&key2=val2&key1=val1"]""")]
    [InlineData("/user{/id}{?token,tab}{&keys*}", """["/user/admin?token=12345&tab=overview&key1=val1&key2=val2", "/user/admin?token=12345&tab=overview&key2=val2&key1=val1"]""")]
    ...
    [Theory]
    [InlineData("{+id}", """["admin%2F"]""")]
    [InlineData("{#id}", """["#admin%2F"]""")]
    [InlineData("{id}", """["admin%252F"]""")]
    [InlineData("{+not_pct}", """["%25foo"]""")]
    [InlineData("{#not_pct}", """["#%25foo"]""")]
    [InlineData("{not_pct}", """["%25foo"]""")]
    [InlineData("{+list}", """["red%25,%2Fgreen,blue%20"]""")]
    [InlineData("{#list}", """["#red%25,%2Fgreen,blue%20"]""")]
    [InlineData("{list}", """["red%2525,%252Fgreen,blue%20"]""")]
    [InlineData("{+keys}", """["key1,val1%2F,key2,val2%2F"]""")]
    [InlineData("{#keys}", """["#key1,val1%2F,key2,val2%2F"]""")]
    [InlineData("{keys}", """["key1,val1%252F,key2,val2%252F"]""")]
