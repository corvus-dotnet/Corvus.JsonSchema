// <copyright file="AzureStorageDraftRunStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared draft-run-store conformance suite against <see cref="AzureStorageDraftRunStore"/> over the
/// Azurite emulator in a container. Each test gets an empty store (the container's blobs and the table's entities
/// are cleared).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class AzureStorageDraftRunStoreConformanceTests : DraftRunStoreConformance
{
    ...
    protected override async ValueTask<IDraftRunStore> CreateStoreAsync()
    {
        ...
        await AzureStorageDraftRunStore.PrepareAsync(blobService, tableService);

        BlobContainerClient packages = blobService.GetBlobContainerClient("arazzo-draftruns");
        ...
        TableClient captures = tableService.GetTableClient("arazzodraftruns");
        try
        {
            await foreach (TableEntity entity in captures.QueryAsync<TableEntity>())
            {
                await captures.DeleteEntityAsync(entity.PartitionKey, entity.RowKey, ETag.All);
            }
        }
        ...
        return await AzureStorageDraftRunStore.ConnectAsync(blobService, tableService);
    }
}