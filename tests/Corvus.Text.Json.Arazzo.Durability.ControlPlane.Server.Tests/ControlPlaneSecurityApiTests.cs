...
public sealed class ControlPlaneSecurityApiTests
{
    ...
    private static async Task<Scoped> StartAsync()
    {
        ...
        app.UseAuthentication();
        app.UseAuthorization();
        app.MapArazzoControlPlane(management, catalog, new InMemoryRunnerRegistry(), ControlPlaneSecurityMode.ScopesOnly, securityPolicyStore: new InMemorySecurityPolicyStore(), auditor: GovernanceAuditor.CreateInMemory());
        await app.StartAsync();
