// <copyright file="MongoAccessRequestStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="AccessRequestStoreConformance"/> suite against <see cref="MongoAccessRequestStore"/>
/// over a real MongoDB server in a container. Each test gets an empty store (the database is dropped).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MongoAccessRequestStoreConformanceTests : AccessRequestStoreConformance
{
    ...
    protected override async ValueTask<IAccessRequestStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        return await MongoAccessRequestStore.ConnectAsync(client, DatabaseName, timeProvider);
    }
}