// <copyright file="MongoObservedIdentityStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="ObservedIdentityStoreConformance"/> suite against <see cref="MongoObservedIdentityStore"/>
...
[TestCategory("docker")]
public sealed class MongoObservedIdentityStoreConformanceTests : ObservedIdentityStoreConformance
{
    ...
    {
        await client.GetDatabase(DatabaseName).DropCollectionAsync("observed_identities");

        // Provision the indexes then open for operation over the caller-owned client.
        await MongoObservedIdentityStore.PrepareAsync(client, DatabaseName);
        return await MongoObservedIdentityStore.ConnectAsync(client, DatabaseName, timeProvider);
    }
}