// <copyright file="MySqlAccessRequestStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="AccessRequestStoreConformance"/> suite against <see cref="MySqlAccessRequestStore"/>
/// over a real MySQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlAccessRequestStoreConformanceTests : AccessRequestStoreConformance
{
    ...
    protected override async ValueTask<IAccessRequestStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlAccessRequestStore.PrepareAsync(dataSource);
        return await MySqlAccessRequestStore.ConnectAsync(dataSource, timeProvider);
    }
}