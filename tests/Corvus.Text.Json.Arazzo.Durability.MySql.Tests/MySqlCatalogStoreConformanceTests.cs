// <copyright file="MySqlCatalogStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared catalog-conformance suite against <see cref="MySqlWorkflowCatalogStore"/> over a real MySQL
...
[TestCategory("docker")]
public sealed class MySqlCatalogStoreConformanceTests : WorkflowCatalogStoreConformance
{
    ...
    protected override async ValueTask<IWorkflowCatalogStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlWorkflowCatalogStore.PrepareAsync(dataSource);
        return await MySqlWorkflowCatalogStore.ConnectAsync(dataSource, timeProvider);
    }
}