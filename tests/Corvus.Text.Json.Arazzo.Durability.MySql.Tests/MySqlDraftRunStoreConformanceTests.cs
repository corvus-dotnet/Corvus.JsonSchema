// <copyright file="MySqlDraftRunStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared draft-run-store conformance suite against <see cref="MySqlDraftRunStore"/> over a real MySQL
...
[TestCategory("docker")]
public sealed class MySqlDraftRunStoreConformanceTests : DraftRunStoreConformance
{
    ...
    protected override async ValueTask<IDraftRunStore> CreateStoreAsync()
    {
        ...
        await MySqlDraftRunStore.PrepareAsync(dataSource);
        return await MySqlDraftRunStore.ConnectAsync(dataSource);
    }
}