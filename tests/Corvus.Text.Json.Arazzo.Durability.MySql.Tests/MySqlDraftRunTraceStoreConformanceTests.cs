// <copyright file="MySqlDraftRunTraceStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared draft-run-trace-store conformance suite against <see cref="MySqlDraftRunTraceStore"/> over a real
/// MySQL server in a container, exercising the data-source (caller-owned client) overloads. Each test gets an empty
/// store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlDraftRunTraceStoreConformanceTests : DraftRunTraceStoreConformance
{
    ...
    protected override async ValueTask<IDraftRunTraceStore> CreateStoreAsync()
    {
        await using (MySqlConnection connection = await dataSource.OpenConnectionAsync())
        {
            ...
            reset.CommandText = "DROP TABLE IF EXISTS draft_run_traces;";
            await reset.ExecuteNonQueryAsync();
        }

        // Provision (DDL) then open for operation (no DDL) over the caller-owned data source.
        await MySqlDraftRunTraceStore.PrepareAsync(dataSource);
        return await MySqlDraftRunTraceStore.ConnectAsync(dataSource);
    }
}