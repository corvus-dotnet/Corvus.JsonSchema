// <copyright file="MySqlEnvironmentAdministratorStoreConformanceTests.cs" company="Endjin Limited">
...
/// <see cref="MySqlEnvironmentAdministratorStore"/> over a real MySQL server in a container. Each test gets an empty store
/// (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlEnvironmentAdministratorStoreConformanceTests : EnvironmentAdministratorStoreConformance
{
    ...
    protected override async ValueTask<IEnvironmentAdministratorStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlEnvironmentAdministratorStore.PrepareAsync(dataSource);
        return await MySqlEnvironmentAdministratorStore.ConnectAsync(dataSource, timeProvider);
    }
}