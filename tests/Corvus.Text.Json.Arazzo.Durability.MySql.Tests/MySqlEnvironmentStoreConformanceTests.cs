// <copyright file="MySqlEnvironmentStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="EnvironmentStoreConformance"/> suite against <see cref="MySqlEnvironmentStore"/> over a
/// real MySQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlEnvironmentStoreConformanceTests : EnvironmentStoreConformance
{
    ...
    protected override async ValueTask<IEnvironmentStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlEnvironmentStore.PrepareAsync(dataSource);
        return await MySqlEnvironmentStore.ConnectAsync(dataSource, timeProvider);
    }
}