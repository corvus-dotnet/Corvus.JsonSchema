// <copyright file="MySqlNativeBuildJobStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="NativeBuildJobStoreConformance"/> suite against <see cref="MySqlNativeBuildJobStore"/> over a
/// real MySQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlNativeBuildJobStoreConformanceTests : NativeBuildJobStoreConformance
{
    ...
    protected override async ValueTask<INativeBuildJobStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlNativeBuildJobStore.PrepareAsync(dataSource);
        return await MySqlNativeBuildJobStore.ConnectAsync(dataSource, timeProvider);
    }
}