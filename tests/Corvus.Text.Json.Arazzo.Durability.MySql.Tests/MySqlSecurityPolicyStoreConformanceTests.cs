// <copyright file="MySqlSecurityPolicyStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="SecurityPolicyStoreConformance"/> suite against <see cref="MySqlSecurityPolicyStore"/>
/// over a real MySQL server in a container. Each test gets an empty store (the tables are dropped and
/// re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlSecurityPolicyStoreConformanceTests : SecurityPolicyStoreConformance
{
    ...
    protected override async ValueTask<ISecurityPolicyStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlSecurityPolicyStore.PrepareAsync(dataSource);
        return await MySqlSecurityPolicyStore.ConnectAsync(dataSource, timeProvider);
    }
}