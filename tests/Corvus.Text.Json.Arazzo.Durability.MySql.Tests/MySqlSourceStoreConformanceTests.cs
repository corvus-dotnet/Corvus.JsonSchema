// <copyright file="MySqlSourceStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="SourceStoreConformance"/> suite against <see cref="MySqlSourceStore"/> over a
/// real MySQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlSourceStoreConformanceTests : SourceStoreConformance
{
    ...
    protected override async ValueTask<ISourceStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlSourceStore.PrepareAsync(dataSource);
        return await MySqlSourceStore.ConnectAsync(dataSource, timeProvider);
    }
}