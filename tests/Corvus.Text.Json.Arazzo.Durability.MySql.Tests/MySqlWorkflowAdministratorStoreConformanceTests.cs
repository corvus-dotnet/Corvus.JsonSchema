// <copyright file="MySqlWorkflowAdministratorStoreConformanceTests.cs" company="Endjin Limited">
...
/// <see cref="MySqlWorkflowAdministratorStore"/> over a real MySQL server in a container. Each test gets an empty store
/// (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlWorkflowAdministratorStoreConformanceTests : WorkflowAdministratorStoreConformance
{
    ...
    protected override async ValueTask<IWorkflowAdministratorStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlWorkflowAdministratorStore.PrepareAsync(dataSource);
        return await MySqlWorkflowAdministratorStore.ConnectAsync(dataSource, timeProvider);
    }
}