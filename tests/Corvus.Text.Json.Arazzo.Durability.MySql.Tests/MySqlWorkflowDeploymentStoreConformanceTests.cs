// <copyright file="MySqlWorkflowDeploymentStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="WorkflowDeploymentStoreConformance"/> suite against <see cref="MySqlWorkflowDeploymentStore"/>
/// over a real MySQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlWorkflowDeploymentStoreConformanceTests : WorkflowDeploymentStoreConformance
{
    ...
    protected override async ValueTask<IWorkflowDeploymentStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlWorkflowDeploymentStore.PrepareAsync(dataSource);
        return await MySqlWorkflowDeploymentStore.ConnectAsync(dataSource, timeProvider);
    }
}