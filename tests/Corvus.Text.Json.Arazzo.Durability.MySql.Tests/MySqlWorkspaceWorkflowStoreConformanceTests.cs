// <copyright file="MySqlWorkspaceWorkflowStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="WorkspaceWorkflowStoreConformance"/> suite against <see cref="MySqlWorkspaceWorkflowStore"/>
/// over a real MySQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class MySqlWorkspaceWorkflowStoreConformanceTests : WorkspaceWorkflowStoreConformance
{
    ...
    protected override async ValueTask<IWorkspaceWorkflowStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await MySqlWorkspaceWorkflowStore.PrepareAsync(dataSource);
        return await MySqlWorkspaceWorkflowStore.ConnectAsync(dataSource, timeProvider);
    }
}