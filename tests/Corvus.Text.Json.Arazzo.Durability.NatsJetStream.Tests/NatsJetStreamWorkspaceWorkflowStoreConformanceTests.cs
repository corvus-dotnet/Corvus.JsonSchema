// <copyright file="NatsJetStreamWorkspaceWorkflowStoreConformanceTests.cs" company="Endjin Limited">
...
/// <see cref="NatsJetStreamWorkspaceWorkflowStore"/> over a real NATS server (with JetStream) in a container. Each test
/// gets an empty store (the KV bucket is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class NatsJetStreamWorkspaceWorkflowStoreConformanceTests : WorkspaceWorkflowStoreConformance
{
    ...
    protected override async ValueTask<IWorkspaceWorkflowStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        try
        {
            await kv.DeleteStoreAsync("arazzo_workspace_workflows");
        }
        ...
        await NatsJetStreamWorkspaceWorkflowStore.PrepareAsync(connection);
        return await NatsJetStreamWorkspaceWorkflowStore.ConnectAsync(connection, timeProvider);
    }
}