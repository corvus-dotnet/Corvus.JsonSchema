// <copyright file="PostgresEnvironmentStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="EnvironmentStoreConformance"/> suite against <see cref="PostgresEnvironmentStore"/> over a
/// real PostgreSQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class PostgresEnvironmentStoreConformanceTests : EnvironmentStoreConformance
{
    ...
    protected override async ValueTask<IEnvironmentStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        return await PostgresEnvironmentStore.ConnectAsync(dataSource, timeProvider);
    }
}