// <copyright file="PostgresNativeBuildJobStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="NativeBuildJobStoreConformance"/> suite against <see cref="PostgresNativeBuildJobStore"/> over a
/// real PostgreSQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class PostgresNativeBuildJobStoreConformanceTests : NativeBuildJobStoreConformance
{
    ...
    protected override async ValueTask<INativeBuildJobStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await PostgresNativeBuildJobStore.PrepareAsync(dataSource);
        return await PostgresNativeBuildJobStore.ConnectAsync(dataSource, timeProvider);
    }
}