// <copyright file="PostgresSourceStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="SourceStoreConformance"/> suite against <see cref="PostgresSourceStore"/> over a
/// real PostgreSQL server in a container. Each test gets an empty store (the table is dropped and re-provisioned).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class PostgresSourceStoreConformanceTests : SourceStoreConformance
{
    ...
    protected override async ValueTask<ISourceStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        return await PostgresSourceStore.ConnectAsync(dataSource, timeProvider);
    }
}