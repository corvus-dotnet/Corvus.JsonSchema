// <copyright file="RedisAccessRequestStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="AccessRequestStoreConformance"/> suite against <see cref="RedisAccessRequestStore"/>
/// over a real Redis server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisAccessRequestStoreConformanceTests : AccessRequestStoreConformance
{
    ...
    protected override async ValueTask<IAccessRequestStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisAccessRequestStore.PrepareAsync(configuration);
        return await RedisAccessRequestStore.ConnectAsync(configuration, timeProvider);
    }
}