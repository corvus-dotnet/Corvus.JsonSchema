// <copyright file="RedisAvailabilityStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="AvailabilityStoreConformance"/> suite against <see cref="RedisAvailabilityStore"/> over a
/// real Redis server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisAvailabilityStoreConformanceTests : AvailabilityStoreConformance
{
    ...
    protected override async ValueTask<IAvailabilityStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisAvailabilityStore.PrepareAsync(configuration);
        return await RedisAvailabilityStore.ConnectAsync(configuration, timeProvider);
    }
}