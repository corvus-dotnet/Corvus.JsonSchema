// <copyright file="RedisCatalogStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared catalog-conformance suite against <see cref="RedisWorkflowCatalogStore"/> over a real Redis
/// server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisCatalogStoreConformanceTests : WorkflowCatalogStoreConformance
{
    ...
    protected override async ValueTask<IWorkflowCatalogStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisWorkflowCatalogStore.PrepareAsync(configuration);
        return await RedisWorkflowCatalogStore.ConnectAsync(configuration, timeProvider);
    }
}