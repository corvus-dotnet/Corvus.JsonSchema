// <copyright file="RedisDraftRunStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared draft-run-store conformance suite against <see cref="RedisDraftRunStore"/> over a real Redis
/// server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisDraftRunStoreConformanceTests : DraftRunStoreConformance
{
    ...
    protected override async ValueTask<IDraftRunStore> CreateStoreAsync()
    {
        ...
        await RedisDraftRunStore.PrepareAsync(configuration);
        return await RedisDraftRunStore.ConnectAsync(configuration);
    }
}