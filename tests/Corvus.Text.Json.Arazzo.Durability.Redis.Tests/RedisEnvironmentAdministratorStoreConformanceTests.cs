// <copyright file="RedisEnvironmentAdministratorStoreConformanceTests.cs" company="Endjin Limited">
...
/// <see cref="RedisEnvironmentAdministratorStore"/> over a real Redis server in a container. Each test gets an empty store
/// (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisEnvironmentAdministratorStoreConformanceTests : EnvironmentAdministratorStoreConformance
{
    ...
    protected override async ValueTask<IEnvironmentAdministratorStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisEnvironmentAdministratorStore.PrepareAsync(configuration);
        return await RedisEnvironmentAdministratorStore.ConnectAsync(configuration, timeProvider);
    }
}