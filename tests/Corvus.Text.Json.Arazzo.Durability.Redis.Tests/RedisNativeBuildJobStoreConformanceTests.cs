// <copyright file="RedisNativeBuildJobStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="NativeBuildJobStoreConformance"/> suite against <see cref="RedisNativeBuildJobStore"/> over a
/// real Redis server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisNativeBuildJobStoreConformanceTests : NativeBuildJobStoreConformance
{
    ...
    protected override async ValueTask<INativeBuildJobStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisNativeBuildJobStore.PrepareAsync(configuration);
        return await RedisNativeBuildJobStore.ConnectAsync(configuration, timeProvider);
    }
}