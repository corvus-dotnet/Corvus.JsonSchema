// <copyright file="RedisObservedIdentityStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="ObservedIdentityStoreConformance"/> suite against <see cref="RedisObservedIdentityStore"/>
/// over a real Redis server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisObservedIdentityStoreConformanceTests : ObservedIdentityStoreConformance
{
    ...
    protected override async ValueTask<IObservedIdentityStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisObservedIdentityStore.PrepareAsync(configuration);
        return await RedisObservedIdentityStore.ConnectAsync(configuration, timeProvider);
    }
}