// <copyright file="RedisSecurityPolicyStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="SecurityPolicyStoreConformance"/> suite against <see cref="RedisSecurityPolicyStore"/>
/// over a real Redis server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisSecurityPolicyStoreConformanceTests : SecurityPolicyStoreConformance
{
    ...
    protected override async ValueTask<ISecurityPolicyStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisSecurityPolicyStore.PrepareAsync(configuration);
        return await RedisSecurityPolicyStore.ConnectAsync(configuration, timeProvider);
    }
}