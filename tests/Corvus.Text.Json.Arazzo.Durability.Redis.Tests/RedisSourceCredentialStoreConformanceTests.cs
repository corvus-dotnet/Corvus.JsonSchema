// <copyright file="RedisSourceCredentialStoreConformanceTests.cs" company="Endjin Limited">
...
/// <summary>
/// Runs the shared <see cref="SourceCredentialStoreConformance"/> suite against <see cref="RedisSourceCredentialStore"/>
/// over a real Redis server in a container. Each test gets an empty store (the database is flushed).
/// </summary>
[TestClass]
[TestCategory("integration")]
[TestCategory("docker")]
public sealed class RedisSourceCredentialStoreConformanceTests : SourceCredentialStoreConformance
{
    ...
    protected override async ValueTask<ISourceCredentialStore> CreateStoreAsync(TimeProvider timeProvider)
    {
        ...
        await RedisSourceCredentialStore.PrepareAsync(configuration);
        return await RedisSourceCredentialStore.ConnectAsync(configuration, timeProvider);
    }
}