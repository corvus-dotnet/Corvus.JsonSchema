...
public class WorkflowExecutorProviderTests
{
    ...
    public async Task Durable_default_produces_a_loadable_host_adapter_bound_to_the_package_by_a_valid_manifest()
    {
        ...
        try
        {
            ...
            WorkflowRunResultKind kind = await hosted.RunAsync(hosted.Descriptor.Sources.ToDictionary(s => s, _ => (IApiTransport)transport, System.StringComparer.Ordinal), Corvus.Text.Json.Arazzo.Durability.WorkflowTransports.NoMessageTransports, workspace, inputs.RootElement, run, default);
