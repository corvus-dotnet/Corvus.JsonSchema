...
public class NatsTransportTests
{
    ...
    public async Task HandlerErrorWithSkipContinuesDelivery()
    {
        ...
        ReadOnlyMemory<byte> channel = "test.skip-continues"u8.ToArray();
    ...
    public async Task DeserializationErrorWithSkipContinuesDelivery()
    {
        ...
        ReadOnlyMemory<byte> channel = "test.deser-skip-continues"u8.ToArray();
        using var received = new SemaphoreSlim(0, 1);
        ...
        // Act — publish invalid JSON (triggers deser error → skip)
        NATS.Client.Core.NatsConnection rawConn = new(new NATS.Client.Core.NatsOpts { Url = NatsFixture.ConnectionString });
        await rawConn.ConnectAsync();
        await rawConn.PublishAsync("test.deser-skip-continues", "NOT JSON!!!"u8.ToArray());
        await Task.Delay(500);
