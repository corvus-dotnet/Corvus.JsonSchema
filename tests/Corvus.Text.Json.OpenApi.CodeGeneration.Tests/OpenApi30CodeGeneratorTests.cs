...
public class OpenApi30CodeGeneratorTests
{
    ...
    public void GenerateServer_ContextSourceBody_EmitsClosureFreeGenericFactory()
    {
        ...
        OpenApi30CodeGenerator gen = new("CovTest.Server", CoverageSchemaTypeMap, contextSourceBodyPointers: contextBodies);
    ...
    public void GenerateServer_OctetStreamResponse_EmitsBinaryBodyFactoryAndWrite()
    {
        ...
        // Gap 2a: the Result struct's Ok factory carries a binary body, offering both a buffered
        // (ReadOnlyMemory<byte>) and a streaming (Func<Stream, CancellationToken, ValueTask>)
        // overload, plus a WriteBinaryBodyAsync method to stream the body to the response.
        ...
        Assert.IsTrue(
            ...
            "Result should expose an Ok(ReadOnlyMemory<byte>) factory");
        Assert.IsTrue(
            result.Content.Contains("public static DownloadResult Ok(Func<Stream, CancellationToken, ValueTask> writeBody", StringComparison.Ordinal)
            || result.Content.Contains("public static DownloadResult Ok(System.Func<Stream, CancellationToken, ValueTask> writeBody", StringComparison.Ordinal),
            "Result should expose an Ok(Func<Stream, CancellationToken, ValueTask>) streaming factory");
        Assert.IsTrue(
            result.Content.Contains("public ValueTask WriteBinaryBodyAsync(Stream stream, CancellationToken cancellationToken)", StringComparison.Ordinal),
            "Result should expose WriteBinaryBodyAsync");

        // Gap 2b: the endpoint streams the raw bytes to the response body via WriteBinaryBodyAsync.
        GeneratedFile registration = files.First(f => f.FileName == "ApiEndpointRegistration.cs");
        Assert.IsTrue(
        ...
        Assert.IsTrue(
            ...
            "Endpoint should stream the raw binary body to the response stream");
    }
    ...
    public void Generate_ParamRefIntegerSchemasUseFormattingNotGetUtf8String()
    {
        ...
        OpenApi30CodeGenerator generator = new("ParamRef.Client", ParamRefSchemaTypeMap());
