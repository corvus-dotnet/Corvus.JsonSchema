...
public class GeneratedServerEndToEndTests
{
    ...
    [TestMethod]
    public async Task CreateItem_WithCorrelationIdHeader_ReturnsCreated()
    {
        HttpRequestMessage request = new(HttpMethod.Post, "/items")
        {
            Content = new StringContent("""{"name":"Widget","tag":"tools"}""", Encoding.UTF8, "application/json"),
        };
    ...
    [TestMethod]
    public async Task ExportData_WhenRequested_ReturnsOk()
    {
        HttpResponseMessage response = await Client.GetAsync("/export");

        // The export endpoint returns its data body (the mock returns "export-data"); assert the real contract.
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
    ...
    {
        HttpResponseMessage response = await Client.GetAsync("/advanced-styles/.id1.id2?limit=1.5");
    ...
    [TestMethod]
    public async Task SearchItemsAdvanced_WithInvalidJsonBody_ReturnsBadRequest()
    {
        HttpRequestMessage request = new(HttpMethod.Post, "/search")
        {
    ...
    [TestMethod]
    public async Task SearchItems_WithInvalidFilterQuery_ReturnsBadRequest()
    {
    ...
    public async Task GetAdvancedStyles_WithInvalidResponseBody_ReturnsInternalServerError()
        => await AssertInvalidResponseBodyAsync(() => Client.GetAsync("/advanced-styles/.id1.id2?limit=10"));
    ...
    public async Task SearchItems_WithInvalidResponseBody_ReturnsInternalServerError()
        => await AssertInvalidResponseBodyAsync(() => Client.SendAsync(CreateSearchRequest("/search?q=test&filter%5Bstatus%5D=active")));
    ...
    public async Task UploadAttachment_WithInvalidResponseBody_ReturnsInternalServerError()
        ...
        {
            MultipartFormDataContent content = new();
            content.Add(new ByteArrayContent("document"u8.ToArray()), "file", "doc.pdf");
            HttpRequestMessage request = new(HttpMethod.Post, "/attachments") { Content = content };
            request.Headers.Add("X-Upload-Token", "token-123");
            return await Client.SendAsync(request);
        });
    ...
    private static async Task AssertProblemDetailsAsync(HttpResponseMessage response, HttpStatusCode expectedStatusCode, string expectedDetail)
    {
        ...
        await AssertJsonResponseAsync(response, expectedStatusCode, $$"""{"type":"about:blank","title":"{{title}}","status":{{(int)expectedStatusCode}},"detail":"{{expectedDetail}}"}""");
    }
}