...
public readonly partial struct GetPreferencesOk
{
...
#endif
    {
        ...
        public static explicit operator Mutable(GetPreferencesOk instance)
        {
        ...
        /// <returns>An immutable instance of a <see cref="GetPreferencesOk"/>, initialized from the <see cref="Mutable"/> value.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static implicit operator GetPreferencesOk(Mutable instance)
        {
        ...
        public void SetTheme(in CanonTests32.Client.Models.JsonString.Source value)
        {
            ...
            if (value.IsUndefined)
            {
                CodeGenThrowHelper.ThrowInvalidOperationException_SetRequiredPropertyToUndefined("theme");
            }

            ComplexValueBuilder cvb = ComplexValueBuilder.Create(_parent, 2);
            if (_parent.TryGetNamedPropertyValue(_idx, JsonPropertyNames.ThemeUtf8, out IJsonDocument? elementParent, out int elementIdx))
            {
                // We are going to replace just the value
                value.AddAsItem(ref cvb);
                _parent.OverwriteAndDispose(_idx, elementIdx, elementIdx + elementParent.GetDbSize(elementIdx, true), 1, ref cvb);
            ...
            {
                // We are going to insert the new value
                value.AddAsPrebakedProperty(JsonPropertyNamesPrebaked.Theme, ref cvb);
                int endIndex = _idx + _parent.GetDbSize(_idx, false);
                _parent.InsertAndDispose(_idx, endIndex, ref cvb);
            }
        ...
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
        private string DebuggerDisplay => $"GetPreferencesOk.Mutable: ValueKind = {ValueKind} : \"{ToString()}\"";
        ...
        /// </para>
        /// </remarks>
        public readonly GetPreferencesOk Clone()
        {
            CheckValidInstance();
        ...
        /// </para>
        /// </remarks>
        public readonly GetPreferencesOk Freeze()
        {
            CheckValidInstance();
            return _parent.FreezeElement<GetPreferencesOk>(_idx);
        }
    }
    ...
    /// <param name="theme">The value of the <c>"theme"</c> property.</param>
    /// <returns>The source from which to build the value.</returns>
    public static Source Build(in CanonTests32.Client.Models.JsonString.Source language, in CanonTests32.Client.Models.JsonString.Source theme)
    {
        return new Source(language, theme);
    }
    ...
    {
        return workspace.CreateBuilder<GetPreferencesOk, Mutable>(this);
    }
}
