...
[TestClass]
public class JsonElementCloneTests
{
    [TestMethod]
...
null
  ...
  [
    ...
    internal static bool IsDisposable(ParsedJsonDocument<JsonElement> document)
    {
        return ((IJsonDocument)document).IsDisposable;
    }

    internal static ParsedJsonDocument<JsonElement> SniffDocument(JsonElement element)
    {
    ...
    private static void CloneAtInner(string innerJson, JsonValueKind valueType)
    {
        ...
        Assert.AreEqual(innerJson, clone.GetRawText());
    }
}
