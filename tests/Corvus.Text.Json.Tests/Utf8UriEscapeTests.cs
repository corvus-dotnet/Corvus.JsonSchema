...
public class Utf8UriEscapeTests
{
    ...
    public void EscapeDataString_EmptyInput()
    {
        ...
        Assert.IsTrue(Utf8Uri.TryEscapeDataString(ReadOnlySpan<byte>.Empty, dest, out int written));
    ...
    public void EscapeDataString_UnreservedPassThrough()
    {
        ...
        Assert.IsTrue(Utf8Uri.TryEscapeDataString(source, dest, out int written));
    ...
    public void EscapeDataString_NonAsciiMultiByte()
    {
        ...
        Assert.IsTrue(Utf8Uri.TryEscapeDataString(source, dest, out int written));
        string result = Encoding.UTF8.GetString(dest.Slice(0, written).ToArray());
        Assert.AreEqual("caf%C3%A9", result);
    }
    ...
    public void EscapeDataString_DestinationTooSmall()
    {
        ...
        Assert.IsFalse(Utf8Uri.TryEscapeDataString(source, dest, out int written));
        Assert.AreEqual(0, written);
    }
    ...
    {
        Span<byte> dest = stackalloc byte[16];
        Assert.IsTrue(Utf8Uri.TryUnescapeDataString(ReadOnlySpan<byte>.Empty, dest, out int written));
        Assert.AreEqual(0, written);
    }
    ...
    public void UnescapeDataString_NoEscapes()
    {
        byte[] source = "hello-world"u8.ToArray();
        Span<byte> dest = stackalloc byte[64];
        Assert.IsTrue(Utf8Uri.TryUnescapeDataString(source, dest, out int written));
        Assert.AreEqual(source.Length, written);
        Assert.IsTrue(source.AsSpan().SequenceEqual(dest.Slice(0, written)));
    }
    ...
    public void UnescapeDataString_DestinationTooSmall()
    {
        ...
        Assert.IsFalse(Utf8Uri.TryUnescapeDataString(source, dest, out int written));
        Assert.AreEqual(0, written);
    }
    ...
    public void EscapeDataString_RoundTrip(string input)
    {
        ...
        Assert.IsTrue(Utf8Uri.TryEscapeDataString(source, escaped, out int escapedLen));

        Span<byte> unescaped = stackalloc byte[source.Length + 16];
        Assert.IsTrue(Utf8Uri.TryUnescapeDataString(escaped.Slice(0, escapedLen), unescaped, out int unescapedLen));

        Assert.IsTrue(source.AsSpan().SequenceEqual(unescaped.Slice(0, unescapedLen)));
    }
