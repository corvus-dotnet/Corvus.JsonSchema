// <copyright file="YamlToJsonBasicTests.cs" company="Endjin Limited">
// Copyright (c) Endjin Limited. All rights reserved.
// </copyright>

using System.Text;
#if STJ
using System.Text.Json;
using Corvus.Yaml;
#else
using Corvus.Text.Json;
using Corvus.Text.Json.Yaml;
#endif
using Microsoft.VisualStudio.TestTools.UnitTesting;

#if STJ
namespace Corvus.Yaml.SystemTextJson.Tests;
#else
namespace Corvus.Text.Json.Yaml.Tests;
#endif

/// <summary>
/// Basic smoke tests for YAML to JSON conversion.
/// </summary>
[TestClass]
public class YamlToJsonBasicTests
{
    [TestMethod]
    public void SimpleMapping()
    {
        byte[] yaml = "name: hello\nvalue: 42"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(JsonValueKind.Object, root.ValueKind);
        Assert.AreEqual("hello", root.GetProperty("name"u8).GetString());
        Assert.AreEqual(42, root.GetProperty("value"u8).GetInt32());
    }

    [TestMethod]
    public void SimpleSequence()
    {
        byte[] yaml = "- one\n- two\n- three"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(JsonValueKind.Array, root.ValueKind);
        Assert.AreEqual(3, root.GetArrayLength());
        Assert.AreEqual("one", root[0].GetString());
        Assert.AreEqual("two", root[1].GetString());
        Assert.AreEqual("three", root[2].GetString());
    }

    [TestMethod]
    public void NullValues()
    {
        byte[] yaml = "a: null\nb: ~\nc:"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(JsonValueKind.Null, root.GetProperty("a"u8).ValueKind);
        Assert.AreEqual(JsonValueKind.Null, root.GetProperty("b"u8).ValueKind);
        Assert.AreEqual(JsonValueKind.Null, root.GetProperty("c"u8).ValueKind);
    }

    [TestMethod]
    public void BooleanValues()
    {
        byte[] yaml = "a: true\nb: false"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.IsTrue(root.GetProperty("a"u8).GetBoolean());
        Assert.IsFalse(root.GetProperty("b"u8).GetBoolean());
    }

    [TestMethod]
    public void NestedMapping()
    {
        byte[] yaml = "outer:\n  inner:\n    value: deep"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual("deep", root.GetProperty("outer"u8).GetProperty("inner"u8).GetProperty("value"u8).GetString());
    }

    [TestMethod]
    public void DoubleQuotedScalar()
    {
        byte[] yaml = "key: \"hello world\""u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual("hello world", root.GetProperty("key"u8).GetString());
    }

    [TestMethod]
    public void SingleQuotedScalar()
    {
        byte[] yaml = "key: 'hello world'"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual("hello world", root.GetProperty("key"u8).GetString());
    }

    [TestMethod]
    public void FlowMapping()
    {
        byte[] yaml = "{a: 1, b: 2}"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(JsonValueKind.Object, root.ValueKind);
        Assert.AreEqual(1, root.GetProperty("a"u8).GetInt32());
        Assert.AreEqual(2, root.GetProperty("b"u8).GetInt32());
    }

    [TestMethod]
    public void FlowSequence()
    {
        byte[] yaml = "[1, 2, 3]"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(JsonValueKind.Array, root.ValueKind);
        Assert.AreEqual(3, root.GetArrayLength());
        Assert.AreEqual(1, root[0].GetInt32());
        Assert.AreEqual(2, root[1].GetInt32());
        Assert.AreEqual(3, root[2].GetInt32());
    }

    [TestMethod]
    public void DocumentStart()
    {
        byte[] yaml = "---\nkey: value"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual("value", root.GetProperty("key"u8).GetString());
    }

    [TestMethod]
    public void Comments()
    {
        byte[] yaml = "# comment\nkey: value # inline comment"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual("value", root.GetProperty("key"u8).GetString());
    }

    [TestMethod]
    public void IntegerFormats()
    {
        byte[] yaml = "dec: 123\nhex: 0xFF\noct: 0o77"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(123, root.GetProperty("dec"u8).GetInt32());
        Assert.AreEqual(255, root.GetProperty("hex"u8).GetInt32());
        Assert.AreEqual(63, root.GetProperty("oct"u8).GetInt32());
    }

    [TestMethod]
    public void FloatValues()
    {
        byte[] yaml = "pi: 3.14\nneg: -1.5"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(3.14, root.GetProperty("pi"u8).GetDouble());
        Assert.AreEqual(-1.5, root.GetProperty("neg"u8).GetDouble());
    }

    [TestMethod]
    public void InfinityAndNan()
    {
        byte[] yaml = "inf: .inf\nninf: -.inf\nnan: .nan"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        // .inf/.nan have no JSON representation → mapped to null
        Assert.AreEqual(JsonValueKind.Null, root.GetProperty("inf"u8).ValueKind);
        Assert.AreEqual(JsonValueKind.Null, root.GetProperty("ninf"u8).ValueKind);
        Assert.AreEqual(JsonValueKind.Null, root.GetProperty("nan"u8).ValueKind);
    }

    [TestMethod]
    public void EmptyDocument()
    {
        byte[] yaml = ""u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(JsonValueKind.Null, root.ValueKind);
    }

    [TestMethod]
    public void CompactNotation()
    {
        byte[] yaml = "- name: Alice\n  age: 30\n- name: Bob\n  age: 25"u8.ToArray();
        using var doc = YamlTestHelper.Parse(yaml);
        var root = doc.RootElement;

        Assert.AreEqual(JsonValueKind.Array, root.ValueKind);
        Assert.AreEqual(2, root.GetArrayLength());
        Assert.AreEqual("Alice", root[0].GetProperty("name"u8).GetString());
        Assert.AreEqual(30, root[0].GetProperty("age"u8).GetInt32());
        Assert.AreEqual("Bob", root[1].GetProperty("name"u8).GetString());
        Assert.AreEqual(25, root[1].GetProperty("age"u8).GetInt32());
    }
}